...
import { resolveSignedUrl, resolveSignedUrls, extractStoragePath } from "@/lib/signedStorageUrl";
import { getLocalMediaUrl, pathFromLocalUrl, deleteLocalMedia } from "@/lib/mediaCache";
...
// The on-device snapshot stores each item's RAW storage path, never a signed
// or blob: URL (signed URLs expire; blob: URLs die with the session).

/** The gallery-bucket storage path behind a display URL (blob:, signed, or raw). */
function galleryPathOf(url: string | null | undefined): string | null {
  if (!url) return null;
  const local = pathFromLocalUrl(url);
  if (local) return local.bucket === "gallery" ? local.path : null;
  return extractStoragePath("gallery", url);
}

const toRawItem = (i: GalleryItem): GalleryItem => {
  const path = galleryPathOf(i.file_url);
  return path ? { ...i, file_url: `/gallery/${path}` } : i;
};

/** Cached items → displayable, using ONLY the on-device media cache (instant, no signing round trip). */
async function hydrateItemsFromLocal(raw: GalleryItem[]): Promise<GalleryItem[]> {
  return Promise.all(raw.map(async (i) => {
    const path = galleryPathOf(i.file_url);
    const local = path ? await getLocalMediaUrl("gallery", path) : null;
    // null (not ""): an empty src makes some engines re-request the page itself.
    return { ...i, file_url: (local ?? null) as unknown as string };
  }));
}
...
const MediaViewer = ({
  ...
  // Real two-finger pinch-zoom + pan + double-tap-zoom — extracted into
  // usePinchZoomPan (shared with Chat's PhotoViewer) so both surfaces stay
  // in sync instead of drifting apart. See that hook's own header comment
  // for why it writes transforms straight to the DOM via ref instead of
  // through React state: that's what keeps a real pinch smooth/lag-free —
  // routing every pointermove through setState was the root cause the
  // ROOT-CAUSE FIX comment above originally chased down. Resets whenever
  // the viewer moves to a different item (resetKey = item?.id) so a
  // zoomed-in transform doesn't carry over onto unrelated media.
  const { containerRef: mediaContainerRef, imgRef: imgElRef, isZoomed, isPinching, onPointerDown: handlePointerDown, onPointerMove: handlePointerMove, onPointerUp: handlePointerUp, onImageTap: handleImageTap } = usePinchZoomPan(item?.id);

  // Live drag-to-dismiss/navigate (WhatsApp-style): dragX/dragY are Framer
  // Motion values, not React state, driving the motion.div's own drag
  // directly — the backdrop fade and photo scale-down below track them via
  // useTransform, which runs off Framer's rAF loop rather than a
  // per-pointermove re-render, so a fast flick stays smooth.
  const dragX = useMotionValue(0);
  const dragY = useMotionValue(0);
  const backdropOpacity = useTransform(dragY, [-240, 0, 240], [0.3, 1, 0.3]);
  const photoScale = useTransform(dragY, [-240, 0, 240], [0.9, 1, 0.9]);
      ...
      if (Capacitor.isNativePlatform()) {
        ...
        try {
          ...
          if (navigator.canShare?.({ files: [file] })) {
            ...
            // A blob: URL (photo served from the on-device cache) is only valid on
            // this phone — it is not a link anyone can open. Only real links go out.
            if (!item.file_url.startsWith("blob:")) await navigator.share({ url: item.file_url, title: "Shared from DuoSpace" });
          }
        } catch { /* user cancelled */ }
      } else {
        if (item.file_url.startsWith("blob:")) {
          toast({ title: "Can't copy a link offline", description: "Open this photo while online to copy its link." });
        } else {
  ...
  return (
    ...
    >
      ...
      <div ref={mediaContainerRef} className="flex-1 flex items-center justify-center relative" onClick={(e) => e.stopPropagation()}>
        ...
        <motion.div
          key={item.id}
          // Gated on the CONTINUOUS isPinching/isZoomed signal, not a
          // discrete double-tap boolean — see this component's top-of-file
          // ROOT-CAUSE FIX comment. Disarmed for the entire duration of a
          // real pinch, and while already zoomed in (so a single finger
          // pans the image via the pointer handlers below instead of
          // Motion reading it as a swipe).
          drag={!isPinching && !isZoomed}
          dragConstraints={{ left: 0, right: 0, top: 0, bottom: 0 }}
          dragElastic={0.3}
          // x/y are our own Framer Motion values (see backdropOpacity/
          // photoScale above) — passing them via style hands Framer the
          // exact values to drive during the gesture, instead of Framer
          // tracking the position internally where nothing outside this
          // element could read it live. That's what lets the backdrop
          // fade and photo shrink track the drag in real time.
          ...
          onDragEnd={(_: unknown, info: PanInfo) => {
            ...
            // navigates prev/next, same threshold logic as before, now
            // with a velocity check too so a quick short flick dismisses
            // the same way a slower long drag does (matches the feel of
            // WhatsApp's own pull-down-to-close).
            const draggedDown = Math.abs(info.offset.y) > Math.abs(info.offset.x);
            if (draggedDown && (info.offset.y > 100 || info.velocity.y > 800)) {
              onClose();
              return;
            }
        ...
        >
          {item.file_type === "video" ? (
            ...
            <img
              ...
              // why that combination was the actual bug). The pinch/pan
              // transform itself is written straight to this element by
              // usePinchZoomPan via imgElRef, not through a style prop
              // here — see that hook's header comment for why (skips a
              // React re-render on every pointermove, which is what keeps
              // a real pinch lag-free).
...
const Gallery = () => {
  const { user } = useAuth();
  const { toast } = useToast();
  // OFFLINE-CLARITY IMPROVEMENT: distinguishes "no photos yet" from "you're
  // offline and this device has never cached any" in the empty state below —
  // an empty grid read as the app being stuck/broken with no way to tell.
  ...
  // ── OFFLINE-FIRST (2026-09-21) ──────────────────────────────────────────────
  // The gallery used to fetch every list on every open, and a failed read came
  // back as an EMPTY list — so offline the grid was empty. Now the last known
  // gallery (item rows + albums, encrypted on this phone — lib/screenCache.ts)
  // paints first and photos come from the on-device media cache
  // (lib/mediaCache.ts, `gallery` bucket). The network refreshes it; a failed
  // refresh never clears what's showing.
  type GallerySnapshot = {
    partnerId: string | null; partnerName: string; myGalleryShared: boolean;
    myItems: GalleryItem[]; partnerItems: GalleryItem[];
    albums: GalleryAlbum[]; albumItemIds: Record<string, string[]>;
  };
  ...
  const fetchGalleryPage = async (
    ...
    // A failed read used to come back as [] — which the callers then treated as
    // "this person has no photos" and replaced the whole gallery with. Offline,
    // the grid emptied itself. It's an error, not an empty page.
  ...
  const loadGallery = useCallback(async (uid: string, pid: string | null) => {
    try {
      ...
      const partner = pid ? await fetchGalleryPage(pid) : null;
      const partnerList = partner ? await resolveGalleryItems(partner) : [];
      // Commit only once EVERYTHING answered, so a half-successful refresh
      // (e.g. connection drops between the two queries) never leaves the
      // screen with fresh photos of mine next to a stale/empty partner tab.
      hasDataRef.current = true; fetchedRef.current = true;
      setMyItems(myList);
      ...
      // Offline / server hiccup: keep whatever is on screen (the on-device
      // snapshot, or the previous load). See the offline-first note above.
    }
  }, [rebuildShared]);
  ...
  const loadMoreMine = useCallback(async () => {
    ...
    try {
      ...
      let page: GalleryItem[];
      try { page = await fetchGalleryPage(user.id, last); } catch { return; } // offline: keep hasMore so it retries
  ...
  const loadMorePartner = useCallback(async () => {
    ...
    try {
      ...
      let page: GalleryItem[];
      try { page = await fetchGalleryPage(partnerId, last); } catch { return; }
  ...
  const loadAlbums = useCallback(async () => {
    const { data: albumRows, error: albumErr } = await supabase.from("gallery_albums")
      .select("id,owner_id,name,created_at").order("created_at", { ascending: false });
    if (albumErr) return; // offline: keep the albums already showing
    const { data: itemRows, error: itemErr } = await supabase.from("gallery_album_items")
  ...
  useEffect(() => {
    if (!user) return;
    let cancelled = false;
    void readScreenCache<GallerySnapshot>(user.id, "gallery").then(async (snap) => {
      if (!snap || cancelled || fetchedRef.current) return;
      const [mine, theirs] = await Promise.all([hydrateItemsFromLocal(snap.myItems), hydrateItemsFromLocal(snap.partnerItems)]);
      if (cancelled || fetchedRef.current) return;
      hasDataRef.current = true;
      setPartnerId(snap.partnerId);
      setPartnerName(snap.partnerName);
      setMyGalleryShared(snap.myGalleryShared);
      setMyItems(mine);
      setPartnerItems(theirs);
      rebuildShared(mine, theirs);
      setHasMoreMine(snap.myItems.length >= GALLERY_PAGE_SIZE);
      setHasMorePartner(snap.partnerItems.length >= GALLERY_PAGE_SIZE);
      setAlbums(snap.albums ?? []);
      setAlbumItemIds(snap.albumItemIds ?? {});
    });
    return () => { cancelled = true; };
  }, [user?.id, rebuildShared]); // eslint-disable-line react-hooks/exhaustive-deps

  // Keep the snapshot in step with what's on screen — only the newest page of
  // each list (what the grid shows first); older pages page in from the server.
  useEffect(() => {
    if (!user || !hasDataRef.current) return;
    writeScreenCache(user.id, "gallery", {
      partnerId, partnerName, myGalleryShared,
      myItems: myItems.slice(0, GALLERY_PAGE_SIZE).map(toRawItem),
      partnerItems: partnerItems.slice(0, GALLERY_PAGE_SIZE).map(toRawItem),
      albums, albumItemIds,
    } satisfies GallerySnapshot);
  }, [user, partnerId, partnerName, myGalleryShared, myItems, partnerItems, albums, albumItemIds]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (profileErr) return; // offline: the on-device snapshot stays on screen
      if (profile) {
    ...
    load();
  }, [user, loadGallery, loadAlbums, reloadTick]);

  // Back online / app resumed → refresh everything from the server.
  useReconnectRefetch(useCallback(() => setReloadTick((t) => t + 1), []));
  ...
  const saveToGallery = async (
    ...
    if (!requireOnline("Uploading")) return { item: null, error: "You're offline" };
  ...
  const savePartnerPhotoToMyGallery = async (item: GalleryItem) => {
    if (!user) return;
    if (!requireOnline("Saving to your gallery")) return;
  ...
  const toggleShare = async (itemId: string, currentlyShared: boolean) => {
    if (!requireOnline("Changing sharing")) return;
    const { error: shareErr } = await supabase.from("gallery_items").update({ is_shared: !currentlyShared }).eq("id", itemId);
    if (shareErr) { toast({ title: "Couldn't change sharing", variant: "destructive" }); return; }
    setMyItems(prev => {
  ...
  const toggleFavorite = async (item: GalleryItem) => {
    if (!requireOnline("Changing a favorite")) return;
    hapticLight();
  ...
  const createAlbum = async (name: string) => {
    if (!requireOnline("Creating an album")) return;
  ...
  const deleteAlbum = async (albumId: string) => {
    if (!requireOnline("Deleting an album")) return;
    hapticWarning();
  ...
  const deleteItem = async (id: string) => {
    ...
    const doomed = [...myItemsRef.current, ...partnerItemsRef.current].find(i => i.id === id);
    const { error: delErr } = await supabase.from("gallery_items").delete().eq("id", id);
    if (delErr) { toast({ title: "Couldn't delete", variant: "destructive" }); return; }
    // Gone for good → don't keep its photo in the on-device cache.
    const doomedPath = galleryPathOf(doomed?.file_url);
    if (doomedPath) void deleteLocalMedia("gallery", doomedPath);
    setMyItems(prev => prev.filter(i => i.id !== id));
    setSharedItems(prev => prev.filter(i => i.id !== id));
  ...
  const deleteSelected = async () => {
    ...
    const doomedPaths = myItemsRef.current.filter(i => selectedIds.has(i.id)).map(i => galleryPathOf(i.file_url));
    const { error: delErr } = await supabase.from("gallery_items").delete().in("id", ids);
    if (delErr) { toast({ title: "Couldn't delete", variant: "destructive" }); return; }
    doomedPaths.forEach(pth => { if (pth) void deleteLocalMedia("gallery", pth); });
  ...
  const toggleGallerySharing = async () => {
    if (!user) return;
    if (!requireOnline("Changing gallery sharing")) return;
    const newVal = !myGalleryShared;
    const { error: gsErr } = await supabase.from("profiles").update({ gallery_shared: newVal }).eq("user_id", user.id);
    if (gsErr) { toast({ title: "Couldn't change sharing", variant: "destructive" }); return; }
      ...
      return (
        <div className="py-16 text-center px-8">
          ...
          <p className="text-sm text-muted-foreground mb-1">
            {isOffline
              ? "You're offline and nothing's saved here yet"
              : isPartnerGrid ? `${partnerName}'s photos will appear here` : "No photos yet"}
          </p>
          {!isPartnerGrid && (
            <>
              <p className="text-xs text-muted-foreground/70 mb-4">
                {isOffline ? "Open the gallery once online and it'll be here offline too" : "Your shared memories will appear here"}
              </p>
              <button
