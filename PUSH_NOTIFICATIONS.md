...
- **Really wired to automatic DB triggers today:** chat / image / voice /
  file messages, replies, reactions, incoming/missed/ended calls,
  ...
  one accepted request), and songs added to Our Playlist
  (`playlist_songs` — `song_added`, added 2026-09-23; see
  `20260923100000_playlist_song_added_push_notification.sql`).
...
1. A row is inserted into `messages` / `message_reactions` / `call_history`
   (insert or status-changing update) / `partner_requests` / `playlist_songs`.
2. A trigger (`notify_push_on_message`, `_on_reaction`, `_on_call`,
   `_on_partner_request`, `_on_playlist_song`) builds a small JSON payload
   and calls `private.dispatch_push(...)`, which POSTs it to
...
channel per sound choice** — 12 for messages, 12 for calls (v3.11.1; was 4+4) — all created
...
`notify_push_on_call`, `notify_push_on_partner_request`,
`notify_push_on_playlist_song`, and the `private.dispatch_push` /
`sync_push_token_to_push_tokens` helpers. All are
...
## /important and /urgent alerts (2026-09-24)

Two tiers, different sound and feel, both meant to get through silent mode and Do Not Disturb.

| | Important (`/important`) | Urgent (`/urgent`) |
|---|---|---|
| Title | `⚡ Important · <name>` | `🚨 URGENT · <name>` |
| Sound | rising three-note bell, loops ≤ 20 s | two-tone siren + rapid beeps, loops ≤ 60 s |
| Vibration | 3 × 700 ms pulses per cycle | 2 × 1500 ms + 3 × 300 ms pulses per cycle |
| Android channel (fallback only) | `duospace_alert_important_v2` | `duospace_alert_urgent_v2` |
| iOS | `alert_important.caf`, time-sensitive | `alert_urgent.caf`, time-sensitive |

Flow: composer sets `messages.important`/`urgent` → trigger `notify_push_on_message` forwards both (urgent implies important) → `send-push` (`_shared/messageAlert.ts`) sets `data.alertLevel` → Android `DuoSpaceMessagingService` starts `MessageAlertService` (alarm-stream sound + alarm vibration; lifts DND only if the person granted Do Not Disturb access) → JS stops it on app resume. With the chat open the server skips the push and the realtime handler plays the in-app version.

Regenerate sounds: `python3 scripts/generate-alert-sounds.py` (numpy + ffmpeg).

**Critical Alerts (iOS):** to override the silent switch Apple must approve the `com.apple.developer.usernotifications.critical-alerts` entitlement. Only after it is in the provisioning profile and the client requests the `.criticalAlert` authorization option, set the Supabase secret `IOS_CRITICAL_ALERTS=true`.

**Verifying on a device:** Settings → Notifications → "Important & urgent alerts" → Test (uses the real service). Try with the phone on silent, then in Do Not Disturb, then with "Total silence" before and after granting Do Not Disturb access.

**Migration ordering warning:** `notify_push_on_message()` is redefined by several migrations; the latest-dated one wins only if applied last. `20260924100000_message_push_alert_levels.sql` is the current definition — start any future change from it and date it later.
