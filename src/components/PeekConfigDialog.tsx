...
import { Camera, Eye, Users, UserX, Gauge, ShieldCheck, Bug, ScanFace } from "lucide-react";
          ...
          <div className="space-y-5 mt-2">
            ...
            <section className="rounded-xl border border-border/60 p-3 space-y-2">
              ...
              <p className="text-[11px] text-muted-foreground">
                ...
                {enrolled > 0 && " Recognition also quietly improves in the background as Peek Guard confirms it's you, and you can add a fresh set of photos any time below."}
              </p>
              <Button size="sm" variant="secondary" className="w-full"
                onClick={() => { hapticLight(); setEnrollOpen(true); }}>
                {enrolled > 0 ? "Add photos / manage" : "Enroll my face"}
              </Button>
            </section>
            ...
            <section className="space-y-3">
              ...
              <Row icon={Eye} label="No face"
                desc="Lock when the owner isn't there at all"
            ...
            <section className="space-y-4">
              ...
              <SliderRow
                ...
                hint={`${appSettings.peekConsistencyFrames} consecutive frame${appSettings.peekConsistencyFrames === 1 ? "" : "s"}`}
                min={1} max={8} step={1}
              ...
              {appSettings.peekAlertOnNoFace && (
                <SliderRow
                  label="Owner-absent delay"
                  value={appSettings.peekNoFaceSustainMs}
                  hint={`Wait ${(appSettings.peekNoFaceSustainMs / 1000).toFixed(1)}s of an empty frame before treating it as "owner not there" — avoids locking on a quick glance away`}
                  min={500} max={10000} step={500}
                  onChange={(v) => updateSetting("peekNoFaceSustainMs", v)} />
              )}
            ...
            {/* Recovery */}
            <section className="space-y-3">
              <p className="text-[11px] uppercase tracking-wider text-muted-foreground">Recovery</p>
              <Row icon={ScanFace} label="Auto-unlock on owner"
                desc="Clear the lock instantly once you're alone in frame again — no tap needed"
                checked={appSettings.peekAutoUnlockOnOwner ?? true}
                onChange={(v) => updateSetting("peekAutoUnlockOnOwner", v)} />
            </section>

            {/* Advanced */}
            <section className="space-y-3">
              <p className="text-[11px] uppercase tracking-wider text-muted-foreground">Advanced</p>
              <Row icon={ScanFace} label="Require attention"
                desc="Owner-verify also checks eyes open + facing the camera, like Face ID's Require Attention"
                checked={appSettings.peekRequireAttention ?? true}
                onChange={(v) => updateSetting("peekRequireAttention", v)} />
