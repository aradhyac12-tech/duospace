...
async function gatherUserData(userId: string) {
  ...
  return {
    ...
    userId,
    profile,
    // Vanish Mode messages are meant to be deleted — never copy them into a
    // backup that would outlive them (and restore them later).
    messages: (messages || []).filter((m) => !isVanishValue(m.disappear_at)),
    gallery: gallery || [],
  };
}
...
/**
 * Restore is best-effort, NOT transactional: messages and gallery_items are
 * two separate Supabase calls (and messages are written in batches of 100),
 * so a failure partway through can leave some rows written and others not.
 * There is no rollback — undoing already-committed upserts would need a
 * server-side transaction spanning both tables, which the current
 * architecture (two independent REST calls) does not support.
 *
 * To avoid the UI ever claiming a full restore that didn't happen, this
 * function:
 *   - inspects the `error` field on every Supabase response (upsert() does
 *     NOT throw on a DB-level failure — it resolves with { error })
 *   - stops immediately on the first failing batch/table instead of
 *     continuing past it
 *   - throws a specific error identifying which batch/table failed and how
 *     much had already been written before the failure, so the caller can
 *     surface a partial-failure message instead of "done"
 */
async function applyRestore(payload: any, userId: string) {
  ...
  const totalMessageBatches = Math.ceil(messages.length / 100);
  let messagesRestored = 0;
  for (let i = 0, batchIndex = 0; i < messages.length; i += 100, batchIndex++) {
    ...
    if (msgErr) {
      throw new Error(
        `Restore stopped: message batch ${batchIndex + 1} of ${totalMessageBatches} failed ` +
        `(${messagesRestored} of ${messages.length} messages written before the failure). ` +
        `Gallery was not restored. Details: ${msgErr.message}`,
      );
    }
    messagesRestored += batch.length;
  }

  if (gallery.length) {
    const { error: galErr } = await supabase.from("gallery_items").upsert(
      gallery.map((g) => ({ ...g, owner_id: userId })),
      { onConflict: "id", ignoreDuplicates: true },
    );
    if (galErr) {
      throw new Error(
        `Restore stopped: gallery metadata failed to write after ${messagesRestored} of ` +
        `${messages.length} messages were restored successfully. Details: ${galErr.message}`,
      );
    }
  }
  ...
  return {
    messagesRestored,
    galleryRestored: gallery.length,
  };
}
...
export const useCloudBackup = () => {
  ...
  // manualKey: when restoring on a device that didn't create the backup,
  // getOrCreateDeviceSecret() would silently mint a NEW unrelated secret
  // instead of the one the backup was actually encrypted with — decryption
  // would then fail every time, with no way to recover (P0 fix, Final
  // Release Audit). Callers pass the key the user pasted in from
  // "Show encryption key" on the original device; falls back to the local
  // device secret for the normal same-device restore case.
  const restore = useCallback(async (path: string, manualKey?: string) => {
    if (!user) return;
    ...
    try {
      ...
      setProgress(45);
      const secret = manualKey?.trim() ? manualKey.trim() : await getOrCreateDeviceSecret();
      ...
      setError(
        manualKey?.trim()
          ? "Couldn't restore with that key — double check it was copied correctly from the original device."
          : e instanceof Error ? e.message : "Restore failed (wrong device key?)",
      );
      setStatus("error");
    } finally {
      releaseLock();
