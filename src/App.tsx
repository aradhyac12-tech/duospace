...
// BUNDLE FIX (Phase 3 §2/§4, same pattern as MoodDetector in AppLayout.tsx):
// PeekGuard was the one remaining statically-imported detector at the App
// root — component + usePeekDetection.ts + faceRecognition.ts + faceMath.ts
// + moodScoring.ts + peekEventLog.ts (~2,750 lines combined) were all in the
// entry chunk for every session regardless of whether Peek Guard is even
// enabled. The MediaPipe WASM/model load itself was already correctly
// deferred inside faceRecognition.ts (dynamic import) — only the JS that
// decides whether to call it wasn't. Fallback is null since PeekGuard
// renders nothing until its own effects decide there's something to show.
const PeekGuard = lazy(() => import("@/components/PeekGuard"));
...
const ProtectedRoutes = () => {
  ...
  useEffect(() => {
    ...
    const LIKELY_NEXT = ["/gallery", "/groic"] as const;
    const handle = idle(() => {
      LIKELY_NEXT.forEach((path) => { routePreload[path]?.().catch(() => {}); });
    });
    return () => {
  ...
  return (
    <ThemeProvider>
      ...
      <TooltipProvider>
        ...
        <Toaster />
        <Suspense fallback={null}><PeekGuard /></Suspense>
