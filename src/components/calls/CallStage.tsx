...
const CallStage = ({
  ...
  // Only a PURE voice call (no camera on either end) gets the minimal
  // cinematic layout / never-hides controls. If either side turns their
  // camera on mid-call, this steps aside automatically since isVoiceCall
  // depends on isVideoOn — matching the brief: hide-on-idle is a VIDEO
  // call behavior, never a voice one.
  ...
  return (
    ...
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.22, ease: [0.22, 1, 0.36, 1] }}
      ...
      {/* Single-row control bar — a glass-DOCK pill, the SAME material and
          ...
          the standalone dock. Icon-only buttons in a line: Speaker,
          Video, Share (hidden on platforms that can't do it — see
          screenShareSupported), More (audio route/camera-switch/PiP live
          here so nothing is lost), End call — End sits at the right end,
          sized and colored to read as the "different" action it is. */}
      <motion.div
        ...
        <div className="glass-dock rounded-full flex items-center justify-between gap-1 px-2 py-2 max-w-[400px] mx-auto">
          ...
          {screenShareSupported && (
            <button onClick={() => { hapticMedium(); toggleScreenShare(); wake(); }}
              aria-label={isScreenSharing ? "Stop screen share" : "Start screen share"} aria-pressed={isScreenSharing}
              className={`h-14 w-14 shrink-0 rounded-full flex items-center justify-center transition-colors ${isScreenSharing ? "bg-primary" : ""}`}>
          ...
          <button onClick={() => { setShowMoreSheet(v => !v); wake(); }}
            aria-label="More call options" aria-haspopup="menu" aria-expanded={showMoreSheet}
            className={`h-14 w-14 shrink-0 rounded-full flex items-center justify-center transition-colors ${showMoreSheet ? "bg-call-stage-foreground/15" : ""}`}>
            <MoreHorizontal className="h-5 w-5 text-call-stage-foreground" aria-hidden="true" />
          </button>
        ...
        {showMoreSheet && (
          <div className="glass-sheet mx-4 mt-4 rounded-2xl overflow-hidden">
            <p className="text-[11px] text-call-stage-foreground/50 px-4 pt-3 pb-1 uppercase tracking-wider">More options</p>
            {audioRoute.supported && (
              <button onClick={() => { hapticSelection(); setShowRoutePicker(true); setShowMoreSheet(false); }}
                className="w-full flex items-center gap-3 px-4 py-2.5 active:bg-call-stage-foreground/10 text-left">
                <RouteIcon type={audioRoute.current?.type} />
                <span className="text-sm text-call-stage-foreground">Audio output</span>
              </button>
            )}
            {/* CAMERA-SWITCH FIX: this used to open a full device list
                (front, back, plus every extra lens the phone reports —
                ultra-wide, telephoto — as separate confusing entries).
                Now a single tap directly flips front↔back via
                cycleCamera(), same action as the on-screen flip button;
                the picker sheet below is gone entirely. Kept here too
                since a "More" entry is a fine secondary way to reach the
                same action, but it's now a direct toggle, not a menu. */}
            {isVideoOn && cameras.length > 1 && (
              <button onClick={() => { hapticSelection(); void cycleCamera(); setShowMoreSheet(false); }}
                className="w-full flex items-center gap-3 px-4 py-2.5 active:bg-call-stage-foreground/10 text-left">
                <SwitchCamera className="h-4 w-4 text-call-stage-foreground/60 shrink-0" aria-hidden="true" />
                <span className="text-sm text-call-stage-foreground">Switch camera</span>
              </button>
            )}
            {pipSupported && participantCount > 1 && (
              <button onClick={() => { togglePip(); setShowMoreSheet(false); wake(); }}
                aria-pressed={isPip}
                className="w-full flex items-center gap-3 px-4 py-2.5 active:bg-call-stage-foreground/10 text-left">
                <PictureInPicture2 className="h-4 w-4 text-call-stage-foreground/60 shrink-0" aria-hidden="true" />
                <span className="text-sm text-call-stage-foreground">{isPip ? "Exit picture-in-picture" : "Picture-in-picture"}</span>
              </button>
            )}
            <button onClick={() => setShowMoreSheet(false)}
              className="w-full text-center text-[11px] text-call-stage-foreground/40 py-2.5 border-t border-call-stage-foreground/10">
              Cancel
            </button>
          </div>
        )}
        ...
        {showRoutePicker && audioRoute.supported && (
          <div className="glass-sheet mx-4 mt-4 rounded-2xl overflow-hidden">
            <p className="text-[11px] text-call-stage-foreground/50 px-4 pt-3 pb-1 uppercase tracking-wider">Audio Output</p>
            {audioRoute.routes.length === 0 ? (
              <p className="px-4 py-2.5 text-sm text-call-stage-foreground/40">No routes found</p>
              ...
              <button key={route.id}
                ...
                <span className={`text-sm truncate ${audioRoute.current?.id === route.id ? "text-call-stage-foreground font-medium" : "text-call-stage-foreground/70"}`}>
                  {route.name}
                </span>
              </button>
            ))}
            <button onClick={() => setShowRoutePicker(false)}
              className="w-full text-center text-[11px] text-call-stage-foreground/40 py-2.5 border-t border-call-stage-foreground/10">
              Cancel
            </button>
          </div>
