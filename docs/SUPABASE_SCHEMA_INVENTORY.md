...
| `unlink_requests` | Two-sided unlink asks (partner must approve) | `requester_id`/`partner_id` | yes (read-only; writes via RPC) | ✅ | ✅ | PRIVATE_COUPLE |
...
| `surprise-assets` | **public** (SELECT only; INSERT/DELETE policies dropped 2026-09-21 — writes are service_role only; earlier note claiming owner-folder scoping was wrong on live, bucket has 0 objects and no client code uses it) | Code-surprise page assets, intentionally shareable with non-account recipients |
