...
import {
  ...
  Layers, FileText, Image as ImageIcon, Type, Loader2, ChevronRight, CheckCircle2, Pencil,
} from "lucide-react";
...
const Shayari = () => {
  ...
  // Edit (author only): the shayari being edited, its draft text, and whether a save is in flight.
  const [editing, setEditing] = useState<ShayariItem | null>(null);
  ...
  // OFFLINE-FIRST: this screen used to fetch on every open and, on a failed
  // fetch, replace the whole page with an error card — offline you saw none of
  // your shayaris. Now the last known list (encrypted on this phone, see
  // lib/screenCache.ts) paints immediately; the network only refreshes it, and
  // a failed refresh while something is already on screen changes nothing.
  type ShayariSnapshot = { shayaris: ShayariItem[]; profiles: Record<string, { name: string; avatar: string | null }>; partnerId: string | null };
  const hasDataRef = useRef(false);   // something (cached or fetched) is on screen
  const fetchedRef = useRef(false);   // the server has answered this session

  useEffect(() => {
    if (!user) return;
    let cancelled = false;
    void readScreenCache<ShayariSnapshot>(user.id, "shayari").then((snap) => {
      if (cancelled || !snap || fetchedRef.current) return;
      hasDataRef.current = true;
      setShayaris(snap.shayaris);
  ...
  // Keep the on-device snapshot in step with what's on screen (server results,
  // realtime deliveries, my own edits/favourites/deletes).
  useEffect(() => {
    if (!user || !hasDataRef.current) return;
    writeScreenCache(user.id, "shayari", { shayaris, profiles, partnerId } satisfies ShayariSnapshot);
  }, [shayaris, profiles, partnerId, user]);

  // Back online / app resumed → refresh from the server.
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (cancelled) return;
        if (data) { hasDataRef.current = true; fetchedRef.current = true; setShayaris(data); }
        ...
        if (cancelled) return;
        // With the last known list already showing, a failed refresh (offline,
        // dead uplink) is not an error the person needs to see.
        if (!hasDataRef.current) setLoadError(capture("DS-SHAYARI-001", { component: "Shayari", action: "load", cause: err }));
      } finally {
  ...
  const addShayari = async () => {
    ...
    if (!requireOnline("Adding a shayari")) return;
    hapticLight();
  ...
  const openEdit = (s: ShayariItem) => {
    // Author only — the button isn't rendered for the partner's shayaris, and
    // this guard covers any other way of getting here.
    if (!canEditShayari(s, user?.id)) return;
    hapticLight();
    setEditDraft({ title: s.title ?? "", content: s.content });
    setEditing(s);
  };

  const saveEdit = async () => {
    if (!user || !editing || savingEdit) return;
    if (!requireOnline("Saving changes")) return;
    if (!canEditShayari(editing, user.id)) return;

    const result = buildShayariEditPatch(editing, editDraft);
    if (result.status === "invalid") return;
    if (result.status === "unchanged") { setEditing(null); return; }

    hapticLight();
    setSavingEdit(true);
    // .select() so a write the database silently refused (0 rows matched —
    // e.g. not the author) is distinguishable from a real save.
    const { data, error } = await supabase
      .from("shayaris")
      .update(result.patch as any)
      .eq("id", editing.id)
      .eq("user_id", user.id)
      .select("id,user_id,title,content,is_favorite,delete_requested_by,created_at")
      .maybeSingle() as any;
    ...
    if (error) {
      capture("DS-SHAYARI-006", { component: "EditShayariDialog", action: "update", cause: error });
      toast({ title: "Couldn't save changes", description: error.message, variant: "destructive" });
      return; // dialog stays open with the draft intact so nothing they typed is lost
    }
    if (!data) {
      toast({ title: "Couldn't save changes", description: "Only the person who wrote a shayari can edit it.", variant: "destructive" });
      return;
    }
    setShayaris((prev) => prev.map((item) => item.id === data.id ? { ...item, ...(data as ShayariItem) } : item));
    setEditing(null);
    toast({ title: "Shayari updated ✨" });
  };

  const toggleFavorite = async (s: ShayariItem) => {
    if (!requireOnline("Changing a favourite")) return;
    hapticLight();
    const { error: favError } = await supabase.from("shayaris").update({ is_favorite: !s.is_favorite } as any).eq("id", s.id);
    // Don't flip the heart (and the on-device copy) for a write the server refused.
    if (favError) { toast({ title: "Couldn't update favourite", variant: "destructive" }); return; }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.15 }} className="flex-1 min-h-0 overflow-y-auto overscroll-contain pb-24" style={{ WebkitOverflowScrolling: "touch" as any }}>
                ...
                <motion.div key={shayari.id}
                  ...
                  <div className="flex items-center justify-between mt-4 pt-3 border-t border-border/40">
                    ...
                    <div className="flex gap-1.5">
                      ...
                      {canEditShayari(shayari, user?.id) && (
                        <button onClick={() => openEdit(shayari)} aria-label="Edit shayari"
                          className="h-9 w-9 rounded-lg flex items-center justify-center text-muted-foreground transition-all duration-150 active:scale-90">
      ...
      {/* Edit dialog — author only (opened from the pencil on their own cards) */}
      <Dialog open={!!editing} onOpenChange={(o) => { if (!o && !savingEdit) setEditing(null); }}>
        <DialogContent className="rounded-2xl max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><Pencil className="h-5 w-5" /> Edit Shayari</DialogTitle>
            <DialogDescription className="sr-only">Change the title or text of your shayari.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1.5">
              <label htmlFor="edit-shayari-title" className="text-[11px] text-muted-foreground uppercase tracking-wider">Title (optional)</label>
              <Input id="edit-shayari-title" value={editDraft.title} onChange={(e) => setEditDraft({ ...editDraft, title: e.target.value })}
                placeholder="e.g. Mohabbat" className="rounded-xl" disabled={savingEdit} />
            </div>
            <div className="space-y-1.5">
              <label htmlFor="edit-shayari-content" className="text-[11px] text-muted-foreground uppercase tracking-wider">Shayari *</label>
              <Textarea id="edit-shayari-content" value={editDraft.content} onChange={(e) => setEditDraft({ ...editDraft, content: e.target.value })}
                placeholder="Write your shayari here..." className="rounded-xl min-h-[120px] resize-none" rows={5} disabled={savingEdit} />
            </div>
          </div>
          <DialogFooter className="flex gap-2">
            <Button variant="outline" onClick={() => setEditing(null)} disabled={savingEdit} className="flex-1 rounded-xl">Cancel</Button>
            <Button onClick={saveEdit} disabled={!editDraft.content.trim() || savingEdit} className="flex-1 rounded-xl bg-primary text-primary-foreground">
