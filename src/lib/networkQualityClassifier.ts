/**
 * networkQualityClassifier — turns raw WebRTC/the call engine network stats into one
 ...
 * ----------------------------------------------------------
 * The call engine's `network-quality-change` event (still driving the existing
 * degrade-to-audio-only logic in the call engine adapter, untouched by this file)
 ...
 * MUCH shorter window than the call engine's 30s average, so a controller built on
...
/** What a controller (e.g. The call engine adapter) should do at each tier, per the
