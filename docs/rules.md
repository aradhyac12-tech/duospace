...
- **Never ship a change without bumping `APP_VERSION`.** It lives in
  `src/lib/errors/DuoSpaceError.ts` and must be kept in lock-step with
  `package.json`'s `"version"` field — both change together, every time.
  It's shown to the user in Settings ("DuoSpace v1.2.3") and stamped into
  every captured error report, so letting it go stale silently misleads
  both the user and support about what build is actually running. Use
  semver (`MAJOR.MINOR.PATCH`): PATCH for fixes, MINOR for new
  features/phases, MAJOR for breaking changes. This is strict, not
  stylistic — treat "I forgot to bump the version" as a real bug, not a
  nitpick.
