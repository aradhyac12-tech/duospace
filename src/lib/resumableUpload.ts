...
export async function resumableUpload(opts: ResumableUploadOptions): Promise<ResumableUploadResult> {
  ...
  //
  // STRENGTHENED (part of the "Missing chunk" fix — see finalize-upload's
  // matching comment for the full explanation): a short settle delay here,
  // scaled to how much was just written, cuts down on how often finalize's
  // own retry-with-backoff even needs to kick in. Scaled rather than a
  // flat delay because a single small chunk (a voice note, most chat
  // photos/files) is exactly the case with the least natural settle time
  // before this call — a large multi-chunk upload has already spent
  // seconds uploading by the time it gets here, which incidentally gives
  // its chunks more time to settle, so it needs less of a deliberate pause
  // added on top. Capped low enough that it's not a perceptible delay on
  // any real send.
  const settleDelayMs = Math.min(300, 80 + totalChunks * 20);
