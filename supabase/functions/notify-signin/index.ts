...
<html><body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f5f0ec;padding:40px 20px;">
  ...
  // Was fire-and-forget: the fetch response was never checked, so a Resend
  // failure (e.g. the resend.dev-domain 403 restriction, or a rate limit)
  // was swallowed silently and this endpoint still told the caller
  // `emailed: true`. There was no way to tell "this device is genuinely
  // new" apart from "we tried to email and it quietly failed."
  let emailed = false;
  ...
  try {
    ...
    if (!emailRes.ok) {
      ...
      emailError = `send-email responded ${emailRes.status}: ${body.slice(0, 300)}`;
      console.error("notify-signin: email failed", emailError);
    }
  } catch (e) {
  ...
  return new Response(
    JSON.stringify({ known: false, emailed, ...(emailError ? { emailError } : {}) }),
    { headers: jsonHeaders },
  );
});
