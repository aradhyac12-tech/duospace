...
export const logPeekEvent = (input: {
  ...
  const now = Date.now();
  // Coalesce rapid-fire events: if the last event was the same reason and
  // happened within 10s, update it instead of creating a new entry. This
  // prevents the history from being flooded with duplicate entries in
  // persistent crowd scenarios where the alarm fires repeatedly.
  const COALESCE_WINDOW_MS = 10_000;
  if (events.length > 0) {
    const last = events[events.length - 1];
    if (last.reason === input.reason && (now - last.ts) < COALESCE_WINDOW_MS) {
      // Update the existing event: bump timestamp, use the higher threat
      // score, and average the lock speed — gives a more accurate picture
      // than n identical entries.
      last.ts = now;
      last.threatScore = Math.max(last.threatScore, input.threatScore);
      if (input.timeToLockMs != null && last.timeToLockMs != null) {
        last.timeToLockMs = Math.round((last.timeToLockMs + input.timeToLockMs) / 2);
      } else if (input.timeToLockMs != null) {
        last.timeToLockMs = input.timeToLockMs;
      }
      // Preserve feedback if already given
      storage.setJSON(KEY, events);
      return last.id;
    }
  }
  const id = `${now}-${Math.random().toString(36).slice(2, 8)}`;
  events.push({ id, ts: now, feedback: null, ...input });
 ...
 */
// LOOSENED (peek guard false-alarm pass): floor lowered 0.3 → 0.2 so relief
// starts kicking in before a third of rated locks have to be bad first, and
// the cap raised 0.06 → 0.1 so it can meaningfully help someone whose
// day-to-day match score just runs low (skin tone/lighting/camera variance
// the static default can't anticipate), on top of the lower base
// matchThreshold and wider borderline grace period above. Still bounded,
// still only ever loosens, still never overrides ABSOLUTE_MIN_MATCH_THRESHOLD.
const FALSE_ALARM_RATE_FLOOR = 0.2;
const MAX_THRESHOLD_RELIEF = 0.1;
