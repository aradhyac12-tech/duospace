...
const Calls = () => {
  ...
  // `isStartingCall` as still `false` and both proceed: two Daily rooms
  ...
  // same synchronous-race class of bug useDailyCall.ts's own
  ...
  // Daily.co to request at join time on WEB (unchanged) — probing it here
  ...
  // Daily's join() never surfaces the OS camera-permission prompt inside
  ...
  // the camera" — Daily wasn't broken, permission was never granted.
  ...
  const startCallImpl = async (mode: "video" | "voice") => {
    ...
    try {
      ...
      if (callCancelledRef.current) {
        ...
        // Bring the machine to a terminal state too — OUTGOING_SESSION_CREATED
        // first since this is the first point the real DB id is known (see
        // callStateMachine.ts's transition table: CANCELLED is legal from
        // OUTGOING_RINGING, not directly from OUTGOING_PREPARING once a
        // session exists). No separate staleness check needed here beyond
        // what the reducer already does: if something else (e.g. a RESET
        // from a newer call) moved the shared machine on since this
        // attempt started, OUTGOING_SESSION_CREATED is simply no longer a
        // legal transition from wherever it is now and is rejected as a
        // no-op — the transition table itself is the generation guard, a
        // stronger check than this call site re-deriving it from a
        // possibly-stale closure value would be.
        if (callRecord) {
          dispatchCallEvent({ type: "OUTGOING_SESSION_CREATED", callId: (callRecord as { id: string }).id, source: "insert-resolved" });
      ...
      if (callRecord) {
        setCurrentCallId((callRecord as { id: string }).id);
        markCallLatency("call_session_created");
        // NOTE ON ORDERING: OUTGOING_SESSION_CREATED / CONNECTING / CONNECTED
        // are dispatched back-to-back here rather than at the points they
        // conceptually represent (room created / ringing-starts /
        // participant-joined) — the DB insert (which is what supplies the
        // real callId the machine's session tracks) is deliberately fired
        // in parallel with, not before, joinCall() above (see the LATENCY
        // FIX comment on insertPromise), so this is the first moment a
        // callId exists to dispatch OUTGOING_SESSION_CREATED with at all.
        // This means OUTGOING_RINGING/CONNECTING are transient bookkeeping
        // states here, not states this device sits in for any real elapsed
        // time on the caller side — Daily's own callState (still the UI's
        // source of truth, unchanged by this pass) is what genuinely
        // reflects "ringing" during the actual join. Giving OUTGOING_RINGING
        // real duration on the caller side would mean moving the DB insert
        // earlier (ahead of the busy-check bailout point above, since a
        // row can't easily be un-created) — worth doing in a follow-up, not
        // safe to fold into this pass.
        ...
        dispatchCallEvent({ type: "CONNECTED", source: "joinCall-resolved" });
      }
  ...
  const endCall = async () => {
    ...
    dispatchCallEvent({ type: "ENDED", source: "end-call-button" });
    leaveCall();
