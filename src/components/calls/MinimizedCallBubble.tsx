...
// the call itself — and CallContext's shared the call engine call object — was still
...
const MinimizedCallBubble = () => {
  const {
    ...
    leaveCall, activeCallId, setActiveCallId, reattachRemoteVideo, hangUpCall,
  } = useCall();
  const { user } = useAuth();
  ...
  // the brief window between tapping the call button and the call engine's own
  ...
  const handleEnd = useCallback(() => {
    ...
    // Instant hang-up (media/UI now, history in the background) — see CallContext.hangUpCall.
    const callId = activeCallId;
    setActiveCallId(null);
    hangUpCall(callId);
    resumeCameraConsumers("call-end");
  }, [isAcceptingCall, cancelAcceptingCall, activeCallId, setActiveCallId, hangUpCall]);
