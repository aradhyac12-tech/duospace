...
import { getLocalMediaUrl, cacheMediaInBackground, isMediaEphemeral } from "@/lib/mediaCache";

const DEFAULT_TTL_SECONDS = 60 * 60 * 24 * 6; // 6 days

/** Buckets whose media is kept on-device and served from there (see mediaCache.ts). */
const LOCAL_FIRST_BUCKETS = new Set(["chat-files", "memories", "gallery"]);
...
/** Drop the in-memory signed URL for one object (Vanish Mode cleanup). */
...
export async function resolveSignedUrl(
  ...
  // OFFLINE-FIRST (2026-09-21): an on-device copy beats everything below. It
  // works with no network, needs no signing round trip, and is the SAME url
  // string on every call (stable <img src>, no re-decode). See mediaCache.ts.
  // Only for buckets whose consumers cope with a device-local blob: URL:
  // chat photos / voice notes, Memories (display only) and Gallery (its
  // download / share / save-to-gallery paths fetch the BYTES from the URL, which
  // works for blob:; the only link-out fallbacks — "share URL" / "copy link" —
  // detect blob: and decline, see Gallery.tsx).
  // Vanish Mode media is deliberately excluded: it must never be read from or
  // written to the on-device copy (see mediaCache.markMediaEphemeral).
  const localFirst = LOCAL_FIRST_BUCKETS.has(bucket) && !isMediaEphemeral(bucket, path);
  if (localFirst) {
    const local = await getLocalMediaUrl(bucket, path);
  ...
  // Definitely offline and not cached locally: there is nothing to sign and no
  // point spending the 4-attempt backoff below (~2.5s per object, times every
  // media message on screen) just to reach the same "null".
      ...
      for (let attempt = 0; attempt < MAX_RETRY_ATTEMPTS; attempt++) {
        ...
        try {
          ...
          if (!error && data?.signedUrl) {
            ...
            // Keep a local copy for next launch / offline — background,
            // bounded, and never delays returning the URL to the caller.
            if (localFirst) cacheMediaInBackground(bucket, path, data.signedUrl);
