...
// Runtime-verifiable call counter for exchangeCodeForSession(). A PKCE code
// is single-use, so this must read exactly 1 per successful sign-in and
// never increment again for the same code — logged alongside the call so
// it's checkable from Logcat/telemetry rather than only by code review.
let exchangeCallCount = 0;
export function getExchangeCallCount(): number {
...
export async function completeAuthCallback(
  ...
  if (code) {
    ...
    logInfo("auth.callback", "exchangeCodeForSession start", {
      ...
      call_count: exchangeCallCount,
    }, traceId);
    const { data, error: exchangeError } = await supabase.auth.exchangeCodeForSession(code);
    const duration_ms = Math.round(performance.now() - t0);
    if (exchangeError) {
      // Full Supabase error surface — status/code/name/message, never swallowed.
      logError("auth.callback", "exchangeCodeForSession failed", {
        request_id: traceId, duration_ms, call_count: exchangeCallCount,
        status: (exchangeError as { status?: number }).status,
        code: (exchangeError as { code?: string }).code,
        name: exchangeError.name,
        message: exchangeError.message,
        details: (exchangeError as { details?: unknown }).details ?? null,
      }, traceId);
      throw exchangeError;
    }
