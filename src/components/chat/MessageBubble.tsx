...
import {
  Reply, Pin, Pencil, ImageIcon, FileText, Ghost,
} from "lucide-react";
  ...
  return (
    <motion.div id={`msg-${msg.id}`}
      ...
      >
        ...
        } ${(isDisappearing||isVanishing) ? "ring-1 ring-primary/20" : ""} ${isFailed ? "ring-1 ring-destructive/50" : ""}`}
          ...
          {(isLastInGroup || isDisappearing || isVanishing || msg.edited_at || msg._sendStatus) && (
            <div className={`flex items-center gap-1 mt-0.5 ${isMine?"justify-end":""}`}>
              ...
              {/* Vanish Mode: no countdown to show (there's no timer) — a
                  ...
                  ends. */}
              {isVanishing && (
                <Ghost className={`h-2.5 w-2.5 ${isMine ? "text-primary-foreground/70" : "text-muted-foreground/60"}`} aria-label="Vanish message" />
              )}
