...
export const GroicProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const isYouTubeAndPlaying = current?.provider === "youtube" && isPlaying;
    if (isYouTubeAndPlaying && !webKeepAliveActiveRef.current) {
      webKeepAliveActiveRef.current = true;
      nativeEngine.startWebKeepAlive({
        title: current!.title,
        artist: current!.artist,
        artworkUrl: current!.thumbnail ?? current!.artwork ?? undefined,
      }).catch(() => { webKeepAliveActiveRef.current = false; });
    } else if (!isYouTubeAndPlaying && webKeepAliveActiveRef.current) {
  ...
  // existing ~500ms tick rather than adding a new one).
  useEffect(() => {
  ...
  }, [Math.floor(position), isPlaying, duration]);
