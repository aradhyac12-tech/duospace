...
export type BackgroundPermissionStatus = 'granted' | 'denied' | 'notRequired';

export interface DuospaceBackgroundGeolocationPlugin {
  /**
   * ANDROID ONLY (iOS already asks for "Always" itself inside start()/
   * requestImmediateFix() — see BackgroundGeolocationPlugin.swift — so JS
   * callers must gate on Capacitor.getPlatform() === 'android').
   * Current state of "Allow all the time". 'notRequired' on Android 9 and
   * below. Never prompts.
   */
  checkBackgroundPermission(): Promise<{ status: BackgroundPermissionStatus }>;

  /**
   * ANDROID ONLY (KI-12 gap 2). Stores the per-device location-upload
   * credential natively so a push-triggered fix can be uploaded even when the
   * app process has no WebView (see edge functions location-push-register /
   * location-push-upload). uploadUrl must be https.
   */
  setPushUploadCredential(options: {
    userId: string;
    credentialId: string;
    secret: string;
    uploadUrl: string;
  ...
  /** ANDROID ONLY. Which user the stored credential belongs to, or null if none. */
  getPushUploadCredentialUser(): Promise<{ userId: string | null }>;

  /** ANDROID ONLY. Deletes the stored credential (call on sign-out). */
  clearPushUploadCredential(): Promise<void>;

  /**
   * ANDROID ONLY. Requests "Allow all the time". Rejects with code 'denied'
   * if foreground location isn't granted yet (Android ignores a background
   * request until then). On Android 10 this shows a system dialog; on
   * Android 11+ the system sends the person to the app's Location settings
   * page instead and this resolves when they return. Always resolves with
   * the re-read real state, not the raw request result.
   */
  requestBackgroundPermission(): Promise<{ status: BackgroundPermissionStatus }>;
