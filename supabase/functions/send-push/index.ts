...
//
// Invoked ONLY internally, by Postgres triggers (see the fcm_push_notifications.sql
// migration and private.dispatch_push) whenever a message/reaction/call/partner_request/
// unlink_request row changes. These calls carry `Authorization: Bearer <service_role_key>`
// and `{ internal: true, ... }` in the body — fully trusted, since only the database itself
// (via Vault-stored secrets) can present that key.
//
// Direct client calls are rejected (403): no client in the app calls this function, and
// accepting them let any signed-in user push arbitrary notifications to any user id.
//
...
import { mintDeclineToken } from "../_shared/callActionToken.ts";
...
Deno.serve(async (req) => {
  ...
  // Internal (service-role) callers only. The user-JWT path below is intentionally unreachable.
  if (!isInternalCaller) return json({ success: false, error: "Forbidden" }, 403);
...
async function dispatchToRecipient(
  ...
  const { data: prefs } = await admin
    ...
    .eq("user_id", recipientId)
    .maybeSingle();
  // /important bypasses do-not-disturb and a conversation mute the same way
  // an incoming call already does — the sender explicitly flagged this one
  // message as needing to get through. It does NOT bypass a blocked sender,
  // a disabled notification type, or the rate limit above; those are about
  // whether this push should exist at all, not just whether it's quiet.
  const bypassesQuiet = CALL_TYPES.has(body.type) || body.important === true;
  if (prefs) {
    if (prefs.do_not_disturb && !bypassesQuiet) return skip("do_not_disturb");
    if (prefs.muted_until && new Date(prefs.muted_until).getTime() > Date.now() && !bypassesQuiet) return skip("muted");
  ...
  // INSTANT DECLINE: incoming-call pushes carry a short-lived token that lets
  // the Android notification's Decline button decline THIS call as THIS
  // receiver via call-decline, without launching the app. Skipped (old
  // app-launch decline path) when CALL_ACTION_SECRET isn't configured.
  if ((body.type === "incoming_audio_call" || body.type === "incoming_video_call") && body.callId) {
    const actionSecret = Deno.env.get("CALL_ACTION_SECRET");
    const supaUrl = Deno.env.get("SUPABASE_URL");
    if (actionSecret && supaUrl) {
      const { token, exp } = await mintDeclineToken(actionSecret, body.callId, recipientId);
      content.data.declineToken = token;
      content.data.declineExp = String(exp);
      content.data.declineReceiver = recipientId;
      content.data.declineUrl = `${supaUrl.replace(/\/+$/, "")}/functions/v1/call-decline`;
    }
  }
