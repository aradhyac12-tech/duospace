...
/**
 * Forget the cached partner. Called when the pairing ends (see
 * partnerUnlink.ts) and when the server says this account has no partner —
 * otherwise the next cold start would pre-fill the ex-partner into Chat
 * (this cache is a head start, and a head start toward the wrong person is
 * worse than none).
 */
export function clearCachedPartner(userId: string): void {
