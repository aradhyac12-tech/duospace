...
import { getCachedPartner, setCachedPartner } from "@/lib/partnerCache";
...
const Chat = () => {
  ...
  const attemptSendText = useCallback(async (clientId: string, text: string, replyToId: string | null, isSilent: boolean) => {
    ...
    try {
      ...
      // a reload.
      savePendingText({ clientId, partnerId: partnerId!, text, replyToId, isSilent, createdAt: Date.now() });
  ...
  //
  // Deliberately NOT auto-resent like media rehydration above: a plain
  // `insert()` has no server-side idempotency key the way a resumable
  // upload's objectPath does, so if the original insert actually
  // succeeded and only the client crashed before seeing the response,
  // blindly re-sending here would violate the "ONE USER ACTION → ONE
  // INTENDED MESSAGE" invariant (section 2) and produce a real duplicate.
  // fetchMessages() has already loaded the server's current message list
  // by the time this runs, so any entry whose text/timing matches an
  // already-loaded message from this user is treated as already sent and
  // silently cleaned up instead of re-surfaced; anything else is shown as
  // a "failed" bubble the person can tap to retry, same as any other
  // failed send — recovering the message from silent data loss without
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled || pending.length === 0) return;
      setMessages(prev => {
        const alreadySent = (entry: { text: string; createdAt: number }) =>
          prev.some(m => m.sender_id === user.id && m.decryptedContent === entry.text &&
            Math.abs(new Date(m.created_at).getTime() - entry.createdAt) < 5 * 60_000);
        const alreadyPresent = (entry: { clientId: string }) => prev.some(m => m.id === entry.clientId);
        const toRehydrate = pending.filter(e => !alreadyPresent(e) && !alreadySent(e));
        // Anything NOT being rehydrated is either already in `messages`
        // (nothing to do) or confirmed already sent server-side — either
        // way its persisted copy has served its purpose.
        for (const e of pending) {
          if (!alreadyPresent(e) && alreadySent(e)) deletePendingText(e.clientId);
        }
        for (const e of toRehydrate) {
          pendingSendPayloads.current.set(e.clientId, { kind: "text", text: e.text, replyToId: e.replyToId, isSilent: e.isSilent });
        }
        if (toRehydrate.length === 0) return prev;
        return [...prev, ...toRehydrate.map(e => ({
  ...
  const startCall = async (mode:"video"|"voice") => {
    ...
    beginCallLatencyTrace("outgoing", mode);
    ...
    markCallLatency("daily_room_requested");
    ...
    try {
      ...
      markCallLatency("daily_room_ready");
      markCallLatency("token_ready");
