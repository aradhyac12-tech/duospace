...
import { detectFaces, loadOwnerProfile, matchAgainstOwner, getAdaptiveMatchThreshold } from "@/lib/faceRecognition";
  ...
  const verifyOwnerAndDismiss = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        // Match best face against owner
        const bestFace = faces.reduce((a, b) => (a.area > b.area ? a : b));
        const matchScore = matchAgainstOwner(bestFace.embedding, owner);
        const threshold = getAdaptiveMatchThreshold(owner, 0.7);
