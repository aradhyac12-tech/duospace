...
const MapView = () => {
  ...
  const { partnerId, partnerName, partnerAvatar, partnerLocation, partnerDeviceStatus, partnerStale, deviceStatusStale, realtimeOk, transportMode, partnerPresence, now, online } = ctx;
  const myLocation = ctx.myLocation;
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      if (!partnerMarkerRef.current) {
        ...
        partnerMarkerRef.current.on("click", () => { hapticLight(); setStatusSheetOpen(true); });
      } else {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col flex-1 min-h-0 overflow-hidden">
      ...
      >
        ...
        <div className="absolute inset-x-3 z-[1000] flex items-start justify-between gap-2" style={{ top: topInset }}>
          <div className="flex items-start gap-2 min-w-0">
            ...
            <button
              type="button"
              onClick={() => { hapticLight(); setStatusSheetOpen(true); }}
                ...
                <span className="block text-[10.5px] text-foreground-secondary truncate tabular-nums">
                  {!partnerId
                    ...
                    : !partnerLocation
                      ...
                      : `${partnerStale ? "Last seen" : "Online"} · ${timeAgo(partnerLocation.updated_at)}`}
                </span>
              </span>
            </button>
          </div>
