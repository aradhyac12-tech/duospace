...
// PERF: html5-qrcode is a large library that only matters once someone actually taps
// "scan". Type-only import here; the runtime module is loaded on demand below so it
// stays out of the entry bundle for every launch.
...
interface QRSignInScannerProps {
   ...
   */
  onSignupInvite?: (inviterId: string, token: string) => void;
  /**
   ...
   *
   * Also called when a signed-in partner's invite QR was scanned while THIS
   * device is signed in: the server links the two accounts on the spot
   * (`immediate: true`). `immediate: false` means the link is recorded and will
   * complete once the other person finishes creating their account.
   */
  onPartnerLinked?: (info?: { immediate: boolean; partnerName?: string | null }) => void;
}
...
const QRSignInScanner = ({ onClose, onSuccess, onSignupInvite, onPartnerLinked }: QRSignInScannerProps) => {
  ...
  const startScanner = async () => {
    ...
    try {
      ...
      // The screen can close while the library chunk is still loading — never open
      // the camera for a scanner nobody is looking at any more.
  ...
  const handleDecoded = async (text: string) => {
    ...
    try {
      ...
      if (data?.kind === "signup_invite") {
        ...
        onSignupInvite?.(String(data.inviter_id ?? ""), parsed.token);
        return;
      }

      // Signed-in scanner + signed-in partner's invite QR: the server has just
      // linked the two accounts. This is the ONLY place that says "linked" for
      // this case, and only because the server confirmed it.
      if (data?.kind === "partner_linked") {
        logInfo("auth.qr", "redeem partner_linked", { request_id: traceId, status: "ok" }, traceId);
        toast({
          title: "Linked ✓",
          description: data.partner_name ? `You're now partners with ${data.partner_name}.` : "You're now partners.",
        });
        onPartnerLinked?.({ immediate: true, partnerName: data.partner_name ?? null });
        return;
      }
      ...
      if (data?.kind === "anon_signup") {
        logInfo("auth.qr", "redeem anon_signup", { request_id: traceId, status: "ok", linked_partner: !!data.linked_partner }, traceId);
        if (!data.linked_partner) {
          // The scanning device wasn't signed in (or its session was rejected), so
          // the server had nobody to link to. Saying "Linked ✓" here was the bug.
          throw new Error("Scanned, but you're not signed in, so nobody was linked. Sign in on this device and scan again.");
        }
        toast({ title: "Scanned ✓", description: "You'll be linked as soon as they finish creating their account." });
        onPartnerLinked?.({ immediate: false });
        return;
      }
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-foreground/70">
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="text-xs text-background/80">
                {status === "starting" ? "Starting camera..." : "Checking the code..."}
              </span>
            </div>
          </div>
        )}
