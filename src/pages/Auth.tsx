...
import { rememberQrClaim } from "@/lib/qrPartnerClaim";
...
const readableError = getAuthErrorMessage;
...
const Auth = () => {
  ...
  // BUG FIX (no way to resend a confirmation email at all — genuinely
  // missing, not previously broken): when "Confirm email" is on, a signup
  // that never got the email (spam filter, typo, or Resend having a
  // transient issue) had no recovery path in the UI — the person just sits
  // signed-up-but-unconfirmed forever, since trying to sign in only ever
  // showed a generic toast with no follow-up action. Tracks whether the
  // most recent login attempt failed specifically because of that, so the
  // login form can offer an actual "resend" action instead of a dead end.
  ...
  // BUG FIX: this used to register its own appUrlOpen/getLaunchUrl listener
  // right here — but that meant it only ever caught the native deep-link
  // callback while THIS component happened to be mounted. Moved to
  // useNativeAuthDeepLink(), mounted once at the app root (App.tsx) so it's
  // always listening regardless of current route or auth state — see that
  // hook's header comment for exactly which real cases were silently
  // dropping the callback before. This component still owns the *web*
  // callback handling above (window.location-driven — a browser tab
  // navigating to /auth/callback naturally lands on this route on its own,
  // so that path never had the same gap).
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        // BUG FIX: "email not confirmed" was only ever a toast that
        // vanished with no way to act on it. Surface it as a persistent
        // inline state instead so the "Resend confirmation email" button
        // below can appear — see the unconfirmedEmail state doc above.
        if (error.message.toLowerCase().includes("email not confirmed")) {
          setUnconfirmedEmail(email.trim());
        }
        toast({ title: "Couldn't sign in", description: getAuthErrorMessage(error), variant: "destructive" });
      } else {
      ...
      toast({ title: "Sign in error", description: getAuthErrorMessage(err), variant: "destructive" });
    }
    setLoading(false);
  };

  // Resends the signup confirmation email. Separate loading/traceId from
  // handleLogin's own — this can be retried independently of a sign-in
  // attempt (e.g. tapped twice while waiting for the email to land).
  const handleResendConfirmation = async () => {
    if (!unconfirmedEmail) return;
    const traceId = newTraceId("resend_confirm");
    const emailHash = await hashEmail(unconfirmedEmail);
    setResendLoading(true);
    try {
      const redirectUri = buildAuthRedirectUri("email_confirm");
      logInfo("auth.resend", "resend start", {
      ...
      if (error) {
        logError("auth.resend", "resend failed", {
          request_id: traceId, email_hash: emailHash, status: "error", supabase_error: supaErr(error),
        }, traceId);
        toast({ title: "Couldn't resend", description: getAuthErrorMessage(error), variant: "destructive" });
      } else {
        logInfo("auth.resend", "resend ok", { request_id: traceId, email_hash: emailHash, status: "ok" }, traceId);
        toast({ title: "Email sent", description: "Check your inbox for the new confirmation link." });
      }
    } catch (err: unknown) {
      logError("auth.resend", "resend threw", {
        request_id: traceId, email_hash: emailHash, status: "exception", err: supaErr(err),
      }, traceId);
      toast({ title: "Couldn't resend", description: getAuthErrorMessage(err), variant: "destructive" });
    }
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        toast({ title: "Couldn't sign up", description: getAuthErrorMessage(error), variant: "destructive" });
      } else {
        ...
        // BUG FIX: an already-registered email is NOT surfaced as an error
        // by Supabase's signUp() — this is deliberate anti-enumeration
        // behavior on GoTrue's part, not a bug in Supabase. When "Confirm
        // email" is on (the real config now that Resend SMTP is set up, per
        // the comment below) and the email already has an account, signUp()
        // returns {data: {user: <a real-looking but pre-existing user
        // object, identities: []>, session: null}, error: null} — no
        // exception, no error field, nothing to catch. This code's old
        // else-branch only ever checked `!data.session` and showed "Check
        // your email" unconditionally — which is actively misleading for
        // an existing account: no new confirmation email is sent in this
        // case, so the person waits for an email that never arrives,
        // instead of being told to just sign in (or reset their password).
        // `identities` is the documented signal GoTrue itself uses for
        // this — an empty array on a signUp() response (as opposed to
        // being absent/undefined, which is what a genuinely new user
        // looks like) means "this email already has an account."
        const alreadyRegistered = Array.isArray(data.user?.identities) && data.user.identities.length === 0;
        if (alreadyRegistered) {
          logInfo("auth.signup", "email already registered (empty identities)", {
            request_id: traceId, email_hash: emailHash,
          }, traceId);
          toast({
        ...
        // QR partner linking is NOT attempted here. Right after signUp() there is
        // usually no session yet ("Confirm email" is on), so the two RPCs that used
        // to be called here ran as `anon` and were refused — nobody was ever linked.
        // The QR token was remembered when it was scanned (see
        // lib/qrPartnerClaim.ts); ProtectedRoutes (App.tsx) claims the link the
        // moment this account has a session, whether that is now or after the
        // confirmation email.

        // Previously this called a `complete-signup` edge function to
        // auto-mint a session immediately, bypassing email confirmation —
        // built as a workaround for when no SMTP/email provider was
        // configured on the Supabase project and the confirmation email
        // could never arrive. Real Resend SMTP is now configured in
        // Supabase Auth, so that workaround is removed: signup now follows
        // the standard flow — no session until the user actually clicks
        // the confirmation link, which Supabase now sends for real. When
        // "Confirm email" is off at the project level, signUp() already
        // returns a session immediately and this branch is simply skipped.
        if (!data.session) {
          toast({
            title: "Check your email",
            description: "We've sent a confirmation link to verify your DuoSpace account.",
          });
          setAuthTab("login");
        } else {
      ...
      toast({ title: "Sign up error", description: getAuthErrorMessage(err), variant: "destructive" });
    }
    setLoading(false);
  };
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        toast({ title: "Failed", description: getAuthErrorMessage(error), variant: "destructive" });
      } else {
  ...
  // Web OAuth uses Lovable Cloud's managed broker. Native OAuth keeps the
  // existing direct authorization request so Capacitor can receive the
  // provider callback on duospace://auth.
  const startOAuth = async (provider: "google" | "apple", extraQueryParams?: Record<string, string>) => {
    ...
    try {
      ...
      // Web: talk to the connected Supabase project directly (same call the
      // native branch above already makes), instead of routing through
      // Lovable's hosted OAuth broker. That broker's `/~oauth/initiate` is a
      // relative path only Lovable's own hosting resolves — on any other
      // host (Vercel, custom domain, etc.) it 404s to the SPA shell instead
      // of starting the OAuth flow, which is what was surfacing as the
      // Apple sign-in error. `signInWithOAuth` here performs a normal
      // full-page redirect to Supabase's /authorize endpoint; the provider
      // sends the browser back to `redirectUri` with a `?code=...`, which
      // the existing `hasAuthCallback()` / `completeAuthCallback()` effect
      // above already exchanges via `exchangeCodeForSession` — no other
      // change needed to complete sign-in.
    ...
    <div className="min-h-dvh flex items-center justify-center bg-background px-6 relative overflow-hidden">
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <Button
            onClick={() => { hapticMedium(); handleAppleLogin(); }}
            disabled={appleLoading || googleLoading || oauthProcessing}
            variant="outline"
            className="w-full h-12 rounded-xl gap-3 text-sm font-medium"
          >
            {appleLoading ? (
        ...
        <Tabs value={authTab} onValueChange={(v) => setAuthTab(v as "login" | "signup")} className="w-full">
          ...
          <TabsContent value="login">
            <form onSubmit={handleLogin} className="space-y-3 mt-4">
              ...
              {unconfirmedEmail && (
                <div className="rounded-xl bg-info/10 border border-info/20 p-3 space-y-2">
                  <p className="text-xs text-info">
                    Your email isn't confirmed yet. Check your inbox for the confirmation link, or resend it.
                  </p>
                  <Button
                    type="button"
                    size="sm"
                  ...
                  >
                    {resendLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Resend confirmation email"}
                  </Button>
                </div>
              )}
          ...
          <TabsContent value="signup">
            <form onSubmit={handleSignUp} className="space-y-3 mt-4">
              ...
              <div className="space-y-1.5">
                ...
                <PasswordStrengthMeter password={password} className="px-0.5 pt-0.5" />
              </div>
              <Button type="submit" disabled={loading}
                className="w-full h-11 rounded-xl bg-primary text-primary-foreground hover:bg-primary/90 text-sm font-medium">
            ...
            <Tabs value={qrPanel} onValueChange={(v) => setQrPanel(v as "scan" | "display")} className="w-full">
              ...
              <TabsContent value="scan" className="mt-4">
                <QRSignInScanner
                  ...
                  onSignupInvite={(inviterId, token) => {
                    // Remember the scanned token: it is the proof this device needs to
                    // claim the partner link once the new account has a session.
                    if (token) rememberQrClaim(token);
                    if (inviterId) {
              ...
              <TabsContent value="display" className="mt-4">
                <QRSignInDisplay
                  ...
                  onClose={() => setShowQrScanner(false)}
                  onRedeemed={(kind) => {
                    // "signup_invite" (the only mode this display ever shows
                    // from the Auth page) means the OTHER device just got
                    // routed into its own signup form — nothing changes on
                    // THIS device's session, so there's no navigation to do
                    // here, just close the dialog since the QR has done its
                    // job. (kind is always "signup_invite" in this call
                    // site's context; the parameter exists because
                    // QRSignInDisplay is shared across modes.)
                    void kind;
                    toast({ title: "Scanned ✓", description: "They're finishing signup on their device." });
                    setShowQrScanner(false);
                  }}
                />
              </TabsContent>
