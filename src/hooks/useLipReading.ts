...
export const useLipReading = ({ language, onResult, videoRef }: UseLipReadingOptions) => {
  ...
  const MIN_FRAME_MS = 60; // ~16fps cap — faster viseme capture, especially important for short syllables
  const WINDOW        = 16;
  const SILENCE_MS     = 600; // REDUCED: 700ms inter-word gap was clipping final syllables of short words
  ...
  const VOTE_WINDOW = 6; // INCREASED from 5: more frames = more robust against single-frame noise
  ...
  const handleFrame = useCallback((symbol: string, confidence: number) => {
    ...
    if (votedWeight < totalWeight * 0.42 || votedWeight < 0.7) return; // LOWERED: less strict plurality — more words recognized
