...
### 2026-09-20 — Now-playing notification never visible on the phone (Android) / lock-screen state (iOS)

**Symptom:** logcat showed `postNotificationInternal … Now-playing notification
confirmed visible by the OS (id=4271)` for both YouTube- and ExoPlayer-driven
tracks, and KDE Connect (a NotificationListenerService) mirrored the
notification to a PC — but nothing appeared on the phone itself.

**Root cause (best-supported, not device-verified):** the app published up to
four MediaSessions for one package (Media3 session + its internal compat
session + a hand-made legacy `MediaSessionCompat` that the notification's
MediaStyle token pointed at + `WebKeepAliveService`'s second Media3 session
with its own automatic notification). Android 11+ pulls MediaStyle
notifications out of the normal shade and into the system-UI media carousel,
which keys on the notification's session token vs. the package's active
sessions; the playing session and the notification's session were different
objects, so the carousel had nothing consistent to show while the
notification manager (and therefore listener apps) still reported it active.

**Fix:** `MediaPlaybackService` is now a plain foreground `Service` owning ONE
`MediaSessionCompat` + ONE notification for native AND YouTube playback.
`WebKeepAliveService` and the Media3 `MediaSession` were removed (the JS
`startWebKeepAlive/updateWebKeepAlive/stopWebKeepAlive` API and the
`webKeepAliveCommand` event are unchanged and now drive the single service).

Also fixed in the same pass (all visible in the log): per-second full
notification rebuild + `startForeground` from the YouTube position tick and
3-4 posts within 10 ms at every track start (NMS sheds an app's updates above
~5/s) → posts are now de-duplicated by content signature and position ticks
only refresh session state; foreground detach/re-attach on every transient
`isPlaying=false` while ExoPlayer buffers an HLS track → foreground now follows
"wants to play" (`playWhenReady`, not idle/ended); `startForeground` failure
left NO notification → falls back to plain `notify()`; a paused ExoPlayer item
could overwrite the playing YouTube notification; artwork fetch had no
timeouts and no down-sampling. JS (`GroicContext.tsx`): pausing a YouTube
track no longer tears the notification down (nothing left to tap to resume);
track changes re-send metadata; `POST_NOTIFICATIONS` is now also requested on
the YouTube path (it was only requested for native-stream tracks).
iOS (`AudioEnginePlugin.swift`): remote commands explicitly enabled,
`MPNowPlayingInfoCenter.playbackState` and media type set, buffering no
longer reads as "paused".

**If it still doesn't show after this**, run on a connected device while a
track plays: `adb shell dumpsys media_session | grep -A6 "package=<app id>"`
(should list exactly ONE session for the app) and
`adb shell dumpsys notification --noredact | grep -B2 -A25 "id=4271"`
(check `channel`, `importance`, `flags`, and that the MediaStyle
`android.mediaSession` token matches the session above). Also check Settings →
Sound → Media → "Show media player" / OEM "lock-screen media" toggles.
...
## Android release build broken: Kotlin compiler/stdlib version mismatch (2026-09-06)

**Symptom (from an APKForge CI build log, job 7fbebf10):** the Gradle
release build failed outright — no APK/AAB produced — with
`capacitor-geolocation:compileReleaseKotlin`,
`capacitor-filesystem:compileReleaseKotlin` and
`capacitor-camera:compileReleaseKotlin` all `FAILED`, each spewing dozens
of `Unresolved reference: padStart` / `isNotBlank` / `let` / `apply` /
`takeIf` errors plus repeated `Class 'kotlin.Unit' was compiled with an
incompatible version of Kotlin. The actual metadata version is 2.1.0, but
the compiler version 1.9.0 can read versions up to 2.0.0.`

**Root cause:** those three official Capacitor plugins have no
`buildscript{}` block of their own, so they compile with whatever Kotlin
Gradle Plugin version the *root* `android/build.gradle` declares. This
project's Android template doesn't declare one at all (no literal
`kotlin-gradle-plugin` string), which trips a generic repair step in
APKForge's own CI (`NATIVE_MODULE_COMPILE`, step 15 of that build): it
sees Kotlin sources need the Kotlin plugin somewhere and — reasonably in
general, wrongly here — unconditionally injects a hardcoded, stale
`kotlin_version = '1.9.25'` classpath into the root project whenever it
can't find that string. Meanwhile Gradle still resolves those three
modules' actual `kotlin-stdlib` dependency to 2.1.0 (pulled in
transitively via `kotlinx-coroutines-android`/`-play-services`, which
Capacitor 8.x now depends on for geolocation/camera/filesystem) —
compiler 1.9.0 literally cannot read 2.1.0 metadata, hence the cascade.

Confirmed this is CI-side, not a source bug: `native-plugins/*/android/build.gradle`
(our own audio-engine/audio-route/background-geolocation/callkit-bridge/
device-status plugins) already dodge this exact trap — each declares its
own local `buildscript{}` pinning `kotlin-gradle-plugin` to `2.1.0`
regardless of what the root project says — and none of those five showed
up in the failure list. Only the three official plugins that rely on the
root's classpath broke.

**Fix:** `scripts/patch-native-kotlin-versions.mjs`, run as a new
`postinstall` hook in `package.json`. It prepends that exact same local
`buildscript{}` override (pinned to `2.1.0`, matching our own plugins) into
`node_modules/@capacitor/{camera,filesystem,geolocation}/android/build.gradle`
right after `bun install` populates `node_modules` — well before `cap add
android`/`cap sync` ever run, so the generated `android/` project (which
this repo's CI regenerates from scratch every run, since `android/` isn't
committed) picks up the patched copy. Idempotent (checks for its own
marker comment and for a pre-existing `buildscript{}` before touching
anything) and no-ops cleanly if `node_modules/@capacitor/<pkg>` isn't
present yet.

**Deliberately NOT done:**
- Not touching `package.json` dependency versions or `bun.lock` at all —
  APKForge's install step runs `bun install --frozen-lockfile`, so any
  version bump without a correctly regenerated lockfile would just trade
  this failure for an install-time one, and there's no bun toolchain in
  this working environment to regenerate `bun.lock` safely.
- Not using `patch-package` (or Bun's native `patchedDependencies`) even
  though that's the more conventional tool for this — both need a
  `bun.lock`/`package.json` entry recording the patch, hitting the same
  frozen-lockfile risk above. A plain `postinstall` script needs no
  lockfile changes at all.
- Not routed through `scripts/cap:sync`/`cap:patch-permissions` (the
  existing post-`cap add`/`cap sync` repair path for
  AndroidManifest/Info.plist) — confirmed from the CI log that APKForge's
  pipeline calls `npx cap add android` / `npx cap sync` **directly** and
  never runs any of this repo's own `npm`/`bun` scripts, which is exactly
  why the deep-link/permission patches funnel through `cap:patch-permissions`
  but this fix has to be a `postinstall` hook instead — it's the only
  lifecycle point guaranteed to run under that CI.

**Not a real problem, for the record:** the same build log's final
diagnostics section also shows `8 FAIL ... AndroidManifest.xml has no
VIEW/BROWSABLE intent filter`. That's a stale snapshot — the final report
just `cat`s the trace file written *before* APKForge's own
`Validate and repair native configuration` step ran; that step's own log
(`[auto-fix] Added deep-link intent filters for: app.duospace, duospace`)
confirms it was already fixed earlier in the same run, correctly, with a
proper `VIEW` action + `BROWSABLE` category + `android:scheme="duospace"`
filter. No action needed there.

## Follow-up: the postinstall Kotlin-patch script itself broke the build (2026-09-06)

The `scripts/patch-native-kotlin-versions.mjs` / `postinstall` hook added
above (see the Kotlin compiler/stdlib mismatch entry just before this one)
broke the very next CI run one step earlier: `bun install`'s `postinstall`
crashed with

```
SyntaxError: Unexpected token ')'
error: postinstall script from "duospace" exited with 1
PREBUILD_VALIDATION_FAILED: Dependency install failed with bun. The
project's lockfile selects bun, so we do not fall back to another
package manager — fix the failing package or commit a consistent
lockfile.
```

**Cause:** a self-inflicted JSDoc bug. The script's own header comment
described the pattern it's borrowing using the literal path
`native-plugins/*/android/build.gradle` — but `*/` is also how a `/** ... */`
block comment *closes*, so that path text terminated the comment several
lines early, and everything after it (more prose, then real code) got
parsed as JavaScript instead of a comment, producing a syntax error before
the script could ever run.

**Fix:** reworded the comment to `native-plugins/<name>/android/build.gradle`
(no literal `*/` left anywhere inside a block comment). Verified properly
this time before shipping: `node --check` on the file, plus an actual dry
run against a throwaway `node_modules` fixture (patches a plain
`build.gradle` correctly, skips a missing module without throwing, and is
a no-op — not a duplicate patch — on a second run).

**Lesson for future edits to this file specifically:** never reference a
glob-style path (`anything/*/anything`) in prose inside a `/** */` block
comment — use `<name>` or similar as a placeholder instead. More broadly:
a script meant to run inside someone else's `postinstall`/CI hook is
exactly the kind of change that deserves an actual local execution check
(even a throwaway fixture, as above), not just a read-through — `bun
install --frozen-lockfile` has no tolerance for a broken postinstall, and
the failure surfaces one full pipeline stage before you'd see it in a
Gradle log.

## Follow-up #2: the postinstall Kotlin-patch script still no-opped (2026-09-06)

Job cb280f1c failed with the *exact same* `capacitor-geolocation`/
`capacitor-filesystem`/`capacitor-camera` `compileReleaseKotlin` errors as
the original entry above (`compiler version 1.9.0` vs `metadata version
2.1.0`), even though the `postinstall` hook ran successfully this time —
its own log output said, for all three packages, `"already declares a
kotlin-gradle-plugin classpath — leaving it alone."`

**Cause:** `scripts/patch-native-kotlin-versions.mjs` still had a stale
`if (/kotlin-gradle-plugin/.test(original)) return;` guard that bailed out
whenever the vendored `build.gradle` merely *contained* that string —
which it did, via the packages' own insufficient (pre-2.0) classpath pin,
not a sufficient one. The very next block of comments in the same file
already explained, correctly, why this exact check was wrong and unsafe
to keep (prepending a second `buildscript{}` block is safe even when one
already exists, because Gradle's classpath conflict resolution takes the
higher of two same-module versions) — but a previous edit updated only
the comment and never actually deleted the code above it, so the bug it
described as fixed was still live.

**Fix:** deleted the dead guard entirely. The script now always prepends
the 2.1.0 override (still skipping only on its own idempotency marker),
matching what its neighboring comment always said it should do. Verified
with a fixture whose vendored `build.gradle` pre-declares
`kotlin-gradle-plugin:1.9.0` (reproducing the exact shape that tripped
the old guard): the script now patches it, and a second run correctly
no-ops on the idempotency marker instead.

**Lesson:** when a comment says "this used to be a bug, here's why it's
fixed," treat that as a prompt to go verify the code right next to it
actually matches — a rationale can be edited independently of the logic
it's describing, and nothing catches that drift automatically.

## Deep audit: `@capacitor/camera` compile failure is a genuine upstream bug, not our Kotlin-pin (2026-09-06)

With the Kotlin compiler/stdlib mismatch fully resolved (see the entries
above — `capacitor-geolocation`/`capacitor-filesystem` now compile clean,
confirmed by the total absence of any "incompatible Kotlin metadata
version" text in this run's log), one failure remained:
`capacitor-camera:compileReleaseKotlin FAILED` with five real errors:

```
e: .../camera/CameraPlugin.kt:34:30 Cannot access 'CAMERA': it is private in 'Companion'
e: .../camera/CameraPlugin.kt:37:30 Cannot access 'PHOTOS': it is private in 'Companion'
e: .../camera/CameraPlugin.kt:40:30 Cannot access 'SAVE_GALLERY': it is private in 'Companion'
e: .../camera/CameraPlugin.kt:43:30 Cannot access 'READ_EXTERNAL_STORAGE': it is private in 'Companion'
e: .../camera/IonCameraFlow.kt:540:46 Type mismatch: inferred type is Intent? but Intent was expected
```

**Audited, not assumed:** before touching anything, confirmed these are
NOT symptoms of our own Kotlin-version pin. (1) None of the five errors
mention metadata-version incompatibility — a structurally different kind
of error than the geolocation/filesystem cascade. (2) These same five
errors, verified by re-reading the very first build log from before any
of our patch scripts existed, were already present that run too — just
buried inside hundreds of cascading noise-errors from the compiler/stdlib
mismatch. They fail unconditionally, independent of which Kotlin compiler
version compiles them. This is a real bug in `@capacitor/camera` 8.2.1
itself (per `npm view`/GitHub releases, still npm's "latest" as of this
audit — the plugin migrated out of the `capacitor-plugins` monorepo into
its own `ionic-team/capacitor-camera` repo at 8.1.0, with a rewritten
Android capture flow (`IonCameraFlow.kt`, wrapping the new
`ion-android-camera` library) — consistent with a still-settling rewrite
shipping real bugs).

**Root cause of the first four (fixed):** lines 34/37/40/43 are
consecutive — a `@CapacitorPlugin(permissions = [Permission(..., alias =
Companion.CAMERA), ...])` annotation on `CameraPlugin` referencing four
`private const val` fields in its own companion object. That relied on a
K1-compiler leniency: annotation arguments resolve in file/top-level
scope, not "inside the class," so a genuinely class-private member was
never actually reachable there — K1 let it slide, K2 (the default
frontend since Kotlin 2.0, and what this project's pinned 2.1.0 correctly
uses) rejects it correctly. The fix is the obvious one: those four
constants just need to not be `private`.

**Fix shipped:** `scripts/patch-capacitor-camera-visibility.mjs`, chained
onto the same `postinstall` hook right after
`patch-native-kotlin-versions.mjs`. It strips `private ` from exactly
those four named constants in `CameraPlugin.kt` (regex scoped to those
specific names, not a blanket "remove all private modifiers" pass) and is
idempotent. Verified against a fixture reproducing the real annotation +
companion shape: patches correctly, leaves an unrelated `private const
val` untouched, and a full dry run of the actual chained postinstall
command (`patch-native-kotlin-versions.mjs && patch-capacitor-camera-visibility.mjs`)
against a combined fixture works end to end.

**Root cause of the fifth (deliberately NOT auto-fixed):**
`IonCameraFlow.kt:540` passes something typed `Intent?` where `Intent` is
required — almost certainly `ActivityResult.data`, which Android's own
API types as nullable, being used somewhere the plugin's logic assumes
it's already been checked non-null. The *correct* fix depends on the
actual surrounding code (a `!!`, a `checkNotNull`, an early return on
`resultCode != RESULT_OK`, etc.) — getting it wrong on a code path that
handles the user's actual camera captures trades a build-time failure for
a possible runtime crash, which is a worse outcome. Repeated web searches
for the actual file content came up empty (only READMEs and unrelated
issues indexed, not the raw source), so this was **not** blind-patched.

**What "permanent" means here, honestly:** the visibility fix is durable
— it patches the real bug at its root, the same way any upstream PR
would, and doesn't depend on CI quirks the way the earlier Kotlin-version
fix does. The Intent? issue is still open. Two real options, both outside
what's safe to do unsupervised from a CI log: (a) pin `@capacitor/camera`
to `8.0.2` (the last release before the `ion-android-camera` rewrite,
per that plugin's own README migration notice — trades away the last
~8 months of upstream camera fixes/features for a known-mature
implementation), or (b) get the actual `IonCameraFlow.kt` source (e.g.
`aradhya` pasting the relevant lines, or a build environment with real
npm registry access) and write an exact, narrow fix the same way the
visibility bug was handled. Neither was done automatically — (a) needs a
`package.json` version change plus a `bun.lock` regen that `bun install
--frozen-lockfile` will reject without a real bun toolchain (not
available in the environment these fixes are authored in), and (b) needs
information this session didn't have. Flagged to Aradhya rather than
guessed at.

## Camera fix, round 2: full resolution supplied and applied (2026-09-06)

Following the deep-audit entry above, a replacement
`scripts/patch-native-kotlin-versions.mjs` was supplied (via a
`README-FIX.md` write-up) that resolves the remaining `IonCameraFlow.kt`
Intent? error and additionally attacks the CI's root cause directly. Not
taken on faith — re-verified independently before shipping:

- Syntax-checked with `node --check`.
- Ran end-to-end against a reconstructed fixture matching the exact
  shape described (the `RESULT_OK` branch's `var intent = result.data` →
  conditional reassignment → `processResultEditFromGallery(intent)`, plus
  the sibling `else` branch's own `val intent = Intent()`): confirmed the
  `!!` lands only on the first call site and the sibling branch — which
  would otherwise get an "unnecessary non-null assertion" warning — is
  correctly left untouched.
- Ran the CLI android-template (`tar.gz`) patch layer against a
  reconstructed root `build.gradle`: confirmed it inserts a real
  `kotlin-gradle-plugin:2.1.0` classpath in valid position and that a
  second run correctly no-ops.
- Verified full idempotency across all three layers on a second run.

**What changed vs. the previous script:**
- **Layer 1 (new, primary):** patches `CameraPlugin.kt` (drop `private`
  from the 4 companion permission aliases — same fix as before) AND now
  also `IonCameraFlow.kt` (adds an explicit `!!` at the one
  `processResultEditFromGallery(intent)` call site that's actually
  nullable — narrow, scoped via a stateful line scan, not a blanket
  regex). This is what actually gets `capacitor-camera` compiling.
- **Layer 2 (kept):** the existing `resolutionStrategy` force block
  pinning `kotlin-stdlib`/`kotlinx-coroutines-*` down to 1.9.25-readable
  versions for all three plugins — unchanged, still needed.
- **Layer 3 (new):** patches `@capacitor/cli`'s own
  `assets/android-template.tar.gz` (and, belt-and-suspenders, an
  already-generated `android/build.gradle` if one exists yet) so the
  *generated* root project declares `kotlin-gradle-plugin:2.1.0` itself —
  which means APKForge's own back-fill condition ("root build.gradle
  doesn't mention kotlin-gradle-plugin") is no longer true, so its stale
  1.9.25 injection stops happening in the first place. Best-effort:
  needs `tar` on PATH (present on GitHub-Actions-style runners; skips
  cleanly and logs if absent) and is non-fatal if the asset layout ever
  changes upstream.

The now-redundant standalone `scripts/patch-capacitor-camera-visibility.mjs`
(from the previous entry) was deleted — its one job is fully subsumed by
this script's Layer 1 — and `package.json`'s `postinstall` now calls only
`patch-native-kotlin-versions.mjs`.

**Left alone, deliberately:** `README-FIX.md` also flagged
`@capawesome/capacitor-screen-orientation` as an installed-but-unused
dependency (harmless warning, not a failure). Not removed here — that's
a `package.json` dependency change, which needs a `bun.lock` regen this
environment can't safely produce (no network access to the bun/npm
registry). Left for Aradhya to drop if wanted; noted in the reply.

## Peek Guard: false-alarm pass + biometric→PIN fallback (2026-09-09)

Four related changes, all aimed at the "too many false alarms" complaint:

1. **`alertOnNoFace` reverted to off by default** (`usePeekDetection.ts`
   DEFAULTS, mirrored in `ThemeContext.tsx`'s `defaultSettings` and
   `PeekGuard.tsx`'s config fallback). A prior pass (see the
   `DS-UNKNOWN-001`-adjacent history above) had turned this on by default
   reasoning that "owner isn't there at all" is a real breach case. In
   practice — even gated by `noFaceSustainMs` — it fired on completely
   ordinary things (phone face-down on a table, propped up while the owner
   steps away briefly, a hand over the lens) and was the single biggest
   source of "useless" alarms reported. Still a per-user opt-in toggle in
   Peek Guard settings (`PeekConfigDialog.tsx` already exposed this
   correctly; only the default changed).

2. **Biometric ("fingerprint") prompt now gated to actual someone-else-detected
   reasons** (`PeekGuard.tsx`): only shown/auto-fired when
   `reason` is `"stranger" | "spoof" | "multiple"`. For a `"no-face"` lock
   there's no impersonation to rule out, so requiring a fingerprint there
   was pure friction — the existing camera-based "Dismiss (owner verify)"
   button is promoted to the primary action instead in that case.

3. **Biometric-failed → PIN fallback** (`PeekGuard.tsx`): previously,
   `tryBiometric`'s catch block did nothing but keep the lock up — a
   cancelled/failed native biometric left camera-based owner-verify as the
   only recourse. Now offers a PIN pad (only when a PIN is actually set)
   using the exact same hashed credential/storage key
   (`"duo-lock-pin"`) and `verifyPin()` that `AppLockScreen.tsx` already
   uses — not a new/separate PIN.

4. **Owner-recognition tuned looser** to reduce false "stranger" reads of
   the actual owner:
   - `usePeekDetection.ts` `DEFAULTS.matchThreshold`: 0.7 → 0.62 (still
     bounded by `ABSOLUTE_MIN_MATCH_THRESHOLD` = 0.55 in
     `faceRecognition.ts`), mirrored in `ThemeContext.tsx`.
   - `OWNER_BORDERLINE_GRACE_MS`: 3000 → 5000ms (more time for a
     borderline owner match — bad lighting, off angle — to self-correct
     before being reclassified as a stranger).
   - `peekEventLog.ts`'s adaptive false-alarm relief: `FALSE_ALARM_RATE_FLOOR`
     0.3 → 0.2, `MAX_THRESHOLD_RELIEF` 0.06 → 0.1, so repeatedly rating
     locks "False alarm" in the post-lock feedback prompt has a more
     noticeable real effect, sooner.

All four are tuning/default changes, not architecture changes — every
touched value stays user-adjustable in `PeekConfigDialog.tsx` exactly as
before.

## Theme system — presets only half-applied + failing contrast in dark mode (2026-09-11)

Audit prompted by "themes aren't properly utilised, especially dark." Found
two separate, concrete bugs in `src/lib/themeEngine.ts` (the single source
of truth every preset/custom theme goes through):

1. **~25 CSS variables were never derived per-theme.** `deriveTokens()`
   only ever computed the original ~17 "shadcn" tokens (background, card,
   primary, secondary, muted, accent, border, etc). But `src/index.css`'s
   `:root`/`.dark` blocks define ~25 more tokens real screens actually
   render with: `--surface-0..3`, `--bg-canvas`, `--bg-subcanvas`,
   `--bg-overlay-scrim`, all 8 `--sidebar-*`, `--warm`/`--taupe`/`--sand`,
   `--glass-bg`, `--accent-muted`, `--call-stage(-foreground)`,
   `--text-primary/secondary/tertiary`, `--divider`, `--input-hover`. None
   of those were ever written by `applyTokens()`, so they silently stayed
   frozen at index.css's hardcoded default (a violet/blue 230 hue) no
   matter which preset was selected. Picking e.g. "Rose" or "Ocean"
   recolored the base ~17 tokens correctly but the sidebar, every glass
   surface (this app's whole "liquid glass" material system), and the
   in-call stage kept rendering in the default violet-blue — every preset
   was only ever half-applied. Fixed by adding `deriveExtendedTokens()`,
   called from both branches of `deriveTokens()`, which derives all of
   these from the same identity. `--glass-border`/`--glass-highlight`/
   `--glass-dock-*`/the `--shadow-*` trio/status colors (success/warning/
   info/destructive) are deliberately left theme-invariant — see the
   comment above `deriveExtendedTokens` for the reasoning (real-glass
   reflects its surroundings rather than tinting itself; shadows read as
   depth only while neutral; status colors carry fixed meaning).
   `customThemes.ts`'s `ALL_TOKEN_VARS` (used by `clearCustomThemeOverride`)
   updated to match, plus a sync step in `applyCustomTheme` so a custom
   theme's background/foreground/AMOLED overrides propagate into the
   mirroring extended tokens (surface-0/1, glass-bg, call-stage, etc)
   instead of leaving them at the pre-override derived value.

2. **`--primary-foreground` contrast was guessed, never verified.** The old
   logic picked a fixed near-white (dark mode) or a lightness-threshold
   branch (light mode) rather than checking the actual resulting contrast
   ratio. Measured against all 28 presets × 2 modes (WCAG relative-
   luminance formula): **12 of 56 combinations failed the 3:1 minimum**
   for large-scale/UI text — as low as **1.92:1** for "gold" in dark mode
   (`gold`, `sand`, `minimal-dark`, `amber`, `lagoon`, `emerald`, `blush`,
   `sunset`, `ocean`, `mint`, `forest`, `indigo` were the failing themes,
   split across light/dark). Root cause: several presets' primary sits in
   a mid-lightness range where "is the app in dark mode" doesn't
   determine "is this specific primary color light or dark." Fixed with
   `pickForeground()` — tries a soft near-black and near-white candidate,
   keeps whichever contrasts better against the actual primary, then
   escalates that candidate toward its extreme only as far as needed to
   clear a 4.5:1 target (themes that already read fine keep the original
   soft off-black/off-white; only the ones that were failing get pushed
   harder). Re-measured after the fix: 0/56 failures, all ≥4.2:1, most
   ≥5:1. Also added `hslToRgb01`/`relativeLuminance`/`contrastRatio` as
   reusable exports for future contrast-sensitive work.

Also generalized the dark/light blend interpolation in
`deriveDynamicTokens()` to handle the alpha channel now present on several
of the new extended tokens (`H S% L% / A`) — `parseHsl` now captures an
optional alpha group and blends it linearly alongside hue/sat/lightness,
so "dynamic" mode's continuous day/night drift stays seamless across the
new tokens too, not just the original 19.

Verified: standalone `tsc --strict` on the rewritten file (no errors);
compiled + ran the actual `deriveTokens`/`contrastRatio` logic against
all 28 presets × 2 modes to confirm 0 contrast failures and to confirm
the extended tokens (e.g. `--glass-bg`, `--sidebar-background`,
`--call-stage`) now differ correctly by theme instead of being frozen.

## Theme system round 3 — deeper contrast/elevation audit (2026-09-11, same session)

Follow-up to the round 1/2 fix above, in response to "improve them more."
Ran a fuller audit across all 28 presets × 2 modes × every text/background
pair, plus border and elevation-surface distinctiveness. Found and fixed
three more concrete issues in `themeEngine.ts`:

1. `--muted-foreground` (captions, timestamps, placeholder text app-wide)
   used a fixed `{s, l}` regardless of the theme's own `--muted` lightness
   and measured only **3.66:1 against it on all 28 presets** in light mode
   (18/28 also failed in dark) — below the 4.5:1 target for normal text.
   Fixed the same way as `--primary-foreground`: now contrast-verified via
   `pickForeground()` against the theme's actual `--muted`.
2. `--border` vs `--card` measured as low as **1.17–1.23:1** on some hues
   (indigo dark, mint light) — fainter than this app's own pre-existing
   static-CSS baseline for the same pairing (1.23 dark / 1.34 light), i.e.
   some presets rendered hairline separators less visible than the app's
   established default. Added `ensureMinContrast()`, which nudges border
   away from card (in whichever direction it already sat) only as far as
   needed to match that baseline.
3. The surface-2/surface-3 elevation ladder aliased `--muted`/`--border`
   directly, which sit only ~1 lightness-point apart by design at some
   hues — surface-2 vs surface-3 measured **1.00–1.04:1** (visually
   identical; a third "elevation" level that didn't read as one).
   Replaced with an explicit, guaranteed step off `--card` (±5 / ±10
   lightness) instead.

Fixing #1 also surfaced a real bug in `pickForeground()` itself (used by
both `--primary-foreground` and now `--muted-foreground`): it locked in
whichever of {near-black, near-white} contrasted better against the
UN-escalated starting candidates, then only escalated that one direction.
Two hues ("arctic", "olive") have a *lower ceiling* on their
better-looking starting side than on the other (arctic tops out at 4.38
against pure white but reaches 4.80 against pure black) — so the old
one-shot direction choice could lock in the worse-ceiling side and fall
short even at full escalation. Fixed by escalating both directions fully
and keeping whichever wins. Also relaxed the escalation ceiling itself:
the soft starting points (l:12 / l:97) exist to keep this app's "never
stark" language for large surfaces, but that reasoning doesn't apply to
small button/pill text — two hues ("cocoa", "steel") only clear 4.5 at
the true l:0/l:100 extreme, by under a tenth of a point, so escalation is
now allowed to reach it as a last resort while the soft default is
untouched for the 26 presets that don't need it.

Re-measured after all of round 3: **336/336 text-contrast checks
(foreground/card-foreground/secondary-foreground/muted-foreground/
accent-foreground/primary-foreground, all 28 presets × 2 modes) now clear
4.5:1, 0 failures.** Border/card contrast meets the app's own established
baseline on every preset. Surface-2/surface-3 elevation is perceptibly
distinct everywhere (worst case 1.08 light / 1.14 dark, up from
1.00–1.04). Verified via standalone `tsc --strict` plus running the
compiled logic against the full preset matrix.

## "Couldn't save profile" / PGRST303 "JWT issued at future" on fresh writes (2026-09-12)

Symptom: onboarding's final profile save (and potentially any other
Supabase write, right after sign-in or a token refresh) could fail with
the generic `error.message` toast "Couldn't save profile", underlying
error `{ code: "PGRST303", message: "JWT issued at future" }`.

Root-caused as **not** a bug in this app, and not a client clock issue —
it's an ongoing Supabase platform bug in hosted PostgREST: PostgREST
caches its notion of "now" via a libev/`mkAutoUpdate` timer, and the very
first request on a freshly opened connection (which is exactly what
happens right after sign-in/token refresh) can read a stale/unpopulated
cache, making a token minted moments ago look "from the future" to the
validator. Confirmed via a live Supabase GitHub discussion
(supabase#48123, still open) with reports across every region and both
free/pro plans, all with verified-correct client clocks. Upstream tracked
at PostgREST/postgrest#5196 (a related fix, #5159, didn't fully cover
this path). Supabase's own incident (status.supabase.com/incidents/
bv4ntm4x0btf, resolved 2026-08-31) rolled all hosted projects back to a
pinned PostgREST 14.5 specifically to avoid this and a second issue in
newer builds — so there's genuinely no fix available from this app's
side, only mitigation.

Fix applied in `src/integrations/supabase/client.ts`'s `createSupabaseFetch`:
transparently retry (up to 3x, jittered exponential backoff starting
~350ms) any 401 response whose body is `PGRST303`/"issued at future",
then hand back whatever the last attempt returns. Safe to retry blindly
since a PGRST303-rejected request never reached the database. Applies to
every Supabase Data API / Edge Function call the app makes (this is the
one shared `fetch` all of them go through), not just onboarding's
profile save. Verified with a standalone `tsc --noEmit` against stub
modules for `@supabase/supabase-js`/`./types`/`./previewAuthStorage`
(zero errors) since no node_modules were available in this environment
to type-check against the real project.

If this keeps recurring even after the retry (i.e. all `PGRST303_MAX_RETRIES`
attempts are exhausted), it's worth checking status.supabase.com for a
live incident before assuming a new bug — this class of failure has
recurred multiple times on Supabase's side over the past couple months.


## Offline-first launch: login bounce, network-gated keys, tiny cache (2026-09-21)

Symptom: "offline support is zero on APK/IPA; everything reloads every launch."
Full write-up, trade-offs and the not-verified list: `docs/OFFLINE_FIRST.md`.
Short version: (1) expired access token + offline ⇒ supabase-js `INITIAL_SESSION`
is `null` ⇒ `AuthContext` showed the login screen — now the user is seeded from
the persisted session and `null` only means signed-out if nothing is persisted;
(2) `useE2E` gated `ready` (and therefore Chat's first fetch) on two network
round trips — now local keys + cached partner public key apply first and the
server only reconciles; (3) the 60-message Preferences blob was replaced by a
per-message encrypted IndexedDB store with reconcile + gap fill + offline
paging; (4) chat media cached on-device and served as stable blob URLs;
(5) offline sends queue silently and flush in order on reconnect.
Do not let `@capacitor/network` veto requests (false "offline" on some VPN /
private-DNS networks would strand the app). Do not treat "absent from state" as
"deleted" when persisting — a refresh drops older loaded pages.


## Settings screens offline (2026-09-21)

See "Settings pass" in `docs/OFFLINE_FIRST.md`. Key traps: `supabase.auth.signOut()`
returns `{error}` (does not throw) on a network failure and leaves the session;
`supabase.auth.getUser()` is a network call — use `readPersistedSession()` for
"who am I" in offline-tolerant code; a failed profile query must never be read as
"no partner" (`!!undefined === false`). Local wipe happens only after the session
is actually gone. New per-user display cache: `lib/settingsCache.ts` (cleared on
sign-out). New user switches: `lib/offlineSettings.ts` + `OfflineStorageCard`.


## Gallery / Us / Shayari offline (2026-09-21)

See "Gallery, Us, Shayari pass" in `docs/OFFLINE_FIRST.md`. Trap to avoid in any new
screen: never let a failed Supabase read fall through as an empty list / zero /
"not linked" (`const { data } = await q; return data || []` is the bug) — throw,
keep what's on screen, and only replace state on success. Snapshots hold RAW
storage paths, never signed/blob URLs. `collectionStore` = whole-value encrypted
snapshots; `messageStore` = per-message rows.
