...
/** Android only: the OS is not showing the now-playing notification.
 *  `reason` is APP_NOTIFICATIONS_DISABLED | CHANNEL_BLOCKED | CHANNEL_MISSING |
 *  NOT_ACTIVE | "POST_FAILED: <message>". Not a playback error. */
export interface NotificationIssueEvent { reason: string; }
 ...
 * mirrors the YouTube track into MediaPlaybackService's ONE MediaSession +
 * notification (state pushed from GroicContext via updateWebKeepAlive();
 * no ExoPlayer decodes any audio for it). A separate WebKeepAliveService /
 * second MediaSession used to do this and was removed 2026-09-20 — competing
 * sessions in one package made the OS hide the controls. Its only real
...
export interface DuospaceAudioEnginePlugin {
  ...
  /** Android: open the app's notification settings. No-op elsewhere. */
  ...
  addListener(eventName: "notificationIssue", listenerFunc: (event: NotificationIssueEvent) => void): Promise<PluginListenerHandle>;
  // DIAGNOSTIC: fires once the OS confirms it's actually displaying the
  // now-playing notification (see MediaPlaybackService.verifyNotificationVisible
  // on Android). No payload — presence of the event is the signal.
  addListener(eventName: "notificationVisible", listenerFunc: () => void): Promise<PluginListenerHandle>;
