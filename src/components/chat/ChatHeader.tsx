...
import {
  ...
  MoreVertical, Heart, Ghost, Reply, Trash2,
} from "lucide-react";
...
const ChatHeader = ({
  ...
  disappearMode, partnerTyping, partnerOnline, e2eReady,
  ...
  onToggleDisappear, recoverChat, setShowClearDialog,
  anniversaryDate,
}: ChatHeaderProps) => {
      ...
      >
        ...
        <div className="min-w-0 text-left">
          ...
          <h1 className="text-[15px] font-semibold text-foreground leading-tight flex items-center gap-1.5 tracking-tight">
            ...
            {disappearMode && (
              <span title="Vanish Mode is on — when it's turned off, everything that's been seen is deleted for both of you"
                ...
                <Ghost className="h-2.5 w-2.5" /> VANISH
              </span>
            )}
      ...
      <div className="flex items-center gap-0.5">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-48 rounded-xl border-border/50">
            ...
            <DropdownMenuItem onClick={() => { onToggleDisappear(); }}>
              <Ghost className="h-4 w-4 mr-2.5" />
              {disappearMode ? "Turn off Vanish Mode" : "Turn on Vanish Mode"}
            </DropdownMenuItem>
          ...
          <div className="flex items-center gap-2 mt-2 glass-sheet rounded-full px-3 py-1.5">
            ...
            {searchResults.length>0 && (
              <div className="flex items-center gap-1">
                ...
                <button onClick={() => { setSearchIndex(i=>Math.max(0,i-1)); }} aria-label="Previous match" className="h-8 w-8 flex items-center justify-center text-muted-foreground"><ChevronUp className="h-3.5 w-3.5" aria-hidden="true" /></button>
                <button onClick={() => { setSearchIndex(i=>Math.min(searchResults.length-1,i+1)); }} aria-label="Next match" className="h-8 w-8 flex items-center justify-center text-muted-foreground"><ChevronDown className="h-3.5 w-3.5" aria-hidden="true" /></button>
              </div>
            )}
            <button onClick={() => { setSearchOpen(false); setSearchQuery(""); }} aria-label="Close search" className="h-8 w-8 flex items-center justify-center text-muted-foreground"><X className="h-3.5 w-3.5" aria-hidden="true" /></button>
          </div>
        </motion.div>
      )}
    </AnimatePresence>
