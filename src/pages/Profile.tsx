...
const Profile = () => {
  ...
  // Painted from the last known values first (partnerCache.ts is the same
  // record Chat uses; settingsCache holds my own display name) so this screen
  // is correct instantly and offline instead of "Loading…" → "No partner".
  const cachedPartner = useMemo(() => (user ? getCachedPartner(user.id) : null), [user]);
  const cachedMe = useMemo(() => readSettingsCache<{ name: string }>(user?.id, "profile-me"), [user]);
  const [partnerName, setPartnerName] = useState(cachedPartner?.partnerName ?? "");
  const [partnerAvatar, setPartnerAvatar] = useState<string | null>(cachedPartner?.partnerAvatar ?? null);
  const [partnerLinked, setPartnerLinked] = useState<boolean | null>(cachedPartner ? true : null);
  const [partnerLoadFailed, setPartnerLoadFailed] = useState(false);
  const [myName, setMyName] = useState(cachedMe?.name ?? "");
  ...
  useEffect(() => {
    if (!user) return;
    let cancelled = false;
    supabase.from("profiles").select("display_name,partner_id,pet_name").eq("user_id", user.id).single()
      .then(({ data, error }) => {
        if (cancelled) return;
        if (error) {
          // With a remembered pairing, a failed refresh (offline) changes
          // nothing the person can see. Only with NOTHING to show is it a
          // load failure — and even then it must not read as "no partner".
          if (!cachedPartner) { setPartnerLoadFailed(true); setPartnerLinked(null); }
          return;
        }
        if (data?.display_name) { setMyName(data.display_name); writeSettingsCache(user.id, "profile-me", { name: data.display_name }); }
        if (data?.partner_id) {
    ...
    <div className="flex flex-col h-full overflow-y-auto">
      ...
      <div className="flex flex-col items-center px-6 pt-6 pb-8">
        <div className="h-24 w-24 rounded-full bg-muted flex items-center justify-center overflow-hidden">
          {partnerLinked === null && !partnerLoadFailed ? (
            <Loader2 className="h-5 w-5 animate-spin text-muted-foreground" />
          ) : partnerAvatar ? (
            <img loading="lazy" decoding="async" src={partnerAvatar} alt="" className="h-full w-full object-cover" />
          ) : appIcon ? (
            <img loading="lazy" decoding="async" src={appIcon} alt={appName} className="h-full w-full object-cover" />
          ) : (
        ...
        <h1 className="text-lg font-semibold text-foreground mt-4">
          {partnerLinked === null ? (partnerLoadFailed ? appName : "\u00A0") : partnerLinked ? partnerName || "Partner" : "No partner linked yet"}
        </h1>

        <p className="text-[12px] text-muted-foreground mt-1 flex items-center gap-1.5">
          {partnerLoadFailed && partnerLinked === null ? (
