...
const CallOutcomeScreen = ({ outcome, partnerName, onCallAgain, onDismiss, autoDismissMs = AUTO_DISMISS_DEFAULT }: CallOutcomeScreenProps) => {
  ...
  const { title, subtitle, Icon } = (() => {
    switch (outcome.type) {
      case "declined":
        return { title: `${partnerName} declined the call`, subtitle: "Try again a little later, or send a message.", Icon: PhoneMissed };
      case "no-answer":
  ...
  return (
    ...
    >
      ...
      <motion.button onClick={() => { onDismiss(); }} aria-label="Close"
