...
/**
 * Phase 2.5 addition — the two smallest-scale members of the spec's named
 * motion language ("quickSpring"/"snappySpring"). Distinct from
 * gentleSpring above: these are for the tiniest, highest-frequency
 * feedback (a button compressing under a finger, a dock tab's press
 * state) where even gentleSpring's 420/34 reads a hair slow. No
 * overshoot on quickSpring; snappySpring allows a touch of settle motion
 * for a slightly more "alive" feel where that's wanted (e.g. a toggle).
 */
export const quickSpring = { type: "spring" as const, stiffness: 620, damping: 38, mass: 0.6 };
export const snappySpring = { type: "spring" as const, stiffness: 560, damping: 26, mass: 0.7 };

/**
 * AUDIT FIX (Phase 7, cross-cutting design-token pass): found by grepping
 * every hardcoded `type: "spring"` literal in the app for exact numeric
 * duplicates across unrelated files — these two turned up independently
 * reinvented, verbatim, in components that share no code path:
 *
 *   - firmSpring (420/38): BackupManager's restore sheet rising in, and
 *     LipReadingOverlay's caption bar rising in. Different features, same
 *     "an important surface is firmly, confidently rising into place"
 *     physics — a real recurring pattern, not a coincidence worth
 *     preserving as two separate magic-number literals.
 *   - swiftSpring (500/35): BottomNav's active-tab indicator sliding
 *     between tabs, and CallHistoryRow's swipe-to-delete snap-back. Both
 *     are "an element quickly, decisively returning/moving to its resting
 *     position" — distinct from quickSpring above (that one's for a
 *     button compressing under a finger, not a larger element traveling).
 *
 * Deliberately NOT touched: the other dozen-plus hardcoded spring literals
 * elsewhere in the app (PeekGuard, MessageBubble, MessageComposer, Groic
 * players, etc.) — each of those is a distinct, apparently-deliberate
 * value with no cross-file duplicate, and retuning them to fit an existing
 * token would change how they actually feel for no functional reason.
 * Consolidating only genuine duplicates keeps this change zero-risk (the
 * four call sites below render bit-identical to before) rather than a
 * broad re-tune of animations that were never reported as wrong.
 */
export const firmSpring = { type: "spring" as const, stiffness: 420, damping: 38 };
export const swiftSpring = { type: "spring" as const, stiffness: 500, damping: 35 };
