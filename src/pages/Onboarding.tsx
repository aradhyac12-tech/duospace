...
type StepKind = "name" | "gender" | "phone" | "cycle" | "username" | "partner";
...
const Onboarding = ({ onComplete }: OnboardingProps) => {
  ...
  // True only when the server confirmed the two accounts are now linked (as opposed to
  // "scanned, and the link will finish later").
  const [partnerLinkedNow, setPartnerLinkedNow] = useState(false);
  ...
  const handleNext = async (fromSkip = false) => {
    if (kind === "username" && !fromSkip) {
    ...
    // Last step — everything else has already been saved incrementally
    // (name/gender/phone/cycle in the final profile update below, username
    // via saveUsername() above, any partner action by sendPartnerRequest/QR
    // flows). Calls no longer need a per-couple provider key: the
    // self-hosted calling stack is configured server-side. This final write only needs the
  ...
  return (
    <div className="min-h-dvh flex items-center justify-center bg-background px-6">
      ...
      >
                ...
                {partnerActionDone ? (
                  <div className="flex flex-col items-center gap-3 py-4 text-center">
                    ...
                    <p className="text-sm text-muted-foreground max-w-[260px]">
                      {partnerLinkedNow
                        ? "You're linked as partners."
                        : "You're all set here — you'll be linked as partners once they finish creating their account."}
                    </p>
                  </div>
                ) : partnerPanel === "menu" ? (
                  <div className="space-y-2">
                  ...
                  <div className="space-y-3">
                    ...
                    {partnerPanel === "scan" && (
                      <QRSignInScanner
                        ...
                        // The scanner already showed the accurate toast ("Linked ✓" only when the
                        // server confirmed the link).
                        onPartnerLinked={(info) => { setPartnerLinkedNow(!!info?.immediate); setPartnerActionDone(true); }}
                        // Comes back only when the server did NOT see a signed-in scanner — nobody
                        // was linked, so never treat it as done.
                        onSignupInvite={() => {
                          toast({
                    ...
                    {partnerPanel === "display" && (
                      <QRSignInDisplay
                        mode="signup_invite"
                        onClose={() => setPartnerPanel("menu")}
                        onRedeemed={(redeemedKind) => {
                          setPartnerActionDone(true);
                          if (redeemedKind === "partner_linked") {
                            setPartnerLinkedNow(true);
        ...
        {(kind === "phone" || kind === "cycle" || kind === "username" || kind === "partner") && (
          <motion.button
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            whileTap={{ scale: 0.96 }}
