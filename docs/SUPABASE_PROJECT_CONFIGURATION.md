...
| `.env.example` | added this session — documents `VITE_SUPABASE_URL`/`VITE_SUPABASE_PUBLISHABLE_KEY`/`VITE_PUBLIC_SITE_URL`, no values pinned (client.ts's own fallback is the source of truth for the default) |
| `DEPLOY.md` | frontend env, auth callback URL, Edge Function deploy target — **not updated this session, still describes the Lovable deployment** |
| `supabase/config.toml` `[auth]` | `site_url = "https://duospace-ten.vercel.app"` (updated this session — was the Lovable preview URL) |

- **Frontend URL:** `https://duospace-ten.vercel.app` (production, per `supabase/config.toml [auth] site_url`) — the previous Lovable preview URL is kept in `additional_redirect_urls` for continuity, not as the primary site_url anymore.
