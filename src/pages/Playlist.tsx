...
const Playlist = () => {
  ...
  // this was previously a single global "blend-sync" channel shared by
  // every user in the app with an active blend session — any
  // authenticated client could observe, and inject spoofed playback
  // broadcasts into, every other couple's blend session. Scoped to
  // "blend-sync:<uid1>:<uid2>" (same colon-delimited pattern as
  // typing:/presence:/groic:) and marked `private: true` so the new RLS
  // policy on realtime.messages actually applies. Do not change this
  // topic shape without updating that migration to match.
  useEffect(() => {
    if (!user || !blendActive || !partnerId) return;

    const channel = supabase
      .channel(`blend-sync:${[user.id, partnerId].sort().join(":")}`, { config: { private: true } })
  ...
  }, [user, blendActive, partnerId, queue]);
                  ...
                  <div className="h-11 w-11 rounded-xl bg-accent flex items-center justify-center shrink-0 overflow-hidden relative">
                    ...
                    {isCurrent && isPlaying && (
                      <div className="absolute inset-0 bg-primary/30 flex items-center justify-center">
                        {/* PERF: height→scaleY (see GroicMiniPlayer's waveform
                            for why). Each of these bars also lives inside a
                            .map() row in a potentially long song list, so
                            the reflow cost used to multiply by however many
                            "now playing" rows were mounted at once. */}
                        <div className="flex gap-0.5 items-end h-3">
                          <motion.div animate={{ scaleY: [0.3, 1, 0.3] }} transition={{ repeat: Infinity, duration: 0.5 }} className="w-[2px] h-3 origin-bottom bg-primary-foreground rounded-full" />
                          <motion.div animate={{ scaleY: [0.6, 0.3, 1] }} transition={{ repeat: Infinity, duration: 0.6 }} className="w-[2px] h-3 origin-bottom bg-primary-foreground rounded-full" />
                          <motion.div animate={{ scaleY: [1, 0.5, 0.8] }} transition={{ repeat: Infinity, duration: 0.4 }} className="w-[2px] h-3 origin-bottom bg-primary-foreground rounded-full" />
                        </div>
                      </div>
                    )}
                  </div>
