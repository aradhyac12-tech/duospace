...
```bash
supabase functions deploy qr-anon-issue issue-qr-token redeem-qr-token notify-signin send-email set-email-password audius-search music-search music-trending
```
...
### Audius (`audius-search`)

Powers native background/lock-screen music playback — see
`docs/MUSIC_NATIVE_PLAYBACK.md` for the full architecture.

- No secret is required to deploy or run it — Audius's search/trending/
  stream-resolution API is free and needs only a non-secret `app_name`
  string.
- Optional: set `AUDIUS_APP_NAME` (defaults to `"DuoSpace"` if unset) —
  only worth setting explicitly if you want usage attributed to a
  different name in Audius's own dashboards.
- Uses the same `SUPABASE_URL` / `SUPABASE_SERVICE_ROLE_KEY` secrets
  above (for its rate-limit RPC call) and `SUPABASE_ANON_KEY` (for
  verifying the caller's session) — no separate secrets to add if those
  are already set.
- Leave `verify_jwt` at its default (`true`) for this function — it
  requires a logged-in user and does its own `supabase.auth.getUser()`
  check; do not add an entry for it in `supabase/config.toml`.
- Depends on the `consume_rate_limit` Postgres RPC already shipped in
  this project's migrations (bucket `audius-search`, 60 requests/min per
  user) — no extra migration needed if you're on a DB that already has
  the earlier `consume_rate_limit` migrations applied.
...
- Automatically enabled when Peek Guard or Privacy Mode is on (iOS). On Android it is always on: MainActivity sets FLAG_SECURE, which also blocks screenshots and screen recording
