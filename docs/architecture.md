...
src/
  ...
  contexts/          ThemeContext (light/dark/auto + theme engine),
                     CallContext (single global Daily call session +
                     incoming-call ring/accept/decline — see below),
                     GroicContext (shared music session)
docs/               This documentation set
```

## Call session architecture (CallContext)

`CallContext.tsx` wraps every protected route (mounted once in `App.tsx`
around `<AppLayout />`) and is the single owner of the Daily.co call
object (`useDailyCall()`), for the same reason there can only be one
`DailyCall` instance page-wide. As of Phase 6 (2026-08-10) it also owns:

- `IncomingCallOverlay` (ring/vibrate/accept-decline UI) — mounted here,
  not in any individual page, so incoming calls are handled regardless of
  which route is on screen.
- `activeCallId`/`setActiveCallId` — the `call_history` row id for
  whichever call is in progress, shared app-wide so `Chat.tsx`/`Calls.tsx`
  can each still close out the right row in their own `endCall()` even if
  the call was accepted while the other page was mounted.
- `isAcceptingCall` — true during the claim→token→join network window
  right after tapping Accept, before `callState` itself becomes
  `"joining"`.

`Chat.tsx` and `Calls.tsx` still each own their own *outgoing*-call flow
(`isStartingCall`, `startCall()`) as page-local state — only the
*incoming*-call and active-call-id pieces are centralized. See
`docs/PRODUCTION_UI_HARDENING.md` §1.1 for the bug this fixed and the
manual QA checklist to run before trusting it.
...
plus `complete-signup` (previously auto-confirmed + minted a session
immediately after sign-up when no SMTP/email provider was configured —
see `phases.md`. Resend SMTP is now configured in Supabase Auth and no
client code calls this function anymore; it's still deployed but unused).
