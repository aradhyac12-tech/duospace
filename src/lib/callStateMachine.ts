 ...
 *   - the call engine adapter's `callState` ("idle" | "joining" | "joined" | "error")
 ...
 * a token fetch, the call engine join) captures the generation it started with;
 ...
 *
 * This is purely additive: it does not call Supabase, the call engine, or any
 * native bridge. It only tracks *what state we believe we're in* and
 * *whether a given async result still applies*. CallContext/the call engine adapter
...
export type CallLifecycleState =
  ...
  | "CONNECTING"
  | "CONNECTED"
  /** In the LiveKit room (and possibly the partner too) but remote audio
   *  has NOT started playing within the bound. Explicitly NOT "connected":
   *  it either recovers (CONNECTED, late audio) or fails (FAILED). */
  | "MEDIA_DEGRADED"
  | "RECONNECTING"
  | "DECLINED"
  | "CANCELLED"
...
export type CallMachineEvent =
  ...
  | { type: "MEDIA_DEGRADED"; source: string }
...
const TRANSITIONS: Partial<Record<CallLifecycleState, Partial<Record<CallMachineEvent["type"], CallLifecycleState>>>> = {
  ...
  CONNECTING: {
    ...
    DECLINED: "DECLINED", // caller side: joined the room, callee then declined
    MISSED: "MISSED", // caller side: ring timed out while we waited in the room
  },
  MEDIA_DEGRADED: {
    CONNECTED: "CONNECTED", // remote audio started late
    ENDED: "ENDED",
    FAILED: "FAILED", // still no usable audio after the grace period
    CANCELLED: "CANCELLED",
  },
  CONNECTED: {
 ...
 *  was kicked off. Async callbacks (claim result, token result, the call engine
