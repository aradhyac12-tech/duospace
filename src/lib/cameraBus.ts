 ...
 *
 * PRIVACY (Phase 1.6): `acquireCamera` now REQUIRES a registered purpose and
 * enforces src/lib/privacy/sensorPolicy.ts before opening the device — an
 * automatic purpose (Peek Guard, Daily Mood, background mood) is refused
 * unless its persisted opt-in is currently ON, so turning a toggle off
 * stops the next capture without a reload. User-action purposes must be
 * passed `{ userInitiated: true }` by a call site that really is a tap.
 * Every consumer is inventoried in the sensor registry; a static test fails
 * if a file uses the camera without being listed there.
 */

import { assertSensorAllowed } from "@/lib/privacy/sensorGate";
import { BUS_CAMERA_PURPOSES, type BusCameraPurpose } from "@/lib/privacy/sensorPolicy";
...
export async function acquireCamera(
  facing: Facing,
  purpose: BusCameraPurpose,
  opts: { userInitiated?: boolean } = {},
): Promise<CameraLease> {
  // Fail closed BEFORE touching the pool or the device. The runtime check
  // also rejects a purpose smuggled in through a cast that isn't a bus purpose.
  if (!(BUS_CAMERA_PURPOSES as readonly string[]).includes(purpose)) {
    throw Object.assign(new Error(`acquireCamera: "${String(purpose)}" is not a camera-bus purpose`), { name: "SensorDeniedError" });
  }
  assertSensorAllowed(purpose, { userInitiated: opts.userInitiated });
