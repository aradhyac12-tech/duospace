...
const GridMenu = ({ onClose, onScheduledMessage, onLoveLetter }: GridMenuProps) => {
  ...
  // Warm the destination chunk the instant a finger touches the row (same as
  // the dock tabs) so the post-close navigation never waits on a download.
  const frequent = frequentItems.map(i => ({
    icon: i.icon, label: i.label, action: () => { runThenClose(() => navigate(i.path)); },
    warm: () => { routePreload[i.path]?.().catch(() => {}); },
  }));
  const more = [...moreItems.map(i => ({
    icon: i.icon, label: i.label, action: () => runThenClose(() => navigate(i.path)),
    warm: () => { routePreload[i.path]?.().catch(() => {}); },
  })), ...actionItems.map(a => ({ ...a, warm: undefined as undefined | (() => void) }))];
