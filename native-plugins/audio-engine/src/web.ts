...
export class AudioEngineWeb extends WebPlugin implements DuospaceAudioEnginePlugin {
  ...
  // Keep-alive mode is a no-op on web — deliberately, not an oversight.
  // See definitions.ts's doc comment on WebKeepAliveMeta: a browser tab
  // doesn't have the OS-process-suspension risk the installed Capacitor
  // app has, and GroicContext's existing silent-audio MediaSession anchor
  // + navigator.mediaSession wiring already do the real work here. These
  // resolve so GroicContext can call them unconditionally on every
  // platform without an `if (Capacitor.isNativePlatform())` branch.
  async startWebKeepAlive(): Promise<void> { /* no-op on web */ }
  async updateWebKeepAlive(): Promise<void> { /* no-op on web */ }
  async stopWebKeepAlive(): Promise<void> { /* no-op on web */ }
  async openNotificationSettings(): Promise<void> { /* no-op on web */ }
}
