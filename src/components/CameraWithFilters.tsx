...
import { hapticMedium, hapticSelection, hapticCameraShutter, hapticSend } from "@/lib/haptics";
...
const CameraWithFilters = ({ onClose, onCapture }: CameraWithFiltersProps) => {
  // Mounted only while open, same as PhotoViewer — see that component's
  // identical call for why this exists (useDockVisibility.ts).
  useSetImmersive("camera", true);
  ...
  const startCamera = useCallback(async (facing: "user" | "environment") => {
    ...
    try {
      const lease = await acquireCamera(facing, "PHOTO_CAPTURE", { userInitiated: true });
      ...
      <div className="fixed inset-0 z-50 bg-black flex items-center justify-center">
        <div className="text-center space-y-4 px-8">
          ...
          <div className="flex items-center justify-center gap-2">
            <button onClick={() => { startCamera(facingMode); }} className="bg-white/20 text-white px-6 py-2.5 rounded-xl text-sm">Retry</button>
            <button onClick={() => { onClose(); }} className="bg-white/10 text-white px-6 py-2.5 rounded-xl text-sm">Close</button>
          </div>
        </div>
        {permissionSheet}
      </div>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-50 bg-black flex flex-col">
      ...
      <div className="absolute top-0 left-0 right-0 z-20 flex items-center justify-between p-4 safe-top">
        <button onClick={() => { onClose(); }} aria-label="Close camera" className="h-10 w-10 rounded-full bg-black/40 backdrop-blur-sm flex items-center justify-center">
