> **UPDATE 2026-09-20 (later) — quiet background tracking.** Ongoing Android
> tracking no longer depends on a running service. `DuoSpaceLocationService`
> ACTION_START now subscribes Play services to deliver fixes to
> `native/android/LocationUpdateReceiver.kt` through a mutable PendingIntent,
> then stops itself. That receiver forwards to JS when the bridge is alive and
> otherwise uploads natively via `location-push-upload`; it also re-subscribes on
> BOOT_COMPLETED / MY_PACKAGE_REPLACED when a push credential exists (i.e.
> someone is signed in). No foreground service, no notification. Limits:
> requires "Allow all the time"; Android throttles background apps; a force-stop
> clears the subscription until the app is opened; NOT compiled or device-tested.
> Sign-out still calls `stop()` -> ACTION_STOP, which removes the subscription.

> **UPDATE 2026-09-19 — Android no-notification mode.** The sections below
> describe the original design, where `DuoSpaceLocationService` was a real
> foreground service. That service no longer calls `startForeground()`, so it
> shows NO notification (persistent or per-push). Consequence: background
> tracking is best-effort — it continues while the app is open and for as long
> as the OS keeps the process alive after minimizing, then stops until the app
> is reopened; push-triggered one-shots need the background-location permission
> and the service stops itself afterwards. `START_STICKY`, the always-running
> watcher and the "foreground service" wording below no longer apply on Android.
> See the header comment in `native/android/DuoSpaceLocationService.kt` for how
> to restore the old behavior (and its notification).
...
## Addendum 2026-09-20 — what actually happens on a message / call push

- Trigger: `DuoSpaceLocationService.requestFixForPush()` (debounced 15s, ignores
  "typing"), called from both `CallNotificationService` and
  `DuoSpaceMessagingService`; plus JS `requestImmediateFix` when a push reaches
  the WebView.
- App in foreground / process alive with the bridge loaded: fix is written to
  Supabase via LocationContext. Expected to work.
- App backgrounded but process alive: works only if Android background
  location ("Allow all the time") is granted — the app never asks for it (KI-12 #1).
- Process dead: fix obtained, then uploaded natively by DuoSpaceLocationService via the `location-push-upload` edge function using a per-device credential registered while the app was open (KI-12 #2, built 2026-09-20, unverified on device).
- Do not describe this feature as "always works" anywhere until KI-12 is closed.
