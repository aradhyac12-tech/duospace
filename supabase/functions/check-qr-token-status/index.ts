...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: row, error } = await admin
      ...
      // "*" on purpose: claimed_by_user_id only exists after migration
      // 20260920130000_qr_partner_link_fix.sql, and naming a missing column
      // would fail the whole poll on a project that hasn't applied it yet.
      .select("*")
    ...
    return new Response(
      JSON.stringify({
        ...
        kind: row.token_type,
        // anon_signup: a signed-in scanner has been recorded (the link completes
        //   when this device finishes signing up).
        // signup_invite: the scanner was signed in, so the link was made on the
        //   spot (claimed_by_user_id is stamped by redeem-qr-token).
        linked_partner: row.token_type === "anon_signup"
          ? !!row.pending_partner_for
          : row.token_type === "signup_invite"
            ? !!(row as { claimed_by_user_id?: string | null }).claimed_by_user_id
            : undefined,
      }),
