...
const IOS_KEYS = [
  ...
  // FIX: these previously named "your partner" explicitly (system-dialog
  // text shown once, not a notification, but same underlying ask as the
  // Android foreground-service notification fix in DuoSpaceLocationService.kt)
  // — reworded to describe what the location is used for (the in-app map)
  // without naming the partner relationship. Still accurate/truthful for
  // App Store review purposes.
  ["NSLocationWhenInUseUsageDescription", "DuoSpace uses your location to show it on the in-app map."],
  ["NSLocationAlwaysAndWhenInUseUsageDescription", "DuoSpace uses your live location for the in-app map, including when the app is in the background."],
  ["NSLocationAlwaysUsageDescription", "DuoSpace uses your live location for the in-app map, including when the app is in the background."],
];
...
const ANDROID_PERMISSIONS = [
  ...
  // Every call goes through the call engine's WebRTC engine, which — like any
  ...
  // /important + /urgent alerts (MessageAlertService): declaring this is what
  // lists DuoSpace under Settings → Do Not Disturb → App access, where the
  // person can let alerts lift Do Not Disturb (incl. "Total silence") for the
  // length of the alert. It is a special access grant, never auto-granted, and
  // nothing changes unless they switch it on there.
  "android.permission.ACCESS_NOTIFICATION_POLICY",
  "android.permission.FOREGROUND_SERVICE",
  "android.permission.FOREGROUND_SERVICE_PHONE_CALL", // Android 14+ foreground service type grant
  // Android 14+ per-type grant for MediaPlaybackService (foregroundServiceType=
  // "mediaPlayback" — see native-plugins/audio-engine). Declared in that plugin's
  // own library manifest too, but belt-and-suspenders here: Gradle manifest merging
  // from library modules is not guaranteed when cap sync skips a plugin's Gradle
  // build. Without this, startForeground() throws ForegroundServiceStartNotAllowedException
  // on Android 14+ when the app tries to show music controls in the background.
  ...
  // Android 12+ requires BLUETOOTH_CONNECT for startBluetoothSco() and
  // getBluetoothHeadset() — audio routing to Bluetooth headsets silently fails
  // without this. Declared in audio-route's library manifest too (same
  // belt-and-suspenders reasoning as FOREGROUND_SERVICE_MEDIA_PLAYBACK above).
  ...
  // Android 13+ granular audio access — belt-and-suspenders for a media-playing
  // app. Not strictly required for streaming-only playback (ExoPlayer fetches
  // via HTTP, not the MediaStore), but Android's media framework uses the presence
  // of this permission to classify the app as a "media" app for battery-optimization
  // and routing-priority purposes on some OEM builds.
  ...
  // Re-subscribes the quiet background location updates after a reboot — see
  // native/android/LocationUpdateReceiver.kt. No notification, no service.
...
const NATIVE_KOTLIN_FILES = [
  "NotificationChannels.kt", "CallNotificationService.kt", "CallRingingService.kt", "IncomingCallNotification.kt", "CallActionReceiver.kt",
  ...
  // /important + /urgent message alerts: the foreground service that rings
  // (alarm stream) and vibrates through silent mode / Do Not Disturb. Started
  // by DuoSpaceMessagingService.kt for pushes carrying data.alertLevel and
  // referenced by NotificationChannels.kt (vibration patterns).
  "MessageAlertService.kt",
  ...
  // Caller-side "Calling…"/in-progress notification + proximity-sensor
  // screen-off — see native/android/CallOngoingService.kt's own header for
  // the gap this closes (outgoing calls previously registered with neither
  // Telecom nor any system notification).
  "CallOngoingService.kt",
  ...
  // Quiet (no-notification) background location: PendingIntent receiver +
  // boot/app-update re-subscribe. Used by DuoSpaceLocationService.kt above.
  "LocationUpdateReceiver.kt",
  // On-device crash capture — see native/android/CrashLogger.kt's header.
  // Independent of every other file in this list (no cross-references),
  // installed first thing in MainActivity.onCreate below.
  "CrashLogger.kt",
];
...
function patchIosPlist() {
  ...
  // (the call engine call audio) — no additional mode needed for that feature,
...
function patchAndroidManifest() {
  ...
  if (!manifest.includes(`.CallNotificationService"`)) {
    const servicesBlock =
      ...
      `        <!-- android:priority: higher = first; CallNotificationService gets\n` +
      `             priority 2 but handles only incoming-call types and delegates\n` +
      `             everything else to DuoSpaceMessagingService.handlePush() — see\n` +
      `             FCM routing fix in KI-08 and CallNotificationService.kt. -->\n` +
      ...
      `    </service>\n` +
      `    <!-- Caller-side "Calling…"/in-progress notification + proximity-sensor\n` +
      `         screen-off — see native/android/CallOngoingService.kt. -->\n` +
      `    <service\n` +
      `        android:name=".CallOngoingService"\n` +
      `        android:exported="false"\n` +
      `        android:foregroundServiceType="phoneCall" />\n` +
      `    <meta-data\n` +
  ...
  if (!manifest.includes(`.DuoSpaceMessagingService"`)) {
    const messagingServiceBlock =
      ...
      `        android:exported="false">\n` +
      `        <!-- Priority 1 (below CallNotificationService's 2): in practice,\n` +
      `             FCM routing goes through CallNotificationService first, which\n` +
      `             delegates non-call types here via handlePush(). This priority\n` +
      `             ensures DuoSpaceMessagingService still catches messages on\n` +
      `             devices where Android resolves to this service instead. -->\n` +
  ...
  // MessageAlertService: /important + /urgent message ringer. Own guard for the
  // same reason as DuoSpaceMessagingService above. foregroundServiceType
  // mediaPlayback (permission FOREGROUND_SERVICE_MEDIA_PLAYBACK is already in
  // the list above); exported=false — only this app starts/stops it.
  if (!manifest.includes(`.MessageAlertService"`)) {
    const alertServiceBlock =
      `    <service\n` +
      `        android:name=".MessageAlertService"\n` +
      `        android:exported="false"\n` +
      `        android:foregroundServiceType="mediaPlayback" />\n`;
    const appCloseIdx = manifest.lastIndexOf("</application>");
    if (appCloseIdx === -1) {
      console.warn("[patch-native-permissions] Could not find </application> in AndroidManifest.xml — add MessageAlertService manually.");
    } else {
      manifest = manifest.slice(0, appCloseIdx) + alertServiceBlock + manifest.slice(appCloseIdx);
      changed = true;
      console.log("[patch-native-permissions] Android: registered MessageAlertService.");
    }
  }

  // CallActionReceiver: instant Decline from the incoming-call notification
  // (no app launch). exported=false — only our own PendingIntent reaches it.
  if (!manifest.includes(`.CallActionReceiver"`)) {
    const block = `    <receiver\n        android:name=".CallActionReceiver"\n        android:exported="false" />\n`;
    const idx = manifest.lastIndexOf("</application>");
    if (idx === -1) {
      console.warn("[patch-native-permissions] Could not find </application> — add CallActionReceiver manually.");
    } else {
      manifest = manifest.slice(0, idx) + block + manifest.slice(idx);
      changed = true;
      console.log("[patch-native-permissions] Android: registered CallActionReceiver.");
    }
  }

  // LocationUpdateReceiver: quiet background location (no foreground service,
  // no notification) — see native/android/LocationUpdateReceiver.kt. Own guard
  // so an android/ directory generated earlier still gets it on the next sync.
  // exported=false is correct: Play services delivers via our own explicit
  // PendingIntent, and the system delivers BOOT_COMPLETED to non-exported
  // receivers of the same app.
  if (!manifest.includes(`.LocationUpdateReceiver"`)) {
    const locationReceiverBlock =
      `    <receiver\n` +
    ...
    const appCloseIdx = manifest.lastIndexOf("</application>");
    if (appCloseIdx === -1) {
      console.warn("[patch-native-permissions] Could not find </application> in AndroidManifest.xml — add LocationUpdateReceiver manually.");
    } else {
      manifest = manifest.slice(0, appCloseIdx) + locationReceiverBlock + manifest.slice(appCloseIdx);
      changed = true;
      console.log("[patch-native-permissions] Android: registered LocationUpdateReceiver.");
    }
  }

  // CallOngoingService: same separate-guard reasoning as
  // DuoSpaceMessagingService above — an android/ directory generated before
  // this service existed (it's added inside the CallNotificationService
  // block above for fresh installs) still needs it added on the next
  // `cap sync` without that outer guard short-circuiting.
  if (!manifest.includes(`.CallOngoingService"`)) {
    const ongoingCallServiceBlock =
      `    <!-- Caller-side "Calling…"/in-progress notification + proximity-\n` +
      `         sensor screen-off — see native/android/CallOngoingService.kt. -->\n` +
      `    <service\n` +
      `        android:name=".CallOngoingService"\n` +
      `        android:exported="false"\n` +
      `        android:foregroundServiceType="phoneCall" />\n`;
    const appCloseIdx = manifest.lastIndexOf("</application>");
    if (appCloseIdx === -1) {
      console.warn("[patch-native-permissions] Could not find </application> in AndroidManifest.xml — add CallOngoingService manually.");
    } else {
      manifest = manifest.slice(0, appCloseIdx) + ongoingCallServiceBlock + manifest.slice(appCloseIdx);
      changed = true;
      console.log("[patch-native-permissions] Android: registered CallOngoingService.");
    }
  }
...
// LOCATION ON MESSAGE PUSH (iOS): inserted as a separate method outside
// didFinishLaunchingWithOptions rather than inside it (a separate function
// at class scope) — Capacitor's AppDelegate template is a class, so this
// is added as an extension by the patch below.
const APP_DELEGATE_SWIFT_BACKGROUND_FETCH = `
// MARK: - Location fix on APNs background push (message pushes while backgrounded)
// Extends the Capacitor AppDelegate to hook application(_:didReceiveRemoteNotification:
// fetchCompletionHandler:), which iOS calls for every APNs background push when
// "remote-notification" is in UIBackgroundModes (already declared). Used to
// request a fresh location fix for message pushes while the app is backgrounded —
// the same trigger Android's DuoSpaceMessagingService fires, just via the iOS
// APNs delivery path. CallKit/VoIP pushes arrive via PushKit and are NOT included.
// The JS layer (usePushNotifications.ts) covers the foregrounded case; this covers
// the backgrounded case that was previously missing on iOS.
extension AppDelegate {
    func application(
        ...
        // Skip call-lifecycle types: VoIP pushes arrive via PushKit (not here),
        // and call-status updates (missed_call/call_ended/call_rejected) don't
        // benefit from a location fix. Skip "typing" — ephemeral, too frequent.
        let isCallOrTyping = type.hasPrefix("incoming_") || type == "typing" ||
            type == "missed_call" || type == "call_ended" || type == "call_rejected"
        if !isCallOrTyping && !type.isEmpty {
            // Request a fresh location fix. DuoSpaceLocationManager.shared is safe
            // to call here — it has no dependency on a Capacitor Bridge or WebView.
            DuoSpaceLocationManager.shared.requestImmediateFix(reason: "push:\(type)") { _ in
                // Location result is handled by LocationFixBridge onFix/onError;
                // the completion handler here only signals to iOS whether new
                // data was fetched. Always call with .newData so the OS knows
                // we did meaningful work (prevents it from throttling future
                // background-fetch opportunities for this app).
...
function patchIosAppDelegate() {
  ...
  // LOCATION ON MESSAGE PUSH (iOS): append the background-fetch extension
  // to the same file. It is a separate `extension AppDelegate { ... }` block
  // rather than a method inserted inside the class body, so insertion point
  // doesn't matter — it just needs to be in the same compilation unit.
  if (!source.includes("didReceiveRemoteNotification userInfo")) {
    source = source + "\n" + APP_DELEGATE_SWIFT_BACKGROUND_FETCH;
    console.log("[patch-native-permissions] AppDelegate.swift: added location-on-message-push background fetch extension.");
  }
...
const MAIN_ACTIVITY_KOTLIN_ADDITIONS = `
    ...
    override fun onCreate(savedInstanceState: android.os.Bundle?) {
        ...
        // see CrashLogger.kt's header.
        com.duospace.app.CrashLogger.install(this)
        ...
        com.duospace.app.CrashLogger.showLastCrashIfAny(this)
        ...
        // Lets Telecom-originated mute/end/decline reach a running WebView
        // WITHOUT launching/foregrounding this activity (see CallBridge.kt).
        com.duospace.app.CallBridge.liveSink = { json ->
            bridge?.webView?.post {
                bridge?.webView?.evaluateJavascript(
                    "(function(){try{window.dispatchEvent(new CustomEvent('duospace-call-action', { detail: \${json} }));}catch(e){}})();",
                    null,
                )
            }
        }
    ...
    override fun onResume() {
        ...
        // DOUBLE-NOTIFICATION FIX: tell CallNotificationService the app is
        // now visible so it knows to skip the OS incoming-call notification
        // (in-app IncomingCallOverlay handles it). Set here rather than in
        // onStart() — onResume() is the earliest point where the Activity is
        // both fully foregrounded AND guaranteed to have a window visible to
        // the user (onStart fires during transitions where the window may not
        // be interactive yet, e.g. mid-animation into the foreground).
    ...
    override fun onPause() {
        ...
        // DOUBLE-NOTIFICATION FIX: app is no longer in the foreground —
        // CallNotificationService should show the full OS notification for
        // the next incoming call (in-app overlay is not visible).
    ...
    override fun onDestroy() {
        ...
        com.duospace.app.CallBridge.liveSink = null
        super.onDestroy()
    }
    ...
    private fun handleDuospaceCallIntent(intent: android.content.Intent?) {
        ...
        // Only "answer / decline / open this ringing call" intents belong here.
        // mute/unmute/end are live controls for a call in progress; they must
        // not stop the ringer or be persisted for replay on the next boot.
        val isRingingAction = action == null || action == "accept" || action == "decline" || action == "open"
        if (!isRingingAction) {
            val live = org.json.JSONObject().apply {
                put("callId", callId); put("action", action)
            }
            bridge?.webView?.post {
                bridge?.webView?.evaluateJavascript(
                    "(function(){try{window.dispatchEvent(new CustomEvent('duospace-call-action', { detail: \${live} }));}catch(e){}})();",
                    null,
                )
            }
            return
        ...
        // Accept/Decline tapped on the ringing notification: it was posted
        // ongoing + non-auto-cancel, so without this it stayed in the shade
        // (still looking like a live incoming call) until its own 45s timeout.
        // A plain "open" (tap / full-screen launch) is left alone — the in-app
        // overlay reports the outcome itself (see nativeIncomingCall.ts).
        if (action == "accept" || action == "decline") {
            try {
                val nm = getSystemService(android.content.Context.NOTIFICATION_SERVICE) as android.app.NotificationManager
                nm.cancel(com.duospace.app.CallNotificationService.NOTIFICATION_ID)
            } catch (_: Exception) {
            }
        }
        ...
        // COLD-START LOST-ACTION FIX: this evaluateJavascript call used to
        // ONLY dispatch a live 'duospace-call-action' CustomEvent — which
        // requires a JS listener to already be registered
        // (usePushNotifications.ts, only wired up after React mounts AND
        // useAuth resolves). A cold-start Accept tap on the full-screen
        // incoming-call notification fires this before that listener
        // exists, so the event was silently dropped: the app opened, but
        // nothing ever actually accepted the call — it just sat on
        // "Connecting…"/ringing until the 30s auto-decline or the
        // accept-side watchdogs gave up. localStorage.setItem is available
        // the instant the WebView has a document (well before React
        // mounts), so writing the SAME payload there first — durable,
        // single-slot, since only one call can ever be ringing at once —
        // lets nativeCallActionBridge.ts (src/lib/nativeCallActionBridge.ts)
        // drain it on boot even if this live dispatch had nobody listening.
        // The live dispatch is left in place unchanged for the warm-app case
        // (listener already registered), where it's still the faster path.
        bridge?.webView?.post {
            bridge?.webView?.evaluateJavascript(
                "(function(){try{var d=\${payload};d.ts=Date.now();try{localStorage.setItem('duospace_pending_call_action', JSON.stringify(d));}catch(e){}; window.dispatchEvent(new CustomEvent('duospace-call-action', { detail: d }));}catch(e){}})();",
                null,
            )
        }
    }
...
/**
 * Block screenshots and screen recording in the Android app (FLAG_SECURE).
 *
 * Requested: "no screenshot or screen recording should be available on
 * apk". FLAG_SECURE on the Activity's window is the platform mechanism for
 * that: the system refuses screenshots (the capture comes out black or is
 * refused with "can't take screenshot due to security policy"), screen
 * recorders and casting/mirroring capture a black frame for this window,
 * and the Recents thumbnail is blanked.
 *
 * Done natively, in MainActivity.onCreate right after super.onCreate(), so
 * it is active from the very first frame — including the splash, sign-in and
 * app-lock screens, before any JavaScript has loaded — and cannot be turned
 * off by a JS setting. (PeekGuard.tsx additionally re-asserts it via the
 * privacy-screen plugin and no longer calls disable() on Android.)
 *
 * Idempotent and independent of patchMainActivity()'s marker on purpose: a
 * MainActivity that was already patched by an older version of this script
 * (marker present, so patchMainActivity() early-returns) still gets this
 * added on the next run. Handles both Capacitor's Java template and the .kt
 * variant. Must run AFTER patchMainActivity(), which is what creates the
 * onCreate override in a freshly generated project.
 */
const SECURE_WINDOW_MARKER = "DUOSPACE FLAG_SECURE";

function patchSecureWindow() {
  const targets = [
    { path: MAIN_ACTIVITY_JAVA, lang: "java" },
    { path: MAIN_ACTIVITY_KT, lang: "kt" },
  ].filter((t) => existsSync(t.path));

  if (targets.length === 0) {
    console.log("[patch-native-permissions] Skipping FLAG_SECURE patch — no MainActivity found (run `npx cap add android` first).");
    return;
  }

  for (const { path, lang } of targets) {
    let source = readFileSync(path, "utf8");
    if (source.includes(SECURE_WINDOW_MARKER)) {
      console.log(`[patch-native-permissions] MainActivity.${lang} already has FLAG_SECURE (screenshots/screen recording blocked).`);
      continue;
    }
    const superCall = source.match(/super\.onCreate\(\s*savedInstanceState\s*\)[ \t]*;?/);
    if (!superCall) {
      // Loud on purpose: silently shipping an APK that still allows
      // screenshots is exactly the failure this step exists to prevent.
      console.error(
        `[patch-native-permissions] ERROR: could not find super.onCreate(savedInstanceState) in MainActivity.${lang} — ` +
        "FLAG_SECURE was NOT added, so this APK would still allow screenshots and screen recording. " +
        "Add getWindow().setFlags(WindowManager.LayoutParams.FLAG_SECURE, WindowManager.LayoutParams.FLAG_SECURE) after super.onCreate() by hand.",
      );
      process.exitCode = 1;
      continue;
    }
    const insertAt = superCall.index + superCall[0].length;
    const snippet = lang === "java"
      ? `\n        // === ${SECURE_WINDOW_MARKER}: block screenshots + screen recording + recents thumbnail ===\n` +
        `        getWindow().setFlags(\n` +
    ...
    console.log(`[patch-native-permissions] MainActivity.${lang}: added FLAG_SECURE (screenshots/screen recording blocked).`);
  }
}
