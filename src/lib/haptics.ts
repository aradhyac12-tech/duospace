...
//
// FIX ("keeps vibrating, even after answering / after the call ended"):
// this used to overwrite `callVibrationInterval` on every call, with no
// "already running?" check. IncomingCallOverlay re-ran its ring logic
// several times per call (see that file's own notes), so every extra
// startCallVibration() left an orphaned setInterval behind — only the LAST
// handle was ever stored, so stopCallVibration() could only ever clear one
// of them and the rest kept firing native haptics every 1.5s forever. The
// pulse()'s two follow-up setTimeouts were also never tracked, so they could
// still fire after a stop.
//
// Now: (1) start is idempotent — a second call while already vibrating is a
// no-op, (2) every pending follow-up pulse is tracked and cancelled on stop,
// (3) a hard safety cap stops the loop on its own so a missed stop signal can
// never turn into unbounded vibration.
const CALL_VIBRATION_PULSE_MS = 1500;
// Longer than the 30s in-app auto-decline / 45s native ring window, so it
// only ever fires if every real stop path somehow failed.
const CALL_VIBRATION_MAX_MS = 60_000;

let callVibrationInterval: ReturnType<typeof setInterval> | null = null;
let callVibrationSafetyTimer: ReturnType<typeof setTimeout> | null = null;
const callVibrationTimeouts = new Set<ReturnType<typeof setTimeout>>();

export const isCallVibrating = () => callVibrationInterval !== null;

export const startCallVibration = () => {
  if (!isNative() && !canVibrate()) return;
  if (callVibrationInterval) return; // already vibrating — never stack a second loop

  const later = (fn: () => unknown, ms: number) => {
    const id = setTimeout(() => {
      callVibrationTimeouts.delete(id);
      if (callVibrationInterval) void fn(); // stopped in the meantime → skip
    }, ms);
    callVibrationTimeouts.add(id);
  };
  const pulse = () => {
    if (!callVibrationInterval) return;
    void hapticHeavy();
    later(hapticMedium, 200);
    later(hapticHeavy, 400);
  };

  callVibrationInterval = setInterval(pulse, CALL_VIBRATION_PULSE_MS);
  callVibrationSafetyTimer = setTimeout(() => stopCallVibration(), CALL_VIBRATION_MAX_MS);
  pulse();
};
...
export const stopCallVibration = () => {
  ...
  if (callVibrationSafetyTimer) {
    clearTimeout(callVibrationSafetyTimer);
    callVibrationSafetyTimer = null;
  }
  callVibrationTimeouts.forEach((id) => clearTimeout(id));
  callVibrationTimeouts.clear();
  // Web: cancel any vibration pattern already handed to the browser.
  if (!isNative() && canVibrate()) {
