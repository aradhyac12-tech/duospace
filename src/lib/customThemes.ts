...
const ALL_TOKEN_VARS = [
  ...
  // Extended tokens (see themeEngine.ts's deriveExtendedTokens) — a custom
  // theme derives and applies these too now, so clearing the override has
  // to remove all of them, not just the original ~19, or a leftover custom
  // sidebar/glass/surface tint would survive switching back to a preset.
  "--surface-0", "--surface-1", "--surface-2", "--surface-3",
  "--bg-canvas", "--bg-subcanvas", "--bg-overlay-scrim",
  "--sidebar-background", "--sidebar-foreground", "--sidebar-primary",
  "--sidebar-primary-foreground", "--sidebar-accent", "--sidebar-accent-foreground",
  "--sidebar-border", "--sidebar-ring",
  "--warm", "--taupe", "--sand",
  "--text-primary", "--text-secondary", "--text-tertiary", "--divider",
  "--overlay", "--accent-muted", "--glass-bg",
  "--call-stage", "--call-stage-foreground", "--input-hover",
];
...
export const applyCustomTheme = (t: CustomTheme, mode?: ColorMode) => {
  ...
  // Keep the extended tokens that are supposed to mirror background/card
  // (see themeEngine.ts's deriveExtendedTokens) in sync with whichever
  // override just won above — otherwise e.g. an AMOLED theme's --background
  // goes true black but --glass-bg/--surface-1/--call-stage etc still carry
  // the pre-override derived tint, since deriveTokens already computed them
  // before this override ran.
  if (t.background || t.foreground || t.amoled) {
    tokens["--surface-0"] = tokens["--background"];
    tokens["--bg-canvas"] = tokens["--background"];
    tokens["--warm"] = tokens["--background"];
    if (resolvedMode === "dark" || t.amoled) {
      tokens["--sidebar-background"] = tokens["--background"];
      tokens["--call-stage"] = tokens["--background"];
    }
    if (tokens["--card"]) {
      tokens["--surface-1"] = tokens["--card"];
      const prevGlass = tokens["--glass-bg"] || "";
      const alphaMatch = prevGlass.match(/\/\s*([\d.]+)\s*$/);
      const alpha = alphaMatch ? alphaMatch[1] : (resolvedMode === "dark" || t.amoled ? "0.6" : "0.78");
      tokens["--glass-bg"] = `${tokens["--card"]} / ${alpha}`;
    }
    if (tokens["--foreground"]) {
      tokens["--text-primary"] = tokens["--foreground"];
