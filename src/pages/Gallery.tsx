...
const MediaThumbnail = ({ item, onClick }: { item: GalleryItem; onClick: () => void }) => {
  if (item.file_type === "video") {
    return (
      <div className="w-full h-full relative cursor-pointer active:scale-95 transition-transform duration-100" onClick={() => { hapticLight(); onClick(); }}>
  ...
  return <img src={item.file_url} alt="" loading="lazy" className="w-full h-full object-cover cursor-pointer active:scale-95 transition-transform duration-100" onClick={() => { hapticLight(); onClick(); }} />;
};
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.15 }} className="flex-1 min-h-0 overflow-y-auto overscroll-contain pb-24" style={{ WebkitOverflowScrolling: "touch" as any }}>
      <PageHeader title="Gallery" subtitle="Our moments">
        <div className="flex gap-2 items-center">
          ...
          >
            {uploading
              ? <span className="text-[10px] font-bold text-accent-foreground" aria-hidden="true">{uploadProgress}%</span>
      ...
      <AlertDialog open={!!showDeleteDialog} onOpenChange={() => setShowDeleteDialog(null)}>
        <AlertDialogContent className="rounded-2xl">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className="rounded-xl">Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => { hapticError(); showDeleteDialog && deleteItem(showDeleteDialog); }}
