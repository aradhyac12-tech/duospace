...
interface SurpriseMessageProps {
  ...
  onOpen: () => void;
  /** See MessageBubble's identical prop — same virtualization re-mount
   *  problem, same fix: skip once this row has already played its
   *  intro once, so scrolling it in and out of a virtualized long
   *  conversation's render window doesn't replay the pop-in. */
  ...
  return (
    <div className={`flex ${isMine ? "justify-end" : "justify-start"} items-end gap-1.5 px-3 py-1`}>
      ...
      <motion.button
        ...
        initial={(prefersReducedMotion || skipEnterAnimation) ? false : { opacity: 0, y: 6, scale: 0.97 }}
