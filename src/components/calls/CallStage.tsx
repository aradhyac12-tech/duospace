...
const CallStage = ({
  ...
  const {
    callState, isVideoOn, isScreenSharing, isAudioOn,
    toggleVideo, toggleAudio, toggleScreenShare, listCameras, cycleCamera, facingMode,
    ...
    isAcceptingCall, cancelAcceptingCall, activeCallType, setIsCallMinimized, remoteAudioReady,
  } = useCall();
  ...
  // at a time — see reattachRemoteVideo's doc comment in the call engine adapter.
  ...
  return (
    ...
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.22, ease: [0.22, 1, 0.36, 1] }}
      ...
      {/* Minimal cinematic voice-call layout — partner avatar, name,
          ...
          (the call engine still needs it for the audio track) but is hidden via
      ...
      <AnimatePresence initial={false}>
        {isVoiceCall && (callUiState === "connected" || callUiState === "reconnecting") && (
          <motion.div key="voice-connected" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={standardTransition}
            ...
            <span className="relative h-52 w-52 max-w-[58vw] max-h-[58vw] rounded-full overflow-hidden bg-call-stage-foreground/10 flex items-center justify-center ring-4 ring-call-stage-foreground/10">
              {partnerAvatar
            ...
            {/* Name + duration visible on-screen, not just sr-only */}
            <div className="text-center space-y-1">
              <p className="text-xl font-semibold text-call-stage-foreground tracking-tight">{partnerName || "Partner"}</p>
              <p className="text-sm text-call-stage-foreground/55" role="status" aria-live="polite">
              ...
              <div className="relative h-20 w-20 mx-auto mb-4">
                ...
                <div className="relative h-20 w-20 rounded-full bg-call-stage-foreground/10 flex items-center justify-center overflow-hidden">
                  {partnerAvatar
                    ? <img loading="lazy" decoding="async" src={partnerAvatar} alt="" className="h-full w-full object-cover" />
                    : isVoiceCall
                      ? <Phone className="h-8 w-8 text-call-stage-foreground/80" />
                      : <VideoIcon className="h-8 w-8 text-call-stage-foreground/80" />}
                </div>
              </div>
      ...
      {!isVoiceCall && (
        <motion.div ref={previewRef} drag dragConstraints={dragBoundsRef} dragElastic={0.06} dragMomentum={false}
          ...
          {/* MIRROR FIX: the front camera used to render exactly as the
              ...
              live track (see attachTrack/cycleCamera in the call engine adapter), so
      ...
      <motion.div
        ...
        <div className="flex-1 text-center min-w-0">
          ...
          <p className="flex items-center justify-center gap-1 text-xs text-call-stage-foreground/55">
            {!isAudioOn && (
              <MicOff className="h-3 w-3 text-destructive/80 shrink-0" aria-hidden="true" />
            )}
      ...
      <motion.div
        ...
        <div className="glass-dock rounded-full flex items-center justify-between gap-1 px-3 py-2 max-w-[480px] mx-auto">
          {/* MUTE BUTTON — WhatsApp puts mute first. Was entirely absent:
              the control bar had Speaker, Video, End but no way to mute
              the microphone at all. Fixed: mic off = filled circle (active
              mute), mic on = outline circle (pass-through state). */}
          <button onClick={() => { hapticMedium(); toggleAudio(); wake(); }}
            aria-label={isAudioOn ? "Mute microphone" : "Unmute microphone"}
            aria-pressed={!isAudioOn}
            className={`h-14 w-14 shrink-0 rounded-full flex items-center justify-center transition-colors ${!isAudioOn ? "bg-destructive/80" : ""}`}>
            {isAudioOn
              ? <Mic className="h-5 w-5 text-call-stage-foreground" aria-hidden="true" />
              : <MicOff className="h-5 w-5 text-call-stage-foreground" aria-hidden="true" />}
          </button>
