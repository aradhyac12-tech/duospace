...
export function deriveCallUiState(input: CallUiStateInput): CallUiState {
  const { callState, isStartingCall, participantCount, everConnected, networkQuality, remoteAudioReady } = input;
  ...
  if (participantCount > 1) {
    if (remoteAudioReady === false) return "connecting"; // stays true once reached, until the call is left
