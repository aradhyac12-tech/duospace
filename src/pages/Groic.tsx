...
const Groic = () => {
  ...
  const search = useCallback(async (raw: string, opts?: { remember?: boolean }) => {
    ...
    setError(null);
    try {
      const data = await invokeEdgeFunction<{ results?: SearchResult[]; error?: string }>(
  ...
  useEffect(() => {
    search(pickDiscoveryQuery(), { remember: false });
          ...
          {loading && results.length === 0 ? (
            ...
            <div role="alert" className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <Button size="sm" variant="outline" className="mt-4 rounded-full"
                onClick={() => search(lastQuery || query || pickDiscoveryQuery())}>
                Try again
              </Button>
            </div>
          ) : results.length === 0 ? (
