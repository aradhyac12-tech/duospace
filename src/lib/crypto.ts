...
async function deriveKeyUncached(myPrivateKeyJwk: JsonWebKey, peerPublicKeyB64: string): Promise<CryptoKey> {
...
// PERF: this used to run the full ECDH pipeline (two importKey calls + deriveKey)
// for EVERY message — opening a conversation decrypts a whole page of messages in
// parallel, so that was N identical key derivations, on a phone, before the first
// bubble could render. The result depends only on (my private key, peer public
// key), so it is derived once and reused. Properties that keep this safe:
//  - the derived AES key is non-extractable (extractable=false above), so caching
//    it never puts raw key material anywhere new;
//  - the cache is keyed by the private-key JWK *object* in a WeakMap, so it can't
//    outlive that key object (sign-out / key change → garbage collected);
//  - a failed derivation is evicted, never cached;
//  - AES-GCM safety comes from a fresh random IV per message (unchanged), not
//    from re-deriving the key.
const derivedKeyCache = new WeakMap<object, Map<string, Promise<CryptoKey>>>();

function deriveKey(myPrivateKeyJwk: JsonWebKey, peerPublicKeyB64: string): Promise<CryptoKey> {
  let perPeer = derivedKeyCache.get(myPrivateKeyJwk);
  if (!perPeer) {
    perPeer = new Map();
    derivedKeyCache.set(myPrivateKeyJwk, perPeer);
  }
  const cached = perPeer.get(peerPublicKeyB64);
  if (cached) return cached;
  const pending = deriveKeyUncached(myPrivateKeyJwk, peerPublicKeyB64);
  perPeer.set(peerPublicKeyB64, pending);
  const bucket = perPeer;
  pending.catch(() => {
    if (bucket.get(peerPublicKeyB64) === pending) bucket.delete(peerPublicKeyB64);
  });
  return pending;
}
