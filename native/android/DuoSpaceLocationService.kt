...
class DuoSpaceLocationService : Service() {
    ...
    // CRASH FIX (2nd native crash on the same post-sign-in path as the
    // DeviceStatusPlugin registerReceiver fix — this one fires from
    // BackgroundGeolocationPlugin.start(), called the moment LocationContext
    // mounts with a signed-in user, i.e. immediately after any successful
    // sign-in/sign-up): this service targets SDK 35. Beginning with Android
    // 14 (API 34), the bare two-argument `startForeground(id, notification)`
    // implicitly requests foreground-service type "none", which the OS
    // rejects outright — not with a catchable rejection, but by throwing
    // `android.app.InvalidForegroundServiceTypeException`, which is an
    // uncaught exception on the service's main-thread callback and takes
    // down the whole native process (the generic "app closed because of a
    // bug" dialog), regardless of which auth method got the user signed in.
    // The manifest already declares android:foregroundServiceType="location"
    // for this service (see scripts/patch-native-permissions.mjs) plus the
    // matching FOREGROUND_SERVICE_LOCATION permission, but declaring the
    // type in the manifest alone does not satisfy the API 34+ check — the
    // type must also be passed to startForeground() itself, exactly as
    // native/android/CallRingingService.kt already does correctly for its
    // own FOREGROUND_SERVICE_TYPE_PHONE_CALL (kept that same SDK_INT-gated
    // shape here for consistency and because the 3-arg overload doesn't
    // exist pre-Q).
