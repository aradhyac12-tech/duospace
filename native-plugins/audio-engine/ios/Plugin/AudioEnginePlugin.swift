...
/// already notes that the call engine's WebRTC engine configures the shared
...
/// an interruption (which is exactly what happens when the call engine's
...
public class AudioEnginePlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    public let pluginMethods: [CAPPluginMethod] = [
        ...
        CAPPluginMethod(name: "startWebKeepAlive", returnType: CAPPluginReturnPromise),
        CAPPluginMethod(name: "updateWebKeepAlive", returnType: CAPPluginReturnPromise),
        CAPPluginMethod(name: "stopWebKeepAlive", returnType: CAPPluginReturnPromise),
    ]
    ...
    private struct QueuedTrack {
        ...
        // Optional duration hint from track metadata — used to seed the
        // lock-screen scrubber before AVFoundation reports the real duration.
    ...
    // MARK: - Web keep-alive (YouTube-provider tracks)
    //
    // See native-plugins/audio-engine/src/definitions.ts's WebKeepAliveMeta
    // doc comment for the full design. `player` above is never touched by
    // any of this — a YouTube track stays on GroicContext's existing
    // hidden IFrame; this only (1) activates the same `.playback`
    // AVAudioSession category `activateSession()` already uses for real
    // Audius/SoundCloud playback, so iOS treats the app as an active
    // background-audio app while a YouTube track plays (on top of the
    // `audio` UIBackgroundMode already declared app-wide — see
    // scripts/patch-native-permissions.mjs), and (2) publishes
    // MPNowPlayingInfoCenter/MPRemoteCommandCenter state for that track
    // directly, since there is no `player`/AVPlayerItem for a YouTube
    // track to read metadata from the way updateNowPlaying() does.
    //
    // `isWebKeepAliveActive` gates configureRemoteCommandCenter()'s
    // handlers below: GroicContext's single-current-track model means
    // this and a real `player` session are never both the active one at
    // once, so a plain flag (not a queue/stack) is sufficient — the same
    // simplifying assumption startWebKeepAlive/stopWebKeepAlive rely on.
    private var isWebKeepAliveActive = false
    private var webKeepAliveTitle = ""
    private var webKeepAliveArtist = ""
    private var webKeepAlivePlaying = false
    private var webKeepAlivePosition: Double = 0
    private var webKeepAliveDuration: Double = 0
    private var webKeepAliveArtwork: MPMediaItemArtwork?
    ...
    private func configureRemoteCommandCenter() {
        ...
        // Explicitly enable every command we handle. addTarget() alone does
        // not reliably turn a command on (next/previous in particular can
        // show greyed-out or be absent on the lock screen / Control Center).
        let handled: [MPRemoteCommand] = [
            center.playCommand, center.pauseCommand, center.togglePlayPauseCommand,
            center.nextTrackCommand, center.previousTrackCommand, center.changePlaybackPositionCommand,
        ]
        handled.forEach { $0.isEnabled = true }
        center.playCommand.addTarget { [weak self] _ in
            guard let self = self else { return .commandFailed }
            if self.isWebKeepAliveActive { self.notifyListeners("webKeepAliveCommand", data: ["action": "play"]); return .success }
            self.player?.play(); self.updateNowPlaying(); return .success
        }
        center.pauseCommand.addTarget { [weak self] _ in
            guard let self = self else { return .commandFailed }
            if self.isWebKeepAliveActive { self.notifyListeners("webKeepAliveCommand", data: ["action": "pause"]); return .success }
            self.player?.pause(); self.updateNowPlaying(); return .success
        }
        center.togglePlayPauseCommand.addTarget { [weak self] _ in
            guard let self = self else { return .commandFailed }
            if self.isWebKeepAliveActive {
                self.notifyListeners("webKeepAliveCommand", data: ["action": self.webKeepAlivePlaying ? "pause" : "play"])
                return .success
            }
        ...
        center.nextTrackCommand.addTarget { [weak self] _ in
            guard let self = self else { return .commandFailed }
            if self.isWebKeepAliveActive { self.notifyListeners("webKeepAliveCommand", data: ["action": "next"]); return .success }
            self.advance(direction: 1); return .success
        }
        center.previousTrackCommand.addTarget { [weak self] _ in
            guard let self = self else { return .commandFailed }
            if self.isWebKeepAliveActive { self.notifyListeners("webKeepAliveCommand", data: ["action": "previous"]); return .success }
            self.advance(direction: -1); return .success
        }
        center.changePlaybackPositionCommand.addTarget { [weak self] event in
            guard let self = self, let e = event as? MPChangePlaybackPositionCommandEvent else { return .commandFailed }
            if self.isWebKeepAliveActive {
                self.notifyListeners("webKeepAliveCommand", data: ["action": "seek", "positionSeconds": e.positionTime])
                return .success
            }
    ...
    // MARK: - Web keep-alive Now Playing (mirrors updateNowPlaying() above
    // but reads from webKeepAlive* state pushed by JS instead of `player`)

    private func updateWebKeepAliveNowPlaying() {
        var info: [String: Any] = [
            MPMediaItemPropertyTitle: webKeepAliveTitle,
            MPMediaItemPropertyArtist: webKeepAliveArtist,
            MPNowPlayingInfoPropertyElapsedPlaybackTime: webKeepAlivePosition,
            MPNowPlayingInfoPropertyPlaybackRate: webKeepAlivePlaying ? 1.0 : 0.0,
        ]
        if webKeepAliveDuration > 0 { info[MPMediaItemPropertyPlaybackDuration] = webKeepAliveDuration }
        if let artwork = webKeepAliveArtwork { info[MPMediaItemPropertyArtwork] = artwork }
        info[MPNowPlayingInfoPropertyMediaType] = MPNowPlayingInfoMediaType.audio.rawValue
        MPNowPlayingInfoCenter.default().nowPlayingInfo = info
        // There is no AVPlayer behind a YouTube track for iOS to infer the
        // playing/paused UI from, so state it explicitly (iOS 13+).
        if #available(iOS 13.0, *) {
            MPNowPlayingInfoCenter.default().playbackState = webKeepAlivePlaying ? .playing : .paused
        }
    }
    ...
    @objc private func handleInterruption(_ notification: Notification) {
        ...
        if type == .began {
            ...
            // this file's header — the calling feature's own the call engine
    ...
    private func updateNowPlaying() {
        ...
        var info: [String: Any] = [
            ...
            // Buffering-while-intending-to-play still counts as playing so the
            // lock screen doesn't flash "paused" on every stall.
            MPNowPlayingInfoPropertyPlaybackRate: p.timeControlStatus == .paused ? 0.0 : 1.0,
            MPNowPlayingInfoPropertyMediaType: MPNowPlayingInfoMediaType.audio.rawValue,
        ]
        // DURATION FIX: AVPlayerItem.duration is often .indefinite during the
        // initial buffering window. Seed the scrubber with the JS-provided hint
        // (from track metadata) so the lock screen shows a progress bar immediately
        // rather than showing nothing until AVFoundation determines the real duration.
        if let duration = p.currentItem?.duration.seconds, duration.isFinite && duration > 0 {
            info[MPMediaItemPropertyPlaybackDuration] = duration
        } else if let hint = track.durationHint, hint > 0 {
            info[MPMediaItemPropertyPlaybackDuration] = hint
        }
        ...
        MPNowPlayingInfoCenter.default().nowPlayingInfo = info
        if #available(iOS 13.0, *) {
            MPNowPlayingInfoCenter.default().playbackState = p.timeControlStatus == .paused ? .paused : .playing
        }
    ...
    // MARK: - Web keep-alive plugin methods

    @objc func startWebKeepAlive(_ call: CAPPluginCall) {
        let meta = call.getObject("meta")
        webKeepAliveTitle = meta?["title"] as? String ?? "DuoSpace"
        webKeepAliveArtist = meta?["artist"] as? String ?? ""
        webKeepAliveArtwork = nil
        isWebKeepAliveActive = true
        // Reuses the exact same session activation real Audius/SoundCloud
        // playback already uses — see this file's header on why keeping
        // it to one place matters. Harmless to call again if a real
        // `player` session happened to leave it active already.
        activateSession()
        updateWebKeepAliveNowPlaying()
        if let urlString = meta?["artworkUrl"] as? String, let url = URL(string: urlString) {
            URLSession.shared.dataTask(with: url) { [weak self] data, _, _ in
                guard let self = self, let data = data, let image = UIImage(data: data) else { return }
                let artwork = MPMediaItemArtwork(boundsSize: image.size) { _ in image }
                DispatchQueue.main.async {
                    // Only apply if keep-alive is still showing this same
                    // track — a slow fetch shouldn't stomp a track the
                    // person has since skipped past (same guard
                    // fetchArtwork() above already applies for `player`).
                    guard self.isWebKeepAliveActive else { return }
                    self.webKeepAliveArtwork = artwork
                    self.updateWebKeepAliveNowPlaying()
                }
            }.resume()
        }
        call.resolve()
    ...
    @objc func updateWebKeepAlive(_ call: CAPPluginCall) {
        guard isWebKeepAliveActive else { call.resolve(); return }
        webKeepAlivePlaying = call.getBool("playing") ?? false
        webKeepAlivePosition = call.getDouble("positionSeconds") ?? 0
        webKeepAliveDuration = call.getDouble("durationSeconds") ?? 0
        if let meta = call.getObject("meta") {
            webKeepAliveTitle = meta["title"] as? String ?? webKeepAliveTitle
            webKeepAliveArtist = meta["artist"] as? String ?? webKeepAliveArtist
        }
        updateWebKeepAliveNowPlaying()
        call.resolve()
    }

    @objc func stopWebKeepAlive(_ call: CAPPluginCall) {
        isWebKeepAliveActive = false
        webKeepAlivePlaying = false
        webKeepAliveArtwork = nil
        // Only clear Now Playing / deactivate the session if a real
        // `player` track isn't also currently showing — GroicContext's
        // start/stop wiring means this shouldn't overlap in practice, but
        // guard it here anyway rather than assume the caller always gets
        // the ordering right.
