...
import { useLiveLocation, type LiveLocationData, type LiveLocationState, type LiveDebug, type LocationErrorKind } from "@/hooks/useLiveLocation";
...
import { ensureLocationPushCredential, clearLocationPushCredential } from "@/lib/locationPushCredential";
 ...
 * The call engine adapter (see the comment at the top of CallContext.tsx) — create
...
interface LocationContextValue {
  ...
  /** Why `myLocationError` is set — "denied" is a real permission problem
   *  (blocking screen is right); "timeout"/"unavailable" are transient GPS
   *  conditions where permission is fine (must NOT show the permission
   *  screen). null when there is no error. */
  myLocationErrorKind: LocationErrorKind | null;
...
export const LocationProvider = ({ children }: { children: ReactNode }) => {
  ...
  // original MapView implementation. Paused while the tab/app is hidden
  // (mirrors MapView.tsx's own 1s ticker) since nothing reads `now` while
  // the screen isn't visible; recomputes immediately on resume rather than
  ...
  // KI-12 gap 2: keep the per-device push-upload credential in step with who is
  // signed in. Separate from the effect below on purpose so it can't disturb it.
  // Never clears while auth is still loading (user is briefly null on cold
  // start) — only on a settled sign-out.
  useEffect(() => {
    if (!Capacitor.isNativePlatform() || authLoading) return;
    if (!user) { void clearLocationPushCredential(); return; }
    void ensureLocationPushCredential(user.id);
  }, [user?.id, authLoading]);
  ...
  const value = useMemo<LocationContextValue>(() => ({
    ...
    myLocationErrorKind: live.errorKind,
    ...
    transportMode, now, online, live.location, live.state, live.error, live.errorKind, live.permission,
