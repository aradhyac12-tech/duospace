import { PhoneIncoming, PhoneOutgoing, PhoneMissed, PhoneOff, Trash2 } from "lucide-react";
...
// Four distinct row states, not three: a plain "missed" (ring lapsed / the
// receiver never got to it) and a "declined" (receiver actively rejected —
// call_history.declined_at IS NOT NULL, migration 20260824_call_declined_marker.sql)
// read the same from `status` alone but mean different things, so they get
// their own icon + label rather than being folded into one "missed" bucket.
//
// AVATAR → DIRECTION ICON: the leading circle used to show the partner's
// profile photo (with a tiny direction badge stacked on top of it). Photo
// removed — the circle IS the direction/status icon now, full-size and
// color-coded, same WhatsApp/Phone-app convention: green for a call that
// connected either way, destructive-red for missed/declined. `direction`
// is computed per-viewer in Calls.tsx (`call.caller_id === user.id`), off
// the same shared call_history row both partners read — so the exact same
// call correctly shows "Outgoing" on the caller's device and "Incoming" +
// "Missed"/"Declined" on the receiver's, without any per-row branching here.
const callIcons = {
...
const callIconColor = {
  outgoing: "text-emerald-500 bg-emerald-500/10",
  incoming: "text-emerald-500 bg-emerald-500/10",
...
const CallHistoryRow = ({
  call, index, isMissed, isDeclined, direction, onDelete, partnerAvatar, partnerName, isLast,
}: {
  call: CallRecord; index: number; isMissed: boolean;
  /** True only when the receiver explicitly rejected the call
   *  (call_history.declined_at IS NOT NULL) — a strict subset of
   *  isMissed, since every declined call is also a missed one. */
  isDeclined: boolean;
  ...
  const type = isDeclined ? "declined" : isMissed ? "missed" : direction;
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      >
        <div className={`h-9 w-9 rounded-full shrink-0 flex items-center justify-center ${callIconColor[type as keyof typeof callIconColor] || "text-muted-foreground bg-muted"}`}>
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium flex items-center gap-2 truncate">
            {partnerName || "Partner"}
            {isMissed && (
              <span className="text-[10px] text-destructive">{isDeclined ? "Declined" : "Missed"}</span>
            )}
          </p>
