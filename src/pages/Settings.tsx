...
import { clearConsentCache } from "@/lib/privacy/consent";
import { secureWipeAll } from "@/lib/privacy/secureStorage";
import { wipeLocalConversationData } from "@/lib/localDb";
import { clearSettingsCache, readSettingsCache, writeSettingsCache } from "@/lib/settingsCache";
...
async function signOutAndClearPushTokens(userId: string | null): Promise<void> {
  // OFFLINE-FIRST FIX (2026-09-21) — three real problems here:
  //  1. The user id came from supabase.auth.getUser(), a NETWORK call: offline
  //     it threw, the whole cleanup (incl. the local-data wipe) was silently
  //     skipped, and the sign-out below then "succeeded" without doing anything.
  //  2. supabase.auth.signOut() does not THROW on a network failure — it returns
  //     { error } and leaves the session in place — so the caller's "couldn't
  //     sign out" toast (whose comment says "e.g. fully offline") could never
  //     fire; the person just stayed signed in with a dead button.
  //  3. My earlier local wipe ran BEFORE signOut, so a sign-out that then
  //     failed would have wiped the device and left the person signed in.
  // Now: the id comes from the local session; going offline is refused up front
  // with a clear reason (signing out must also stop THIS phone receiving your
  // push notifications, which needs the server — a local-only sign-out would
  // leave them on); the session is removed even if the server call errors
  // (staying signed in is worse than a stale server row, as the original
  // comment said); and local data is wiped only AFTER the session is gone.
  if (!requireOnline("Signing out")) throw new Error("offline");
  try {
    if (userId) {
      await supabase.from("profiles").update({ push_token: null, push_platform: null }).eq("user_id", userId);
      const deviceId = await getDeviceId();
      await supabase.from("push_tokens").update({
        is_valid: false, invalidated_reason: "signed_out",
      } as never).eq("user_id", userId).eq("device_id", deviceId);
    }
  } catch {
    /* best-effort — a failed cleanup call must never block sign-out itself */
  }
  clearConsentCache(); // Phase 1 privacy foundation — stop trusting any cached consent state for the outgoing session
  const { error } = await supabase.auth.signOut();
  if (error) {
    // The server call failed; still end THIS device's session.
    const { error: localError } = await supabase.auth.signOut({ scope: "local" });
    if (localError) throw localError;
  }
  if (userId) {
    // Phase 1 privacy foundation: wipe secureStorage-encrypted local data (see
    // .ai/SECURITY_MODEL.md Key Management), plus the offline-first stores:
    // on-device message history, cached chat media, cached settings values.
    await secureWipeAll(userId).catch(() => {});
    await wipeLocalConversationData(userId).catch(() => {});
...
const Settings = () => {
  ...
  // Seeded from the last known values (see lib/settingsCache.ts) so the hub
  // shows real data instantly and offline instead of "Loading…" / a wrong
  // "Not connected yet".
  type HubCache = { username: string; partnerLinked: boolean; partnerName: string; pendingRequestCount: number };
  const hubSeed = useMemo(() => readSettingsCache<HubCache>(user?.id, "hub"), [user?.id]);
  const [partnerLinked, setPartnerLinked] = useState<boolean | null>(hubSeed ? hubSeed.partnerLinked : null);
  const [partnerName, setPartnerName] = useState(hubSeed?.partnerName ?? "");
  const [pendingRequestCount, setPendingRequestCount] = useState(hubSeed?.pendingRequestCount ?? 0);
  ...
  useEffect(() => {
    if (!user) return;
    // Latest known values, merged as answers arrive and written back so the
    // next open (or an offline open) starts from them.
    const snapshot: HubCache = {
      username: hubSeed?.username ?? "", partnerLinked: hubSeed?.partnerLinked ?? false,
      partnerName: hubSeed?.partnerName ?? "", pendingRequestCount: hubSeed?.pendingRequestCount ?? 0,
    };
    const persist = () => writeSettingsCache(user.id, "hub", snapshot);
    if (hubSeed?.username) setUsernameInput(hubSeed.username);

    supabase.from("profiles")
      .select("username,partner_id")
      .eq("user_id", user.id).single()
      .then(({ data, error }: { data: any; error: unknown }) => {
        // A FAILED query says nothing about the pairing. It used to fall
        // through to setPartnerLinked(!!undefined) === false, i.e. "Not
        // connected yet" for a linked person whenever the phone was offline.
        if (error || !data) return;
        if (data.username) { setUsernameInput(data.username); snapshot.username = data.username; }
        setPartnerLinked(!!data.partner_id);
        snapshot.partnerLinked = !!data.partner_id;
        if (!data.partner_id) { setPartnerName(""); snapshot.partnerName = ""; }
        persist();
        if (data.partner_id) {
          supabase.from("profiles").select("display_name").eq("user_id", data.partner_id).single()
            .then(({ data: pp }: { data: any }) => {
              if (!pp) return;
              const name = pp.display_name || "Partner";
              setPartnerName(name); snapshot.partnerName = name; persist();
            });
        }
      });
    ...
    supabase.from("partner_requests" as any)
      ...
      .then(({ count, error }: { count: number | null; error: unknown }) => {
        if (error) return; // keep the last known badge rather than zeroing it
        setPendingRequestCount(count || 0);
        snapshot.pendingRequestCount = count || 0; persist();
      });
    // eslint-disable-next-line react-hooks/exhaustive-deps -- hubSeed is derived from user.id
  }, [user]);

  const saveUsername = async () => {
    if (!user || !usernameInput.trim() || savingUsername) return;
    if (!requireOnline("Saving your username")) return;
  ...
  // Returns false on failure so ConfirmActionDialog (which now closes itself
  // after a successful confirm) keeps the dialog open for a retry.
  const handleSignOut = async (): Promise<boolean> => {
    if (signingOut) return false; // guards duplicate taps while the round-trip is in flight
    setSigningOut(true);
    hapticMedium();
    try {
      await signOutAndClearPushTokens(user?.id ?? null);
      ...
      setSigningOut(false);
      // Offline was already explained by requireOnline() — don't stack a second toast on it.
  ...
  return (
    ...
    >
      ...
      <div className="px-5 space-y-2 pt-5">
        ...
        {matches("notification sound ringtone call ring haptic vibrate vibration chime pop marimba bell droplet crystal harp sparkle tick whistle glass retro digital melody bells pulse sunrise waltz groove background push") && (
          <SettingsHubRow
        ...
        {matches("privacy ai consent data insights mood processing cloud local") && (
          <SettingsHubRow
            icon={Sparkles} label="Privacy & AI Data"
            summary="Control what AI features can access — off by default"
            onClick={() => navigate("/settings/privacy-ai")}
          />
        )}
