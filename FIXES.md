...
## 20. Music search — found and fixed the actual root cause, deployed live

**File:** `supabase/functions/music-search/index.ts`

You reported: no latest songs, search returning zero results, and
Trending/Home showing the same fixed songs that never change. Checked
the live edge function directly (`get_edge_function`, not just the repo
source) and found **the deployed version (v8) was a different, older
build than what's in this repo** — the repo's own comments claim the
hardcoded mock-song fallback was removed, but the live function still
had it:

```
fallbackResults(query) → Stephen Sanchez, Ed Sheeran, Billie Eilish,
One Direction, Glass Animals, JVKE — returned as a normal 200 response
whenever YouTube + Piped both failed.
```

That's exactly "the same fixed songs every time" — an outage silently
disguised as a working search. It also had no Invidious tier at all
(YouTube → Piped → mock pool only).

Pulled live edge function logs (`query_logs`) and got hard evidence
instead of guessing: every single Piped mirror in the hardcoded list was
failing on every real request — `api.piped.yt` and
`piped-api.privacy.com.de` with DNS resolution errors, `pipedapi.leptons.xyz`
with 502, `pipedapi.adminforge.de` with 404. `pipedapi.r4fo.com` doesn't
appear on Piped's own current maintained instance list at all anymore.
No YouTube-related log lines appeared anywhere in the window, meaning
the YouTube stage never returns anything either (`YOUTUBE_API_KEY` is
either unset or not producing results — I can't read the secret value
to tell which).

**Fixed and deployed directly to the live project** (`deploy_edge_function`,
now v9):
- Removed the hardcoded mock-song fallback entirely — a total provider
  outage now returns a real 502 with a debug trail instead of 6 fake
  "results".
- Refreshed the Piped instance list to ones currently confirmed live on
  TeamPiped's own maintained list (kavin.rocks flagship instance,
  owo.si, codespace.cz, ducks.party — dropped the ones confirmed dead in
  the logs above).
- Refreshed the Invidious instance list (docs.invidious.io) — `yewtu.be`
  and `invidious.f5.si` dropped (multiple 2026 sources flag yewtu.be as
  unreliable; f5.si isn't on any current list).
- Added the Invidious tier back as a genuine second independent
  provider, matching the repo's own source (which the live deploy never
  actually picked up).

**The one thing I still can't fix for you:** whether `YOUTUBE_API_KEY`
is actually working. The debug trail in the search response (or the
edge function logs) will now say exactly why on the next real search —
most likely either the secret isn't set, the YouTube Data API v3 isn't
enabled for that key in Google Cloud Console, or its daily quota is
exhausted. Even without it, search should now actually work again via
the Piped/Invidious tiers — but a real API key is the only durable fix,
since public mirrors will keep rotting on their own schedule regardless
of what's hardcoded here.

**Also applied a pending migration** (`whatsapp_import_batches_and_media`,
dated in the repo but not yet on the live database) — checked existing
data first (`execute_sql`: all 421 `imported_chats` rows were already
`file_type = 'text'`) before adding the new `CHECK` constraint, so it
was safe to apply as-is.
