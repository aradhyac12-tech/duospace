...
import { deleteLocalMedia } from "@/lib/mediaCache";
...
const MemoryWall = ({ partnerId, focusMemoryId }: MemoryWallProps) => {
  ...
  // OFFLINE-FIRST: the wall paints from the last known list (encrypted on this
  // phone — lib/screenCache.ts) and photos come from the on-device media cache,
  // so it opens instantly and offline. The cache holds the RAW stored image
  // path, never a signed URL (those expire), which is re-resolved on load;
  // rawUrlById remembers the raw value for each row on screen.
  const rawUrlById = useRef(new Map<string, string | null>());
  ...
  useEffect(() => {
    if (!user) return;
    let cancelled = false;
    void readScreenCache<Memory[]>(user.id, `memories-${partnerId ?? "solo"}`).then(async (rows) => {
      if (cancelled || !rows || fetchedRef.current) return;
      rows.forEach((m) => rawUrlById.current.set(m.id, m.image_url));
      const resolved = await resolveSignedUrls("memories", rows, "image_url");
  ...
  }, [user?.id, partnerId]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    if (!user || !hasDataRef.current) return;
    writeScreenCache(user.id, `memories-${partnerId ?? "solo"}`,
      memories.map((m) => ({ ...m, image_url: rawUrlById.current.has(m.id) ? rawUrlById.current.get(m.id)! : m.image_url })));
  }, [memories, user, partnerId]);
  ...
  const load = async () => {
    ...
    try {
      ...
      if (data) {
        (data as Memory[]).forEach((m) => rawUrlById.current.set(m.id, m.image_url));
        hasDataRef.current = true; fetchedRef.current = true;
        setMemories(await resolveSignedUrls("memories", data as Memory[], "image_url"));
      }
    } catch (err) {
      // Cached list already showing → a failed refresh isn't an error card.
      if (!hasDataRef.current) setLoadError(capture("DS-US-005", { component: "MemoryWall", action: "load", cause: err }));
    } finally {
      setLoading(false);
    }
  };
  ...
  useReconnectRefetch(useCallback(() => { void loadRef.current(); }, []));
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "memories" }, (payload) => {
        if (payload.eventType === "INSERT") {
          ...
          if (ids.includes(m.creator_id)) {
            rawUrlById.current.set(m.id, m.image_url);
  ...
  const addMemory = async () => {
    ...
    if (!requireOnline("Adding a memory")) return;
    setUploading(true);
    try {
      ...
      if (data) {
        ...
        rawUrlById.current.set(created.id, created.image_url);
  ...
  const deleteMemory = async (memory: Memory) => {
    if (!requireOnline("Deleting a memory")) return;
    setDeleting(true);
    ...
    // Deleted for good → don't leave its photo behind in the on-device cache.
    const raw = rawUrlById.current.get(memory.id);
    const storedPath = raw ? extractStoragePath("memories", raw) : null;
    if (storedPath) void deleteLocalMedia("memories", storedPath);
    rawUrlById.current.delete(memory.id);
