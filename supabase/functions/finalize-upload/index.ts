...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // BUG FIX ("Missing chunk 0" on voice/photo/file/camera sends): the
    // client's storage.upload() call resolving successfully does not
    // guarantee the object is immediately visible to a .download() call
    // that follows within milliseconds — read-after-write is not
    // instantaneous across every path in the storage backend. This bites
    // hardest on exactly the case reported: a SMALL, single-chunk upload
    // (a short voice note, a modest photo), where chunk 0 is both the
    // first AND the only/last chunk uploaded, so it gets the LEAST time to
    // settle before finalize's very next line tries to read it back — for
    // a large multi-chunk file, chunk 0 is uploaded earliest and so
    // incidentally gets the most settling time by the time this loop
    // reaches it, which is why this was never noticed on bigger files.
    // A short, cheap retry-with-backoff before concluding a chunk is
    // truly gone (as opposed to just not-yet-visible) fixes this without
    // weakening the real "missing chunk" error path for an actually lost
    // chunk (e.g. one that failed all client-side retries) — that still
    // correctly 422s after these attempts are exhausted.
    async function downloadChunkWithRetry(partName: string, attempts = 6) {
      let lastError: unknown = null;
      // STRENGTHENED (still reported after the first pass of this fix):
      // widened from 4 attempts/~1.45s total to 6 attempts/~4.75s total.
      // The original window was sized around a "typical" read-after-write
      // lag; if storage is under real load (many chunks in flight for a
      // multi-chunk video, or a busy moment generally), the tail of that
      // lag distribution can run longer than ~1.5s. This is still a
      // one-time cost paid only in the rare case a chunk genuinely isn't
      // visible yet — the common case (chunk already visible) still
      // returns on the very first attempt with zero added delay.
      const delays = [0, 150, 400, 900, 1800, 2000];
      ...
      console.error(`[finalize-upload] chunk still missing after ${attempts} attempts: ${partName}`, lastError);
      return null;
    }
