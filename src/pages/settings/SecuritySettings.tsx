...
import { APP_LOCK_TIMEOUT_OPTIONS } from "@/lib/appLockTimer";
          ...
          {appSettings.biometricLock && (
            <div className="flex items-center gap-3 px-4 py-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-muted-foreground">Grace period before re-locking when backgrounded</p>
              </div>
              <div className="flex items-center gap-0.5 rounded-full bg-muted p-1 overflow-x-auto max-w-[150px]" role="radiogroup" aria-label="Lock after">
                {APP_LOCK_TIMEOUT_OPTIONS.map((opt) => (
                  <button
                    key={opt.value}
                    role="radio"
                    aria-checked={(appSettings.appLockTimeout ?? 0) === opt.value}
                    aria-label={opt.label}
                    onClick={() => { hapticLight(); updateSetting("appLockTimeout", opt.value); }}
                    className={`px-2 py-1 rounded-full text-[10px] whitespace-nowrap transition-colors shrink-0 ${
                      (appSettings.appLockTimeout ?? 0) === opt.value ? "bg-accent text-accent-foreground" : "text-muted-foreground"
                    }`}
                  >
                    {opt.value === 0 ? "Instant" : `${opt.value}m`}
                  </button>
                ))}
              </div>
            </div>
