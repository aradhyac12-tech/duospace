...
const runWhatsAppImport = async (
  ...
  if (zip && mediaRows.length) {
    ...
    // SPEED FIX: upload up to 4 attachments at once (was strictly one by
    // one), and give each a real Content-Type — JSZip blobs have none, so
    // everything used to land as application/octet-stream and wouldn't
    // preview/play.
    ...
    const unique = Array.from(new Set(mediaRows.map(m => m.attachName!.toLowerCase())));
    let done = 0;
    let cursor = 0;
    const worker = async () => {
      while (cursor < unique.length) {
        const idx = cursor++;
        const base = unique[idx];
        const zipPath = byBasename.get(base);
        try {
          if (!zipPath) continue; // referenced but not in this export ("Without media")
          const raw: Blob = await zip.files[zipPath].async("blob");
          const ext = base.split(".").pop() || "";
          const contentType = MIME[ext] || "application/octet-stream";
          const blob = new Blob([raw], { type: contentType });
          const safeName = base.replace(/[^a-z0-9_.-]/gi, "_");
          const storagePath = `${user.id}/imports/${batchId}/${idx}_${safeName}`;
          const { error: upErr } = await supabase.storage.from("chat-files").upload(storagePath, blob, { contentType, upsert: true });
          if (upErr) {
          ...
          setImportProgress(`Uploading media… ${done}/${unique.length}`);
        }
      }
    };
  ...
  return (
    ...
    >
      ...
      <div className="px-5 pt-5 space-y-3">
        ...
        <input ref={whatsappFileRef} type="file" accept=".txt,.zip,text/plain,application/zip,application/x-zip-compressed,application/x-zip,application/octet-stream" className="hidden"
          onChange={async e => {
            ...
            try {
              let text = "";
              // FIX: Android share targets / file pickers often hand over the
              // WhatsApp export with no ".zip" extension (or ".ZIP", or a
              // content:// name like "WhatsApp Chat with X"), so the old
              // endsWith(".zip") check silently fell through and nothing
              // loaded. Sniff the real ZIP signature ("PK") instead.
              ...
              if (!isZip) {
                ...
                try {
                  setImportProgress("Opening ZIP…");
                  const JSZip = (await import("jszip")).default;
                  const zip = await JSZip.loadAsync(file);
                  const txtFiles = Object.keys(zip.files).filter(f =>
                    !zip.files[f].dir && /\.txt$/i.test(f) && !f.startsWith("__MACOSX/") && !f.split("/").pop()!.startsWith("._"));
                  // iOS exports "_chat.txt"; Android "WhatsApp Chat with X.txt".
                  // A chat can also CONTAIN .txt documents, so prefer those
                  // names, then the largest .txt, rather than the first one.
                  const txtFile =
                    txtFiles.find(f => /(^|\/)_chat\.txt$/i.test(f)) ||
                    txtFiles.find(f => /whatsapp/i.test(f.split("/").pop()!)) ||
                    txtFiles.sort((a, b) => ((zip.files[b] as any)._data?.uncompressedSize ?? 0) - ((zip.files[a] as any)._data?.uncompressedSize ?? 0))[0];
                  if (!txtFile) throw new Error("This ZIP has no chat .txt file. In WhatsApp use ⋮ → More → Export chat.");
                  text = await zip.files[txtFile].async("text");
                  zipRef.current = zip; // kept for the media-extraction pass in runWhatsAppImport
                } catch (zipErr: any) {
                  const msg = zipErr?.message || String(zipErr);
                  toast({
                    title: "ZIP import failed",
                    description: /memory|allocation|RangeError/i.test(msg)
                      ? "This export is too large to open on this device. Export the chat again with \"Without media\"."
                      : msg,
                    variant: "destructive",
                  });
                  setImportingWhatsApp(false); setImportProgress(""); e.target.value = ""; return;
                }
              }
              text = text.replace(/^\ufeff/, "");
              if (!text.trim()) {
              ...
              const re = /^\[?(\d{1,4}[\/\-\.]\d{1,2}[\/\-\.]\d{2,4}),?\s+(\d{1,2}[:.]\d{2}(?:[:.]\d{2})?(?:\s*[APap]\.?\s?[Mm]\.?)?)\]?\s*[-–]?\s*([^:]+):\s*(.*)/;
              const JUNK_CONTENT = [
              ...
              const parseTimestamp = (datePart: string, timePart: string): Date | null => {
                ...
                const tp = timePart.trim().replace(/([ap])\.\s*m\.?/i, "$1m").replace(/^(\d{1,2})\.(\d{2})(?:\.(\d{2}))?/, (_m, h, mi, se) => se ? `${h}:${mi}:${se}` : `${h}:${mi}`).replace(/\s*(am|pm)$/i, m => " " + m.trim().toUpperCase());
