...
describe("NetworkQualityTracker hysteresis", () => {
  ...
  it("requires a much longer streak to recover than to degrade (asymmetric hysteresis)", () => {
    ...
    // ...but does recover once the streak requirement (6) is actually met.
    tracker.sample(sample({ rttMs: 40, packetLossPct: 0, jitterMs: 10 })); // 3
    tracker.sample(sample({ rttMs: 40, packetLossPct: 0, jitterMs: 10 })); // 4
    expect(tracker.sample(sample({ rttMs: 40, packetLossPct: 0, jitterMs: 10 }))).toBeNull(); // 5 — one short
