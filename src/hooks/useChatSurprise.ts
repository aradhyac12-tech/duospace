    ...
    for (const s of surprises) {
      if (s.creator_id !== user.id && !receivedAlready.has(s.id)) {
        ...
        if (s.created_at > sessionStartRef.current) {
          void (async () => {
            // Slim list row → fetch the body first: the mood (and so the
            // haptic score) is read from the surprise's own code.
            const full = await ensureSurpriseBody(s);
            if (!full) return;
            const mood = analyzeSurpriseContent(full.html_content, full.css_content, full.js_content).mood;
