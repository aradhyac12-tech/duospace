...
import { DockBadgesProvider } from "@/contexts/DockBadgesContext";
// FIX (same class of bug as DS-UNKNOWN-001 on PeekGuard — see App.tsx):
// MoodDetector was dynamically imported to keep @mediapipe/tasks-vision's
// several-hundred-KB wrapper JS out of the entry chunk, but that heavy
// payload was already correctly isolated behind its own dynamic import()
// inside faceRecognition.ts's getLandmarker() — `import type` here is
// compile-time only, zero runtime cost. The lazy() wrapper around the much
// smaller MoodDetector component itself bought no real deferral, just an
// extra async chunk boundary that's the same class of hazard already fixed
// for PeekGuard. Reverted to a static import for the same reason.
...
import UnlinkRequestHost from "@/components/UnlinkRequestHost";
...
const AppLayout = () => {
  ...
  // ACCIDENTAL-EXIT/RECOVERY FIX: useAppNative's backButton handler used to
  // have no idea a call could be active — see that hook's own comment.
  // machineState.status !== IDLE and not terminal is the same "is there
  // really a call right now" check CallStage/MinimizedCallBubble already
  // key off of elsewhere in this app.
  const { callMachineState, setIsCallMinimized } = useCall();
  const isCallActive = useCallback(
    () => callMachineState.status !== "IDLE" && !isTerminal(callMachineState.status),
    [callMachineState.status],
  );
  const onMinimizeCall = useCallback(() => setIsCallMinimized(true), [setIsCallMinimized]);
  const { isOnline, connectionStatus } = useAppNative(isAppLocked, setIsAppLocked, appSettings.biometricLock, appSettings.appLockTimeout, isCallActive, onMinimizeCall);
  ...
  return (
    <GroicProvider>
      ...
      {/* Mounted once, above the isPrimaryTab route swap below, so the dock
          badge subscriptions/counts survive navigation between primary
          tabs and every other page instead of tearing down and re-fetching
          (and visibly flashing back to 0) every time — see
          DockBadgesContext.tsx. */}
      <DockBadgesProvider>
      ...
      <div className="h-[100dvh] bg-background overflow-x-hidden flex flex-col no-overscroll">
        ...
        <GroicFullPlayer />
        <GroicInviteBanner />
        {/* Approval dialog for "your partner asked to unlink" — mounted here so
            it appears on whatever screen the person is on, not just Settings. */}
        <UnlinkRequestHost />
        {isPrimaryTab ? (
