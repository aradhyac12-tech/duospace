 ...
 *
 * Closing on success is this component's job, not each caller's: once
 * `onConfirm` settles, the dialog closes itself. A caller that wants the
 * dialog to STAY open (e.g. the request failed and the person should be able
 * to retry) resolves to `false`; throwing also leaves it open. Any other
 * return value — including nothing at all — counts as success. (This used to
 * be left to every caller, and none of them did it, so the dialog sat there
 * after the action had already succeeded.)
 */
export interface ConfirmActionDialogProps {
  ...
  authNote?: string;
  /**
   * Optional fifth row for actions that need someone else's consent before
   * anything happens (e.g. unlinking a partner). Omit for actions that only
   * involve the person tapping the button.
   */
  approvalNote?: string;
  destructive?: boolean;
  confirmLabel?: string;
  /** Resolve to `false` to keep the dialog open (failed / needs a retry). */
  onConfirm: () => void | boolean | Promise<void | boolean>;
}

const ConfirmActionDialog = ({
  open, onOpenChange, title, whatHappens, dataAffected, reversible, reversibleNote,
  authRequired, authNote, approvalNote, destructive = true, confirmLabel, onConfirm,
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      // Close on success. Calls the parent's setter directly (not the
      // pending-guarded wrapper below) — same as the Cancel button does.
      ...
      >
        ...
        <div className="space-y-3 py-1">
          ...
          <dl className="space-y-2 rounded-xl bg-muted/40 p-3">
            ...
            {approvalNote && (
              <div className="flex gap-2 text-[11px]">
                <dt className="w-24 shrink-0 font-medium text-muted-foreground">Approval</dt>
                <dd className="text-foreground/90 font-medium">{approvalNote}</dd>
              </div>
            )}
          </dl>
        </div>
