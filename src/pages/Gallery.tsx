...
const MediaThumbnail = ({ item, onClick, selectionMode, selected }: {
  ...
  if (item.file_type === "video") {
    return (
      <div className={`w-full h-full relative cursor-pointer active:scale-95 transition-transform duration-100 ${selected ? "ring-2 ring-inset ring-primary" : ""}`} onClick={() => { onClick(); }}>
...
const Gallery = () => {
  ...
  const loadGallery = useCallback(async (uid: string, pid: string | null) => {
    const { data: mine } = await supabase.from("gallery_items").select("id,owner_id,file_url,file_type,is_shared,is_favorite,created_at")
      .eq("owner_id", uid).order("created_at", { ascending: false });
    const myList = await resolveGalleryItems((mine || []) as GalleryItem[]);
    setMyItems(myList);

    if (pid) {
      const { data: partner } = await supabase.from("gallery_items").select("id,owner_id,file_url,file_type,is_shared,is_favorite,created_at")
              ...
              return (
                ...
                >
                  <div className="h-20 w-20 rounded-xl bg-muted overflow-hidden flex items-center justify-center">
                    {cover ? (
                      <img src={cover.file_url} alt="" className="h-full w-full object-cover" />
                    ) : (
