...
- Inside the installed Capacitor app: Audius/SoundCloud tracks get a real,
  durable background notification from the native engine
  (`MediaSessionService` on Android, `MPNowPlayingInfoCenter` on iOS) that
  survives backgrounding/locking. A YouTube track playing in the app's
  WebView gets the same `navigator.mediaSession` wiring and can show a
  notification while the WebView is alive, but Android can suspend a
  backgrounded WebView's JS/audio without the foreground-service
  protection a native player gets.
  **Update (2026-09-09):** closing this gap does NOT require extracting
  YouTube's audio, as this doc previously said — see
  `docs/MUSIC_BACKGROUND_KEEPALIVE.md`. A second, audio-less foreground
  service/Now Playing registration now mirrors GroicContext's own
  already-known IFrame state (position/duration/playing/metadata) into a
  real OS MediaSession, giving Android the same "this app is actively
  doing background media work" signal Audius/SoundCloud tracks already
  had — without this plugin ever touching a YouTube audio byte. This is a
  structurally-complete first draft, written to the real Media3/
  AVFoundation APIs but not compiled/run in this working environment (no
  Android SDK/Xcode available) — see that doc's own verification-status
