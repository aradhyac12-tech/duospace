...
import { NetworkQualityTracker, recommendationForTier, type QualityTier, type NetworkSample } from "@/lib/networkQualityClassifier";
import { mark as markCallLatency, finishTrace as finishCallLatencyTrace } from "@/lib/callLatency";
...
interface UseDailyCallReturn {
  ...
  /**
   * Fine-grained 5-tier classification (excellent/good/fair/poor/critical)
   * from src/lib/networkQualityClassifier.ts — a finer signal than
   * `networkQuality` above, which stays driven by Daily's own coarser
   * ~30s-averaged assessment (unchanged). Not wired into any UI by this
...
export const useDailyCall = (): UseDailyCallReturn => {
  ...
  // Fine-grained adaptive quality controller (see
  // src/lib/networkQualityClassifier.ts) — polls getNetworkStats() every
  // ~2s while joined and feeds it a hysteresis tracker. Separate from (not
  // a replacement for) the network-quality-change handler below, which
  // drives the existing degrade-to-audio-only behavior off Daily's own
  // coarser, ~30s-averaged assessment. This one is what makes a bounded,
  // debounced RECOVERY back to video possible — see qualityPollRef's setup
  // near call.join() for why that's new behavior, not just new telemetry.
  const qualityTrackerRef = useRef(new NetworkQualityTracker());
  const qualityPollRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const [detailedNetworkQuality, setDetailedNetworkQuality] = useState<QualityTier>("good");
  ...
  // CALL-TEARDOWN-ON-ERROR FIX (see teardownLiveCall below): mirrors
  // callRef, but assigned the instant a call object is created rather
  // than only once join() succeeds. `call` itself is declared `const`
  // inside joinCall's try block for full type-narrowing everywhere else
  // in that function — this ref exists purely so the catch block (a
  // separate block scope, where the try's `const call` isn't visible at
  // all) can still find and tear down an object whose join() rejected.
  const lastCreatedCallRef = useRef<DailyCall | null>(null);
  ...
  // CALL-TEARDOWN-ON-ERROR FIX: applyError() (below) only ever flipped UI
  // state to "error" — the live DailyCall object, its duration timer, and
  // its mic/camera tracks were never actually torn down. Every one of the
  // brief's explicit "must become impossible" examples was reachable this
  // way: a Daily "error" event, a join() rejection, or the 30s
  // network-reconnect-timeout below all left `callState` showing an error
  // while callRef.current stayed alive — meaning callDuration kept
  // incrementing, the mic/camera stayed live, and (worse) a person could
  // tap "call" again while the dead object still sat in callRef.current,
  // hitting the BUG-03 destroy-existing-call path unnecessarily on what
  // should have already been a clean slate.
  //
  // Guarded by `callRef.current === call`: several of these teardown call
  // sites live inside event handlers closed over one specific `call`
  // instance from when it was created. If a newer joinCall() has since
  // replaced callRef.current with a fresh instance (e.g. a stale
  // "error" event from an already-superseded object arrives late), this
  // must be a no-op — tearing down unconditionally would kill the NEW,
  // healthy call because of a stale event from the old one.
  const teardownLiveCall = useCallback((call: DailyCall) => {
    ...
    try { call.leave(); } catch (err) { console.warn('[useDailyCall] leave() failed during error teardown (already left):', err); }
    destroyCall(call);
    callRef.current = null;
    if (lastCreatedCallRef.current === call) lastCreatedCallRef.current = null;
  ...
  const joinCall = useCallback(async (url: string, token?: string, videoOff = false) => {
    ...
    try {
      ...
      if (callRef.current) {
        ...
        callRef.current = null;
        lastCreatedCallRef.current = null;
        cleanupAudioElements();
        if (timerRef.current) { clearInterval(timerRef.current); timerRef.current = null; }
      }
      ...
      lastCreatedCallRef.current = call;
      ...
      call.on("joined-meeting", () => {
        ...
        if (Object.keys(call.participants()).length > 1 && !timerRef.current) {
          markCallLatency("remote_participant_detected");
      ...
      call.on("participant-joined", evt => {
        if (evt?.participant) {
          ...
          if (evt.participant.tracks?.audio?.state === "playable") markCallLatency("first_remote_audio");
        }
      ...
      call.on("error", (evt: DailyErrorEvent) => {
        clearJoinWatchdog();
        applyError(evt);
        // See teardownLiveCall's comment — a Daily "error" event used to
        // leave the call object, its duration timer, and its mic/camera
        // tracks alive underneath the error UI.
        teardownLiveCall(call);
      });
      ...
      call.on("network-connection" as never, ((evt: DailyNetworkConnectionEvent) => {
        if (evt?.event === "interrupted") {
          ...
          reconnectTimerRef.current = setTimeout(() => {
            ...
            // Same teardown gap as the "error" handler above — a timed-out
            // reconnect must not leave the dead call/mic/timer running
      ...
      // Adaptive quality controller polling (see networkQualityClassifier.ts).
      // Fresh tracker per call — a tier learned from a previous call must
      // never carry over and immediately fire a transition against this one.
      qualityTrackerRef.current.reset("good");
      if (qualityPollRef.current) clearInterval(qualityPollRef.current);
      qualityPollRef.current = setInterval(() => {
        if (!mountedRef.current || callRef.current !== call) return; // stale interval from a superseded call — see teardownLiveCall's comment on this same guard pattern
        // Cast defensively rather than trusting the bundled DailyCall type
        // to declare getNetworkStats()'s exact shape (verified against
        // Daily's docs at https://docs.daily.co/reference/daily-js/instance-methods/get-network-stats,
        // but not something this sandbox could compile-check — see report).
        (call as unknown as { getNetworkStats: () => Promise<unknown> }).getNetworkStats().then((stats) => {
          if (!mountedRef.current || callRef.current !== call) return;
          const latest = (stats as { stats?: { latest?: Record<string, number | null> } })?.stats?.latest ?? {};
          const rttMs = typeof latest.averageNetworkRoundTripTime === "number" ? latest.averageNetworkRoundTripTime : (typeof latest.networkRoundTripTime === "number" ? latest.networkRoundTripTime : null);
          const packetLossPct = typeof latest.totalRecvPacketLoss === "number" ? latest.totalRecvPacketLoss : (typeof latest.totalSendPacketLoss === "number" ? latest.totalSendPacketLoss : null);
          const jitterMs = typeof latest.videoRecvJitter === "number" ? latest.videoRecvJitter : (typeof latest.audioRecvJitter === "number" ? latest.audioRecvJitter : null);
          const availableBitrateBps = typeof latest.availableOutgoingBitrate === "number" ? latest.availableOutgoingBitrate : null;
          const sample: NetworkSample = { rttMs, packetLossPct, jitterMs, availableBitrateBps };
          const newTier = qualityTrackerRef.current.sample(sample);
          if (newTier === null) return; // no hysteresis-confirmed change this poll
          setDetailedNetworkQuality(newTier);

          // RECOVERY: turn video back on after a SUSTAINED return to good
          // quality — gated tightly:
          //   - only if OUR auto-downgrade turned it off (autoAudioFallback),
          //     never a manual user choice (toggleVideo clears the flag the
          //     instant the user takes over — see toggleVideo above)
          //   - only on "good"/"excellent", not merely "not critical anymore"
          //   - only after upgradeStreak consecutive good polls (see
          //     NetworkQualityTracker's asymmetric hysteresis — 4x the
          //     degrade streak by default), so this cannot flap.
          // This is new behavior versus the auto-fallback logic's original
          // "stays off until the user manually re-enables it" comment above
          // — that comment predates this tracker existing; the flapping
          // concern it was guarding against is exactly what the asymmetric
          // hysteresis here is for. Flagged prominently in the PR/report
          // this shipped with, not just here, since it's a product-behavior
          // change worth a second pair of eyes, not just an engineering one.
          const rec = recommendationForTier(newTier);
          if (rec.videoEnabled) {
            setAutoAudioFallback((wasAutoOff) => {
              if (!wasAutoOff) return wasAutoOff; // not our fallback to undo — leave alone (covers: never auto-fell-back, or user already took manual control)
              setIsVideoOn((videoCurrentlyOn) => {
                if (videoCurrentlyOn) return videoCurrentlyOn; // already on somehow — nothing to do
                try {
                  callRef.current?.setLocalVideo(true);
                  logInfo("call.quality", `Auto-restored video after sustained recovery to '${newTier}'`);
                  return true;
                } catch (err) {
                  logWarn("call.quality", "Failed to auto-restore video after recovery", extractErrorMessage(err, ""));
                  return videoCurrentlyOn;
                }
              });
              return false; // clear the auto-fallback flag — we just handled it
            });
          }
        }).catch(() => {}); // getNetworkStats() is best-effort telemetry — never let a rejected poll surface as a call error
      }, 2000);
      ...
      clearJoinWatchdog();
      applyError(err);
      // CALL-TEARDOWN-ON-ERROR FIX: `call` (the try block's own `const`)
      // isn't visible in this catch — a rejected call.join() (or anything
      // else in the try after createCallObject()) used to leave a fully
      // created DailyCall instance alive in callRef.current, with every
      // listener above still attached, doing nothing until the NEXT
      // joinCall() attempt's own BUG-03 cleanup happened to destroy it.
      // lastCreatedCallRef exists purely to reach that same object here.
      if (lastCreatedCallRef.current) teardownLiveCall(lastCreatedCallRef.current);
    } finally {
  ...
  }, [attachTrack, attachAudioTrack, cleanupAudioElements, destroyCall, applyError, clearJoinWatchdog, teardownLiveCall]);
