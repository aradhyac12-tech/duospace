...
export const GroicProvider = ({ children }: { children: ReactNode }) => {
  ...
  // one-time listener the current implementation.
  ...
  // STALE-CLOSURE FIX: the realtime handlers are bound once, inside
  // subscribeChannel(), when the guest joins. Anything this callback reads
  // from render scope is frozen at that moment — `isPlaying` was, so a
  // guest that later diverged from the host could never be corrected by a
  // tick (it compared against a value from join time and kept deciding
  // "already in the right state"). All live reads now go through refs, so
  // this callback is genuinely stable and always sees current values.
  const onGuestEvent = useCallback((event: string, payload: any) => {
    ...
    if (event === "tick") {
      ...
      // a flaky mobile connection). The tick carries the host's videoId —
      // if it doesn't match what we're playing, resync the track itself
      // instead of drift-correcting against the wrong song forever.
      if (payload.videoId && currentRef.current?.videoId !== payload.videoId) {
        loadVideo(payload.videoId, true);
        setCurrent((cur) => cur && cur.videoId === payload.videoId ? cur : {
          id: payload.videoId, videoId: payload.videoId,
          title: cur_titleFallback(payload), artist: "", thumbnail: null,
        });
        setIsPlaying(true);
        return;
      }
      ...
      if (payload.isPlaying && !playing) { p.playVideo?.(); setIsPlaying(true); }
      if (!payload.isPlaying && playing) { p.pauseVideo?.(); setIsPlaying(false); }
    }
  }, [loadVideo]);
  ...
  // FIX: neither GroicMiniPlayer nor GroicFullPlayer exposed any way to
  // actually stop playback and dismiss the player — GroicMiniPlayer even
  // had an unused `X` icon import, a leftover sign a close button was
  // meant to exist but never got wired up. The only way to make the mini
  // bar disappear was to let a track finish with an empty queue.
  const close = useCallback(() => {
    const p = playerRef.current;
    try { p?.stopVideo?.(); } catch { /* player not ready */ }
  ...
  const value: GroicAPI = {
    current, queue, isPlaying, position, duration, expanded, hidden,
