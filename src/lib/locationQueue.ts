...
export async function flushQueuedLocations(): Promise<FlushResult> {
  ...
  for (const entry of all) {
    ...
    try {
      // Send the original capture time (not "now") so a delayed flush of an
      // old entry can never look newer than a fresher write that already
      // landed while this was queued — enforced server-side by
      // locations_monotonic_write_guard_trg, which silently no-ops a stale
      // UPDATE rather than erroring (so this still counts as `success` here,
      // matching "never lose the newest valid location").
      const { error } = await supabase
        .from("locations")
        .upsert(
          { user_id: entry.user_id, latitude: entry.latitude, longitude: entry.longitude, captured_at: new Date(entry.captured_at).toISOString() },
          { onConflict: "user_id" },
        );
