...
const COMBINED_RE = new RegExp(
  [
    ...
    // Privilege changes (groups 9-12). Needed to recognise a deliberately
    // service-role-only table: RLS on, zero policies, and every client role
    // revoked. Without this the check could not tell that from a forgotten policy.
    `revoke\\s+all\\s+(?:privileges\\s+)?on\\s+(?:table\\s+)?${TABLE_TOKEN}\\s+from\\s+([^;]+)`,
    `grant\\s+[^;]*?\\s+on\\s+(?:table\\s+)?${TABLE_TOKEN}\\s+to\\s+([^;]+)`,
  ].join("|"),
  "gi",
);
...
function roleList(raw) {
  return raw.split(",").map((r) => r.trim().replace(/"/g, "").toLowerCase()).filter(Boolean);
}
...
function extractEvents(sql) {
  ...
  while ((m = COMBINED_RE.exec(sql)) !== null) {
    ...
    else if (m[9] && m[10]) events.push({ kind: "revoke_all", table: normTable(m[9]), roles: roleList(m[10]), pos });
    else if (m[11] && m[12]) events.push({ kind: "grant", table: normTable(m[11]), roles: roleList(m[12]), pos });
  }
  return events;
}
...
// table -> Set of client roles (anon/authenticated) that currently hold NO privileges
// because of an explicit REVOKE ALL (cleared again by any later GRANT to that role).
const revokedClientRoles = new Map();
function revokedFor(table) {
  let s = revokedClientRoles.get(table);
  if (!s) { s = new Set(); revokedClientRoles.set(table, s); }
  return s;
}
  ...
  for (const ev of events) {
    switch (ev.kind) {
      ...
      case "drop_policy":
        ...
        break;
      case "revoke_all":
        for (const r of ev.roles) if (CLIENT_ROLES.includes(r)) revokedFor(ev.table).add(r);
        if (ev.roles.includes("public")) CLIENT_ROLES.forEach((r) => revokedFor(ev.table).add(r));
        break;
      case "grant":
        for (const r of ev.roles) {
          if (r === "public") CLIENT_ROLES.forEach((c) => revokedFor(ev.table).delete(c));
          else revokedFor(ev.table).delete(r);
        }
        break;
    }
  }
...
for (const table of liveTables) {
  ...
  if (active.size === 0) {
    // RLS on + no policies denies every client by default. That is only an
    // intentional "service-role-only" table if the migrations ALSO explicitly
    // revoked all privileges from BOTH client roles and never granted them
    // back; otherwise it is a forgotten policy and still fails.
    if (CLIENT_ROLES.every((r) => revokedFor(table).has(r))) {
      serviceOnly.push(table);
      continue;
    }
...
if (serviceOnly.length) {
  console.log(`Service-role-only tables (RLS on, no policies, anon+authenticated explicitly revoked): ${serviceOnly.join(", ")}`);
}
...
console.log(
  ...
  "  at least one currently-active policy (or is an explicit service-role-only\n" +
  "  table, listed above), and storage.objects has policies.\n" +
