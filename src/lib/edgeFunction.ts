...
export class EdgeFunctionError extends Error {
  ...
  constructor(message: string, cause: "network" | "timeout" | "http" | "unknown", requestId: string, status?: number, debug?: string[]) {
    super(message);
...
async function parseFunctionErrorBody(error: unknown): Promise<{ message: string | null; debug?: string[] }> {
  ...
  try {
    ...
    try {
      ...
      return { message, debug: Array.isArray(parsed.debug) ? parsed.debug : undefined };
    } catch {
...
export async function invokeEdgeFunction<T = unknown>(
  ...
  const attempt = async (attemptNum: number): Promise<T> => {
    ...
    try {
      ...
      if (error) {
        const { message: detail, debug } = await parseFunctionErrorBody(error);
