...
import { Pencil, X, Paperclip, BellOff, Send, Check, Mic, Trash2, Zap, AlertTriangle } from "lucide-react";
...
interface MessageComposerProps {
  ...
  /** Image paste handler: called when the user pastes an image from the
   *  clipboard (Ctrl/Cmd+V or mobile long-press paste with an image on the
   *  clipboard). Chat.tsx owns the actual upload logic; this just threads
   *  the File through. Optional so existing callers need no change. */
...
const MessageComposer = ({
  ...
  showGridMenu, setShowGridMenu, setShowSchedulePicker, attachActions, onPasteImage,
}: MessageComposerProps) => {
  const [isInputFocused, setIsInputFocused] = useState(false);

  // Clipboard image paste — fires when the user pastes while the textarea
  // is focused and the clipboard contains an image (e.g. screenshot, image
  // copied from browser, or mobile "Copy image" context menu).
  // Does NOT interfere with normal text paste — only acts when at least one
  // clipboard item is an image type and onPasteImage is wired up.
  const handlePaste = (e: React.ClipboardEvent<HTMLTextAreaElement>) => {
    if (!onPasteImage) return;
    const items = Array.from(e.clipboardData?.items ?? []);
    const imageItem = items.find((item) => item.type.startsWith("image/"));
    if (!imageItem) return; // let normal text paste through unchanged
    e.preventDefault(); // don't also paste the image as text
  ...
  const isUrgentDraft = /^\/(?:urgent|emergency)(\s|$)/i.test(message);
  const isImportantDraft = !isUrgentDraft && /^\/important(\s|$)/i.test(message);

  // Slash-command menu: typing "/" (before any space) lists the available
  // send modes, filtered as you type. Tapping one inserts "/mode " and
  // keeps focus so the person just continues typing their message.
  const SLASH_COMMANDS: { cmd: string; label: string; hint: string; Icon: LucideIcon; tone: string }[] = [
    { cmd: "silent", label: "/silent", hint: "Send without notifying", Icon: BellOff, tone: "text-muted-foreground" },
    { cmd: "important", label: "/important", hint: "Notifies even on Do Not Disturb", Icon: Zap, tone: "text-amber-500" },
    { cmd: "urgent", label: "/urgent", hint: "Strongest alert, marked Urgent", Icon: AlertTriangle, tone: "text-destructive" },
  ];
  const slashQuery = !editingMsg && /^\/[a-z]*$/i.test(message) ? message.slice(1).toLowerCase() : null;
          ...
          <div className="relative flex-1 min-w-0">
            {createPortal(<AnimatePresence>
              {slashMatches.length > 0 && (() => {
                // Portaled + fixed: the composer sits inside
                // DuoSpaceBottomSurface's overflow-hidden glass shell, which
                // would clip anything positioned above the input.
                const r = inputRef.current?.getBoundingClientRect();
                const pos = r
                  ? { left: Math.max(12, r.left - 8), bottom: Math.max(12, window.innerHeight - r.top + 14) }
                  : { left: 16, bottom: 140 };
                return (
                <motion.div
                  ...
                  role="listbox" aria-label="Message modes"
                  className="z-[70] w-64 max-w-[calc(100vw-2rem)] bg-card/95 backdrop-blur-xl border border-border rounded-2xl shadow-xl p-1.5 origin-bottom-left"
                >
                  <p className="px-2.5 pt-1 pb-1.5 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Send as</p>
                  {slashMatches.map((c, i) => (
                    <button key={c.cmd} type="button" role="option" aria-selected={i === slashIndex}
                      // onMouseDown (not onClick) so the textarea never blurs
                      // and the mobile keyboard stays open.
                      onMouseDown={e => { e.preventDefault(); pickSlash(c.cmd); }}
                      onTouchEnd={e => { e.preventDefault(); pickSlash(c.cmd); }}
                      className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-xl text-left transition-colors ${i === slashIndex ? "bg-muted" : "active:bg-muted"}`}>
                      <span className={`h-7 w-7 rounded-full bg-muted/70 flex items-center justify-center shrink-0 ${c.tone}`}>
            ...
            <textarea
              ...
              onKeyDown={e => {
                if (slashMatches.length > 0) {
                  ...
                  if (e.key === "Enter" || e.key === "Tab") { e.preventDefault(); pickSlash(slashMatches[Math.min(slashIndex, slashMatches.length - 1)].cmd); return; }
            ...
            {isImportantDraft && (
              <span className="absolute -top-1 right-0 text-[10px] text-amber-600 dark:text-amber-400 shrink-0 flex items-center gap-0.5 bg-amber-500/10 border border-amber-500/25 rounded-full px-1.5 py-0.5 backdrop-blur-sm">
                <Zap className="h-3 w-3" aria-hidden="true" /> important
              </span>
            )}
            {isUrgentDraft && (
              <span className="absolute -top-1 right-0 text-[10px] text-destructive shrink-0 flex items-center gap-0.5 bg-destructive/10 border border-destructive/20 rounded-full px-1.5 py-0.5 backdrop-blur-sm">
