 ...
 *
 * TOP-UP MODE: once something's already enrolled, this same wizard doubles
 * as an "improve recognition" flow — at the end, the person chooses
 * Replace (discard the old samples, save only this run — saveOwnerProfile's
 * original behavior) or Add (merge this run into the existing profile via
 * addOwnerSamples, FIFO-bounded so it can't grow unbounded). Add is the
 * recommended path for topping up coverage a first session didn't capture
 * — glasses on/off, a beard grown in, different lighting — without
 * throwing away everything already learned.
 */
...
import {
  detectFaces, saveOwnerProfile, addOwnerSamples, clearOwnerProfile, loadOwnerProfile, cosineSim,
...
const FaceEnrollmentDialog = ({ open, onClose, onEnrolled }: Props) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const lease = await acquireCamera("user", "FACE_ENROLLMENT", { userInitiated: true });
  ...
  const save = useCallback(async (merge: boolean) => {
    ...
    setIsSaving(true);
    try {
      const payload = samples.map((s) => ({ embedding: s.embedding, pose: s.pose }));
      if (merge) {
        await addOwnerSamples(payload);
        hapticMedium();
        toast({ title: "Recognition improved", description: `${samples.length} more samples added to your profile` });
      } else {
        await saveOwnerProfile(payload);
        hapticMedium();
      ...
      <DialogContent className="max-w-sm p-0 overflow-hidden">
        <DialogHeader className="p-5 pb-2">
          <DialogTitle className="flex items-center gap-2 text-base">
            <Camera className="h-4 w-4" /> {existingCount > 0 ? "Improve recognition" : "Enroll your face"}
          </DialogTitle>
          <DialogDescription className="text-[11px]">
            {existingCount > 0
              ? `A guided ~30-40s scan — center, left, right, up, down. You already have ${existingCount} samples saved; at the end you can add this new set on top (recommended — covers new lighting, glasses, etc.) or replace everything. Photos never leave your device.`
              : "A guided ~30-40s scan — center, left, right, up, down — so Peek Guard recognises you from any angle you actually hold your phone at. Photos never leave your device."}
          </DialogDescription>
        </DialogHeader>
          ...
          {existingCount > 0 ? (
            <>
              <Button
                ...
                title="Discard your existing samples and save only this new set"
              >
                Replace
              </Button>
              <Button
                ...
                title="Keep your existing samples and add this new set on top — recommended"
              >
                {isSaving
