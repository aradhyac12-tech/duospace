...
import { acquireCamera, explainGumError, isCameraPaused, type CameraLease } from "@/lib/cameraBus";
import { detectMoodFrame, getMoodLandmarker, type BlendshapeScores } from "@/lib/faceRecognition";
...
import {
  ...
  scoreMoods, scoreMoodsFromBlendshapes, softmaxScores, sampleLuma, weightedAvg,
  weightedAvgBlendshapes, filterBlinks, filterBlinksBlendshapes, topMoodWithMargin,
  recordMoodFeedback, getLocalDistrust, moodFrameIssue, MOOD_MIN_LUMA, sessionBaseline,
  blendshapeBaseline,
} from "@/lib/moodScoring";
import { useBackgroundMoodDetection } from "@/hooks/useBackgroundMoodDetection";
import { canPersistDerivedMood } from "@/lib/privacy/derivedMoodGate";
...
const MoodDetector = () => {
  ...
  // Consent check for saving a camera-derived read to the account. Tells the
  // person, once per session, why nothing was saved instead of failing silently.
  const savedDeniedToastRef = useRef(false);
  const canSaveDerivedMood = useCallback(async (userId: string): Promise<boolean> => {
    const gate = await canPersistDerivedMood(userId);
    if (!gate.allowed && !savedDeniedToastRef.current) {
      savedDeniedToastRef.current = true;
      toast({ title: "Mood not saved", description: "Turn on “Save mood readings” under Settings › Privacy & AI to keep these." });
    }
    return gate.allowed;
  }, [toast]);
  ...
  // Parallel pool of trained-model blendshape reads, collected from the SAME
  // frames as samplesRef whenever the blendshape sub-graph populated (see
  // detectMoodFrame in lib/faceRecognition.ts) — this is the primary,
  // higher-accuracy scoring path; samplesRef's geometry pool is now only the
  // fallback for when this comes up short (see analyzeMood below).
  const blendshapeSamplesRef = useRef<BlendshapeScores[]>([]);
  ...
  const startDetection = useCallback(async () => {
    ...
    // FIX (mood detection fighting an active call for the camera): unlike
    // usePeekDetection, this component never listened for the camera bus's
    // pause signal at all — a call already asks other camera consumers to
    // yield (see CallContext.tsx / Calls.tsx's pauseCameraConsumers calls),
    // but nothing here was checking for that, so tapping "detect mood"
    // mid-call would either steal the call's camera or fail with a
    // confusing generic camera error. Surface it as the same cameraIssue
    // UI other camera failures already use, with an honest reason instead
    // of a raw getUserMedia error message.
    if (isCameraPaused()) {
      setDetecting(false);
      setCameraIssue({ code: "busy", message: "The camera is busy with an active call. Try again once you hang up." });
      return;
    }

    try {
      const lease = await acquireCamera("user", "MOOD_DETECTION");
      ...
      // blocked, etc.), every later detectMoodFrame() call will also reject —
      // we surface that distinctly rather than blaming it on lighting.
      getMoodLandmarker().catch(() => { modelFailCountRef.current += 999; });

      // Sample real facial landmarks (+ trained blendshapes, see
      // detectMoodFrame) throughout the window instead of reading a single
      // final frame — smooths out blinks, motion blur, and momentary
      // expressions into a stable read.
      sampleTickRef.current = window.setInterval(async () => {
        ...
        try {
          ...
          const landmarker = await getMoodLandmarker();
          const { faces, blendshapes } = detectMoodFrame(landmarker, v, tsRef.current);
          ...
          if (faces.length === 1 && !tooDarkForFace) {
            ...
            if (issue) {
              ...
              // Trained-model signal for the SAME frame, when it populated —
              // see scoreMoodsFromBlendshapes in lib/moodScoring.ts for why
              // this is preferred over the geometry sample above whenever
              // it's available.
              if (blendshapes) blendshapeSamplesRef.current.push(blendshapes);
            }
  ...
  const analyzeMood = useCallback(async () => {
    ...
    if (samples.length < 3) {
      ...
      if (user) {
        // PRIVACY (Phase 1.6): a camera-derived value only reaches the server
        // when MOOD_PROCESSING consent is granted (see derivedMoodGate.ts).
        if (await canSaveDerivedMood(user.id)) {
          const { error: logErr } = await supabase.from("mood_logs").insert({
            ...
            features: { source: "camera_detected", sample_count: samples.length, face_presence_ratio: faceRatio, reason: issue ?? "insufficient_face_samples" },
    ...
    // Prefer the trained blendshape model whenever this session collected
    // enough usable reads from it — see scoreMoodsFromBlendshapes' doc
    // comment in lib/moodScoring.ts for why it's the more accurate path.
    // Geometry (extractExpression + sessionBaseline) is the fallback for
    // when blendshapes came up short this session (older device, model
    // slow to warm up, etc.), not a second vote — using both at once would
    // just reintroduce the geometry pipeline's own biases into a blended
    // score instead of actually replacing them.
    const blendshapePool = filterBlinksBlendshapes(blendshapeSamplesRef.current);
    let evidence: Record<MoodLabel, number>;
    // How many geometry samples the blink filter dropped (0 on the blendshape
    // path). Hoisted: it was read below as `pool.length` outside the block that
    // declares `pool` → ReferenceError on every mood save.
    let blinkFilteredSamples = 0;
    if (blendshapePool.length >= 3) {
      const avgBlendshapes = weightedAvgBlendshapes(blendshapePool);
      // See blendshapeBaseline's doc comment — corrects for camera-angle /
      // per-face bias the same way sessionBaseline does for the geometry
      // path below (this is the path that was actually reproducing "always
      // Surprised", since it's preferred whenever available).
      evidence = scoreMoodsFromBlendshapes(avgBlendshapes, distrust, blendshapeBaseline(blendshapePool));
    } else {
      // Reject blink frames before averaging, then take a recency-weighted
      // average — expressions typically settle over the 5s window (the
      // first ~1s often still carries "surprised by the camera turning
      // on"), so later samples count more toward the final read. Shared
      // with the background detector — see lib/moodScoring.ts.
      const pool = filterBlinks(samples);
      blinkFilteredSamples = samples.length - pool.length;
      ...
      // See sessionBaseline's doc comment — normalizes for this face's own
      // resting brow/mouth-curve/eye-openness instead of fixed universal
      // constants (the bug behind mood detection always reading "Surprised").
      const baseline = sessionBaseline(pool);
      evidence = scoreMoods({ mouthCurve, mouthOpen, browRaise, eyeOpenness }, distrust, baseline);
    }
    ...
    // Margin gate: a softmax "winner" can still be a near-tie (see
    // topMoodWithMargin's doc comment) — when the top two moods are close,
    // that's genuine ambiguity in the expression, not a confident read, so
    // fall back to Neutral rather than reporting a coin-flip mood.
    const ranked = topMoodWithMargin(smoothed);
    const mood = ranked.ambiguous ? "Neutral" : ranked.mood;
    const topProb = ranked.ambiguous ? smoothed.Neutral : ranked.topProb;
    ...
    // actually had a usable face in frame. An ambiguous top-two margin
    // caps confidence outright, even after falling back to Neutral, so the
    // UI doesn't show high confidence for a read that was actually a toss-up.
    const magnitudeConf = mood === "Neutral" ? Math.max(0.4, topProb) : Math.min(0.97, 0.4 + topProb * 0.6);
    const marginCap = ranked.ambiguous ? 0.55 : 1;
    const confidence = Math.max(0.3, Math.min(0.97, magnitudeConf * Math.min(1, faceRatio + 0.3) * marginCap));
    ...
    if (user) {
      ...
      // PRIVACY (Phase 1.6): the camera-derived read only reaches the server
      // when MOOD_PROCESSING consent is granted. Without it the person still
      // sees the result on this screen — nothing is stored.
      if (await canSaveDerivedMood(user.id)) {
        const { data: logData, error: logErr } = await supabase.from("mood_logs").insert({
          ...
          valence: va.valence,
          arousal: va.arousal,
          // KI-04: raw facial measurements (mouth curve, brow raise, eye
          // openness…) are deliberately NOT stored — mood_logs is readable by
          // the partner and those values add nothing the UI reads. Only the
          // derived label/probabilities and capture-quality numbers are kept.
          // `source` is provenance: this value was inferred by an on-device
          // model, not reported by the person.
          features: {
            source: "camera_detected",
            sample_count: samples.length, face_presence_ratio: faceRatio,
            distrust_applied: distrust[mood] ?? 1,
            blink_filtered_samples: blinkFilteredSamples,
            mood_probabilities: probs,
            top_margin: ranked.margin,
            margin_ambiguous: ranked.ambiguous,
          },
        }).select("id").single();
        if (logErr) console.error("mood_logs insert failed:", logErr.message);
      ...
      // PRIVACY (Phase 1.6): NO write to the partner-visible profile here.
      // This used to publish "Feeling <mood>" to profiles.mood_text the
      // instant the camera read finished — presenting an ML guess to the
      // partner as the person's own statement, before they'd even seen it.
      // The profile is now only updated when the person confirms the read
      // (giveFeedback(true)) or picks a mood themselves (selectManualMood).
    }
  }, [user, toast, canSaveDerivedMood]);
  ...
  const giveFeedback = async (accurate: boolean) => {
    // Not gated on lastLogId any more: when saving to the account isn't
    // permitted (no MOOD_PROCESSING consent) there is no server row, but
    // the on-device calibration below should still work.
    ...
    if (accurate) {
      setMoodSource("confirmed"); setConfidencePct(100);
      // The person has now seen and confirmed this mood, so it stops being
      // an unreviewed model guess and becomes their own statement — the
      // first point at which it may appear on the partner-visible profile.
      if (user) {
        const moodItem = moods.find(m => m.label === detectedMood);
        await supabase.from("profiles").update({
          mood_emoji: moodItem?.emoji || "😐",
          mood_text: `Feeling ${detectedMood.toLowerCase()}`,
          mood_updated_at: new Date().toISOString(),
        }).eq("user_id", user.id);
      }
    }
