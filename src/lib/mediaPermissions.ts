...
// "Connecting…", immediately followed by the call engine's call.join() opening the
...
// entirely once it's set — the call engine's own open becomes the only one, same
...
// invalidateNativeMicGrantCache() below clears it the moment the call engine's own
...
/**
 * Call this the moment the call engine join (or any other post-probe use of the
 ...
 * permission check, and the call engine's own call.join() opens the mic AGAIN a
 ...
 * The call engine's own open be the only one. Native has no equivalent API, so it
