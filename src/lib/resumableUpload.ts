...
import { logError, logInfo, logWarn, getErrorMessage } from "@/lib/telemetry";
...
export async function resumableUpload(opts: ResumableUploadOptions): Promise<ResumableUploadResult> {
  ...
  // SPEED FIX: upload up to 4 chunks concurrently instead of strictly one
  // after another — on mobile networks per-request latency, not bandwidth,
  // dominated large-file send time.
  const CONCURRENCY = 4;
  let nextIndex = 0;
  const worker = async () => {
    while (true) {
