...
const PeekGuard = () => {
  ...
  const peekConfig = useMemo(() => ({
    ...
    // Off by default (matches usePeekDetection's DEFAULTS — see its doc
    // comment): "no face at all" turned out to be the biggest source of
    // false-alarm locks (phone face-down, propped up while stepping away
    // briefly, a hand over the lens), not a reliable "someone else is
    // trying to look" signal. The fallback here must keep agreeing with
    // the hook's own default so unset Settings always mean the same thing
    // in both places.
  ...
  // PIN fallback: offered when native biometric verification actually fails
  // (wrong finger, cancelled) rather than leaving the lock stuck with no
  // recourse besides re-doing camera-based owner verification. Reuses the
  // same PIN as the app lock screen (STORED_PIN_KEY) — only offered when
  // one is actually set.
  ...
  const verifyOwnerAndDismiss = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        // Face-ID-style "Require Attention": the embedding matches, but a
        // matching face is trivially satisfied by, say, a stranger holding
        // a photo/screen of the owner up to the lens. Requiring eyes open +
        // roughly facing the camera in this same frame closes that gap the
        // same way Face ID's own attention check does. Opt-out via Peek
        // Guard settings for anyone who finds it too strict in practice
        // (low light, glasses glare, etc).
        ...
        dismissPeek();
        setDismissed(true);
        // Continuous template adaptation (Face ID does this quietly after
        // every successful unlock) — fire-and-forget, never blocks the
        // dismiss above, and is itself conservative about what it accepts
        // (see maybeAdaptOwnerProfile's own gating).
  ...
  }, [lastEventId, dismissPeek, toast, appSettings.peekRequireAttention]);
  ...
  const tryBiometric = useCallback(async () => {
    ...
    try {
      ...
      // Biometric genuinely failed or was cancelled. Previously this just
      // left the lock stuck up with camera-based owner verification as the
      // only way out. Offer PIN instead — same credential as the app lock
      // screen — when one is actually set, so a failed fingerprint doesn't
      // strand a legitimate owner with the phone in low light or gloves on.
      hapticError();
  ...
  // PIN unlock: verified against the same hashed PIN the app lock screen
  // uses (STORED_PIN_KEY). Only reachable after a real biometric failure —
  // see tryBiometric's catch block above.
  const submitPinDigit = useCallback((digit: string) => {
    setPinValue((prev) => {
      ...
      if (next.length === PIN_LENGTH) {
        ...
        if (!stored) return next; // PIN got cleared elsewhere mid-entry — nothing to check against
        setPinBusy(true);
  ...
  // Someone-else-detected reasons only — biometric proves "this is the
  // owner", which is the right check when a stranger/crowd/spoof was seen,
  // but not when the reason is simply "no face" (owner not there at all,
  // opt-in only — see usePeekDetection DEFAULTS). Nobody impersonating
  // anyone was detected in that case, so requiring a fingerprint is just
  // friction; camera-based owner verification below already covers it.
    ...
    <>
      ...
      {showAlert && (
        <motion.div
          ...
          aria-modal="true" role="alertdialog" aria-label={dismissed ? "Was that accurate?" : showPinFallback ? "Enter PIN to unlock" : `Privacy lock. ${reasonText}`}
        >
          {!dismissed && showPinFallback ? (
          <motion.div
          ...
          >
            ...
            <div className="space-y-2 pt-1">
              {/* Biometric ("fingerprint") is only offered when someone else's
                  face is what triggered the lock — that's the scenario it
                  actually verifies against. For a "no face" lock (owner just
                  not there — opt-in only), there's no impersonation to rule
                  out, so this reduces to the plain owner-verify button below. */}
              {strangerReason && (
                <button
              ...
              >
                <Lock className={strangerReason ? "h-3 w-3" : "h-3.5 w-3.5"} /> {authBusy ? "Verifying…" : "Dismiss (owner verify)"}
              </button>
            </div>
          </motion.div>
