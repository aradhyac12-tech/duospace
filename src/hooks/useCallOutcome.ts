...
interface UseCallOutcomeArgs {
   ...
   *  leaveCall() itself. */
  onRemoteEnded: () => void;
  /** PHASE 4 (self-hosted): the signaling bridge. When provided AND the
   *  provider is self_hosted, the caller learns of decline / busy / ring
   *  timeout DIRECTLY over the WebSocket layer (CALL_REJECTED / CALL_BUSY /
   *  CALL_TIMEOUT) instead of waiting for a call_history change to come back
   *  through Realtime. The Realtime subscription below stays as the
   *  RECOVERY path (socket down / frame lost): outcomes are idempotent, so
   *  whichever signal arrives first wins and the other is a no-op. */
  signalingBridge?: { onMessage: (handler: (m: SignalingMessage) => void) => () => void };
 ...
 * it does not touch the call engine lifecycle, push/VoIP, CallKit, or FCM code —
 * decline_call/cancel_call/claim_call and their triggers are unmodified.
 */
export function useCallOutcome({ currentCallId, everConnected, onRemoteEnded, signalingBridge, activeProvider }: UseCallOutcomeArgs) {
  ...
  // PHASE 4: WebSocket-driven outcomes for the CALLER of a self-hosted call.
  // The signaling client has already dropped events for other calls, old
  // sessions, wrong senders and illegal states; only the current call's
  // pre-connect terminal events reach here. `handledRef` makes the outcome
  // exactly-once per call across this path and the Realtime recovery path.
  const handledCallRef = useRef<string | null>(null);
  useEffect(() => {
    if (!currentCallId || activeProvider !== "self_hosted" || !signalingBridge) return;
    return signalingBridge.onMessage((m) => {
      if (m.callId !== currentCallId || everConnectedRef.current) return;
      if (handledCallRef.current === currentCallId) return;
      if (m.type === "CALL_REJECTED") setOutcome({ type: "declined" });
      else if (m.type === "CALL_TIMEOUT") setOutcome({ type: "no-answer" });
      else if (m.type === "CALL_BUSY") setOutcome({ type: "failed", message: "They're on another call right now." });
      else return;
      handledCallRef.current = currentCallId;
      onRemoteEndedRef.current();
    });
  }, [currentCallId, activeProvider, signalingBridge]);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", {
        ...
        // Recovery path: skip if the WebSocket already delivered this outcome.
        if (handledCallRef.current === currentCallId) return;
        ...
        if (row.status === "missed" || row.status === "cancelled") handledCallRef.current = currentCallId;
      })
      .subscribe();
