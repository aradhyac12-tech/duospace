...
function curateResults(results: MusicResult[], limit = 20): MusicResult[] {
  ...
  // Keep quality filtering, but sample from the good pool randomly rather
  // than always returning the identical top-N in the identical order.
  const good = Array.from(best.values()).filter(x => x.score >= 0).map(x => x.result);
  const pool = good.length >= limit ? good : Array.from(best.values()).sort((a, b) => b.score - a.score).map(x => x.result);
  return shuffleArr(pool.slice(0, Math.max(limit * 2, 20))).slice(0, limit);
}
// --- end shared curation ---
...
// VARIETY FIX: was a fixed query + order=viewCount (all-time most viewed)
// per language, so trending never changed. Now: a random query from a pool,
// restricted to videos published in the last 60 days, ordered by relevance
// or date at random — fresh, actually-recent results each load.
type TrendingMode = string;

const SEARCH_BUCKET_CONFIG: Record<string, { queries: string[]; relevanceLanguage?: string; region: string }> = {
  hindi:     { queries: ["new hindi songs", "bollywood new song", "latest hindi song", "hindi romantic song"], relevanceLanguage: "hi", region: "IN" },
  punjabi:   { queries: ["new punjabi song", "latest punjabi songs", "punjabi hit song"], relevanceLanguage: "pa", region: "IN" },
  urdu:      { queries: ["new urdu song", "pakistani new song", "coke studio"], relevanceLanguage: "ur", region: "PK" },
  tamil:     { queries: ["new tamil song", "tamil latest songs"], relevanceLanguage: "ta", region: "IN" },
  telugu:    { queries: ["new telugu song", "telugu latest songs"], relevanceLanguage: "te", region: "IN" },
  bengali:   { queries: ["new bengali song", "bangla new song"], relevanceLanguage: "bn", region: "IN" },
  marathi:   { queries: ["new marathi songs", "marathi latest song"], relevanceLanguage: "mr", region: "IN" },
  gujarati:  { queries: ["new gujarati song", "gujarati latest songs"], relevanceLanguage: "gu", region: "IN" },
  kannada:   { queries: ["new kannada song", "kannada latest songs"], relevanceLanguage: "kn", region: "IN" },
  malayalam: { queries: ["new malayalam song", "malayalam latest songs"], relevanceLanguage: "ml", region: "IN" },
  haryanvi:  { queries: ["haryanvi songs new", "new haryanvi song"], region: "IN" },
  bhojpuri:  { queries: ["new bhojpuri song", "bhojpuri latest songs"], region: "IN" },
  arabic:    { queries: ["new arabic song", "arabic hits"], relevanceLanguage: "ar", region: "AE" },
  spanish:   { queries: ["nueva cancion", "reggaeton nuevo", "musica latina nueva"], relevanceLanguage: "es", region: "MX" },
  korean:    { queries: ["kpop new song", "kpop mv"], relevanceLanguage: "ko", region: "KR" },
};
const ALLOWED_LANGS = new Set(["english", ...Object.keys(SEARCH_BUCKET_CONFIG)]);
const pickOne = <T,>(xs: T[]) => xs[Math.floor(Math.random() * xs.length)];
const shuffleArr = <T,>(xs: T[]) => { const a = [...xs]; for (let i = a.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [a[i], a[j]] = [a[j], a[i]]; } return a; };
...
async function fetchYouTubeBucket(mode: TrendingMode): Promise<{ results: MusicResult[]; debug: string }> {
  ...
  if (mode === "english") {
    const url = `https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails&chart=mostPopular&videoCategoryId=10&regionCode=${pickOne(["US", "GB", "CA", "AU", "IN"])}&maxResults=50&key=${YT_KEY}`;
  ...
  const cfg = SEARCH_BUCKET_CONFIG[mode];
  if (!cfg) return { results: [], debug: `youtube(${mode}): unknown language` };
  const q = encodeURIComponent(pickOne(cfg.queries));
  const langParam = cfg.relevanceLanguage ? `&relevanceLanguage=${cfg.relevanceLanguage}` : "";
  const publishedAfter = new Date(Date.now() - 60 * 86400_000).toISOString();
  const order = pickOne(["relevance", "relevance", "date", "viewCount"]);
  const searchUrl = `https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&videoCategoryId=10${langParam}&regionCode=${cfg.region}&order=${order}&publishedAfter=${encodeURIComponent(publishedAfter)}&maxResults=30&q=${q}&key=${YT_KEY}`;
...
async function fetchMirrorBucket(mode: TrendingMode): Promise<{ results: MusicResult[]; debug: string }> {
  const region = mode === "english" ? "US" : (SEARCH_BUCKET_CONFIG[mode]?.region ?? "IN");
...
serve(async (req) => {
  ...
  try {
    // Client asks for specific languages ({ languages: [...] }); legacy
    // callers with an empty body get the original four.
    let requested: string[] = ["english", "hindi", "marathi", "haryanvi"];
    try {
      const body = await req.json();
      if (Array.isArray(body?.languages)) requested = body.languages.filter((l: unknown) => typeof l === "string" && ALLOWED_LANGS.has(l)).slice(0, 4);
    } catch { /* no body */ }
    const buckets = await Promise.all(requested.map((l) => fetchBucket(l)));
    const out: Record<string, unknown> = {};
    const debug: Record<string, string[]> = {};
    requested.forEach((l, i) => { out[l] = buckets[i].results; debug[l] = buckets[i].debug; });
    return new Response(JSON.stringify({ ...out, debug }), { headers: jsonHeaders });
  } catch (err: unknown) {
