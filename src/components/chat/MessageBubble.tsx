...
import {
  Reply, Pin, Pencil, ImageIcon, FileText, Ghost, Download, Loader2, Zap, AlertTriangle, BellOff,
} from "lucide-react";
...
const MessageBubble = ({
  msg, isMine, isDisappearing, isVanishing, isHighlighted, isActiveResult,
  repliedMsg, partnerName, userId,
  isFirstInGroup, isLastInGroup, isSenderChange, partnerAvatar,
  onReply, onLongPress, onPhotoView, onLetterOpen, formatTime, onRetry,
  allReactions, mediaVisible, isReactingTo, onReactionPickerClose,
  skipEnterAnimation,
}: {
  msg: DecryptedMessage; isMine: boolean; isDisappearing: boolean; isVanishing?: boolean;
  ...
  onPhotoView: (url: string, id: string) => void;
  /** Opens the full envelope-opening LetterReader over the decrypted letter
   *  content; optional so any other caller of this component that doesn't
   *  wire it up just keeps the sealed preview inert. */
  onLetterOpen?: (content: string) => void;
  ...
  /** VIRTUALIZATION FIX: only meaningful when MessageTimeline's virtualized
   *  path (long conversations) mounts/unmounts bubbles as they scroll in
   *  and out of the render window — every one of those re-mounts would
   *  otherwise replay this bubble's own initial-mount fade/slide-in, which
   *  reads as messages continuously "arriving" again on every scroll
   *  (exactly the kind of flutter this is meant to fix, not cause).
   *  MessageTimeline tracks which ids have already played this once and
   *  passes true from then on so a re-mount renders in its final state
   *  immediately. Defaults to false so every other caller (and the
   *  non-virtualized path) animates in exactly as before. */
  ...
  return (
    <motion.div id={`msg-${msg.id}`}
      layout="position"
      initial={skipEnterAnimation ? false : { opacity: 0, y: 4, scale: 1 }} animate={{ opacity: isDisappearing ? 0.75 : 1, y: 0, scale: 1 }}
      exit={isDisappearing ? {
      ...
      <motion.div
        className="flex items-end gap-1.5 max-w-[80%] min-w-0 relative"
        style={{ x: dragX }}
        drag="x"
      ...
      >
        ...
        {!isMine && (
          isFirstInGroup ? (
            <div className="h-6 w-6 rounded-full overflow-hidden shrink-0 mb-0.5 bg-muted flex items-center justify-center text-[10px] font-medium">
              {partnerAvatar ? <img loading="lazy" decoding="async" src={partnerAvatar} alt="" className="h-full w-full object-cover" /> : (partnerName?.[0]?.toUpperCase() || "?")}
            </div>
        ...
        {/* FIX (Vanish Mode text not visible): the partner bubble had no
            explicit text-color class of its own — its text was relying on
            being inherited from an ancestor. `color` is an inherited CSS
            property, but inheritance passes down the already-computed
            pixel value, not the `var(--foreground)` reference itself — so
            an ancestor far above this bubble (outside the `.vanish-mode`
            class, which only wraps Chat's own root div) had already
            resolved that color using the REGULAR theme, and every
            descendant without its own color class just kept that value
            regardless of `.vanish-mode` being active closer to here. The
            bubble's own background (`--surface-2`) DID re-resolve
            correctly, because it's set directly on this element — the fix
            is giving text the same treatment: an explicit `text-foreground`
            declared here re-reads `var(--foreground)` at this element's
            position in the tree, picking up Vanish Mode's near-white value
            instead of the stale inherited one. Sent bubbles never had this
            problem since they already set `text-primary-foreground`
            explicitly right alongside `bg-primary`. */}
        <div className={`relative rounded-2xl px-3 py-2 select-none min-w-0 overflow-hidden ${
          isMine
            ? `bg-primary text-primary-foreground ${isLastInGroup ? "rounded-br-md" : "rounded-br-2xl"}`
            : `bg-[hsl(var(--surface-2))] text-foreground ${isLastInGroup ? "rounded-bl-md" : "rounded-bl-2xl"}`
        } ${(isDisappearing||isVanishing) ? "ring-1 ring-primary/20" : ""} ${isFailed ? "ring-1 ring-destructive/50" : ""} ${!isFailed && msg.urgent ? "ring-2 ring-destructive/70" : !isFailed && msg.important ? "ring-2 ring-amber-500/60" : ""}`}
        ...
        >
          ...
          {/* FIX: /important and /urgent used to look exactly like normal
              text — the flag only changed the push notification. Both
              partners now see a clear label on the bubble itself. */}
          {msg.urgent ? (
            <div className={`flex items-center gap-1 mb-1 -mx-0.5 px-1.5 py-0.5 rounded-full w-fit text-[10px] font-semibold uppercase tracking-wide ${isMine ? "bg-primary-foreground/20 text-primary-foreground" : "bg-destructive/12 text-destructive"}`}>
              <AlertTriangle className="h-3 w-3" aria-hidden="true" /> Urgent
            </div>
          ) : msg.important ? (
            <div className={`flex items-center gap-1 mb-1 -mx-0.5 px-1.5 py-0.5 rounded-full w-fit text-[10px] font-semibold uppercase tracking-wide ${isMine ? "bg-primary-foreground/20 text-primary-foreground" : "bg-amber-500/15 text-amber-600 dark:text-amber-400"}`}>
              <Zap className="h-3 w-3" aria-hidden="true" /> Important
            </div>
          ) : msg.silent && isMine ? (
            <div className="flex items-center gap-1 mb-1 opacity-60 text-[9px]">
              <BellOff className="h-2.5 w-2.5" aria-hidden="true" /> Sent silently
            </div>
          ) : null}
          {/* Voice */}
          ...
          {/* Love letter — a sealed envelope preview; tapping it opens the
              full open/close envelope sequence (LetterReader), lifted to
              Chat.tsx the same way photos are via onPhotoView. */}
          {msg.message_type==="letter" && msg.decryptedContent && (
            <button
              onClick={(e) => { e.stopPropagation(); onLetterOpen?.(msg.decryptedContent!); }}
              className={`flex items-center gap-2.5 rounded-xl px-3 py-2.5 mb-1 w-full text-left active:scale-[0.98] transition-transform ${isMine?"bg-primary-foreground/15":"bg-primary/5 border border-primary/15"}`}
            >
              <span className="text-xl shrink-0">💌</span>
              <span className={`text-[13px] font-medium truncate ${isMine?"":"text-foreground"}`}>
                {parseLetterContent(msg.decryptedContent).subject}
              </span>
            </button>
          )}
            ...
            mediaVisible!==false ? (
              <div className={`relative -mx-3 mb-1 ${
                ...
                <motion.img layoutId={`photo-${msg.id}`}
                  onClick={() => { onPhotoView(mediaSrc, msg.id); }} src={mediaSrc} alt="shared"
              ...
              <button onClick={() => { onPhotoView(mediaSrc, msg.id); }}
          ...
          {msg.message_type==="file" && msg.file_name && (
            <button type="button"
              disabled={isSending || downloading}
              onClick={async (e) => {
                e.stopPropagation();
                if (isSending || !mediaSrc || downloading) return;
                setDownloading(true);
                const r = await downloadToDevice(mediaSrc, msg.file_name!);
                setDownloading(false);
                toast(r.ok
                  ? { title: "Saved to your device", description: `${msg.file_name} · ${r.where}` }
                  : { title: "Couldn't save file", description: r.error, variant: "destructive" });
              }}
              className={`relative flex items-center gap-2 mb-1 rounded-lg px-2 py-1.5 w-full text-left overflow-hidden ${isMine?"bg-primary-foreground/15":"bg-muted/50"}`}>
              <FileText className="h-3.5 w-3.5 shrink-0 opacity-50" />
              <span className="text-xs truncate flex-1">{msg.file_name}</span>
              {!isSending && (downloading
                ? <Loader2 className="h-3.5 w-3.5 shrink-0 opacity-70 animate-spin" />
          ...
          {(isLastInGroup || isDisappearing || isVanishing || msg.edited_at || msg._sendStatus) && (
            <div className={`flex items-center gap-1 mt-0.5 ${isMine?"justify-end":""}`}>
              ...
              {/* Vanish Mode: no countdown to show (there's no timer) — a
                  small static ghost mark is enough to mark this message as
                  part of the active vanish session, gone the moment it
                  ends. ("vanish_after_seen" = the session ended before it
                  was read; it's kept and vanishes once it has been seen.) */}
              {isVanishing && (
                <Ghost className={`h-2.5 w-2.5 ${isMine ? "text-primary-foreground/70" : "text-muted-foreground/60"}`} aria-label={msg.disappear_at === "vanish_after_seen" ? "Vanishes once seen" : "Vanish message"} />
              )}
...
// PERF: MessageTimeline re-renders on lots of chat state that has nothing to
// do with any individual bubble (composer keystrokes, typing indicator,
// search index, reacting-to state for a *different* message, etc.). Before
// this, every bubble in the conversation re-rendered on every one of those —
// the more messages in a chat, the laggier scrolling/typing got. Wrapping in
// memo() with a comparator that ignores callback-prop identity (those are
// freshly-created closures every render regardless, but they always call
// through to the same current handlers) means a bubble only re-renders when
// its own actual data changed. Visual output is unchanged.
const arePropsEqual = (
  prev: Readonly<Parameters<typeof MessageBubble>[0]>,
  next: Readonly<Parameters<typeof MessageBubble>[0]>,
) =>
  prev.msg === next.msg &&
  prev.isMine === next.isMine &&
  prev.isDisappearing === next.isDisappearing &&
  prev.isVanishing === next.isVanishing &&
  prev.isHighlighted === next.isHighlighted &&
  prev.isActiveResult === next.isActiveResult &&
  prev.repliedMsg === next.repliedMsg &&
  prev.partnerName === next.partnerName &&
  prev.userId === next.userId &&
  prev.isFirstInGroup === next.isFirstInGroup &&
  prev.isLastInGroup === next.isLastInGroup &&
  prev.isSenderChange === next.isSenderChange &&
  prev.partnerAvatar === next.partnerAvatar &&
  prev.allReactions === next.allReactions &&
  prev.mediaVisible === next.mediaVisible &&
  prev.isReactingTo === next.isReactingTo &&
  prev.skipEnterAnimation === next.skipEnterAnimation &&
  // onRetry flips between a function and undefined based on send status —
  // that IS meaningful (a failed bubble needs its retry affordance to show).
