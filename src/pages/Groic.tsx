...
import { Search, Loader2, Plus, Play, Sparkles, Users, ChevronLeft, WifiOff, Download, Trash2, ListMusic, ExternalLink, Check, RefreshCw } from "lucide-react";
...
import { TRENDING_LANGUAGES, getTrendingRail } from "@/lib/music/trendingMix";
...
const LANG_OPTIONS = TRENDING_LANGUAGES;
type TrendingBuckets = Record<string, GroicTrack[]>;
...
// Rail for mixed-provider trending (SoundCloud / YouTube / Audius tracks).
const PROVIDER_BADGE: Record<string, string> = { soundcloud: "SC", youtube: "YT", audius: "AU" };
const TrackRail = ({
  title, items, currentId, onPlay, onEnqueue,
}: {
  title: string;
  items: GroicTrack[];
  currentId: string | undefined;
  onPlay: (t: GroicTrack, pool: GroicTrack[]) => void;
  onEnqueue: (t: GroicTrack) => void;
}) => {
  if (items.length === 0) return null;
  return (
    <section>
      ...
      <div className="flex gap-3 overflow-x-auto pb-1 -mx-1 px-1">
        {items.map((t) => (
          <div key={t.id} className="shrink-0 w-[128px]">
            <div className="relative aspect-square rounded-2xl overflow-hidden bg-muted">
              {(t.artwork || t.thumbnail) && <img src={t.artwork || t.thumbnail} alt={t.title} className="h-full w-full object-cover" loading="lazy" />}
              <span className="absolute top-1.5 right-1.5 px-1.5 h-4 rounded-full bg-background/80 backdrop-blur text-[8px] font-bold flex items-center text-foreground/80">
                {PROVIDER_BADGE[t.provider] ?? ""}
              </span>
              <button onClick={() => onPlay(t, items)} aria-label={`Play ${t.title}`}
                className="absolute bottom-1.5 right-1.5 h-8 w-8 rounded-full bg-primary text-primary-foreground flex items-center justify-center shadow-lg active:scale-90">
                <Play className="h-3.5 w-3.5 ml-0.5" />
              </button>
              {currentId === t.id && (
                <div className="absolute top-1.5 left-1.5 px-1.5 h-4 rounded-full bg-primary/90 text-primary-foreground text-[8px] font-bold flex items-center">NOW</div>
              )}
            </div>
            <div className="mt-1.5 flex items-start justify-between gap-1">
              <div className="min-w-0">
              ...
              <button onClick={() => onEnqueue(t)} className="text-muted-foreground active:scale-90 mt-0.5 shrink-0" aria-label="Add to queue">
                <Plus className="h-3.5 w-3.5" />
              </button>
            </div>
          </div>
...
const Groic = () => {
  ...
  const [langPrefs, setLangPrefs] = useState<string[]>(() => {
    try {
      ...
      return Array.isArray(saved) ? saved.filter((x: string) => LANG_OPTIONS.some(l => l.id === x)) : ["hindi", "english"];
  ...
  const toggleLangPref = useCallback((id: string) => {
    setLangPrefs(prev => {
      ...
      // Tap = select, tap again = deselect (any chip, including the last).
  ...
  const loadTrending = useCallback((force = false) => {
    let cancelled = false;
    if (langPrefs.length === 0) { setTrending({}); setTrendingLoading(false); return () => {}; }
    setTrendingLoading(true);
    setTrendingError(false);
    Promise.all(langPrefs.map(id => getTrendingRail(id, 15, force).then(r => [id, r] as const)))
      .then((pairs) => {
        if (cancelled) return;
        const next: TrendingBuckets = {};
        pairs.forEach(([id, r]) => { next[id] = r; });
        setTrending(next);
        if (pairs.every(([, r]) => r.length === 0)) setTrendingError(true);
      })
      .catch((err) => {
        if (cancelled) return;
  ...
  const addSoundCloudToPlaylist = async (t: GroicTrack) => {
    hapticLight();
    const { error: addError } = await ourPlaylist.addSong({
      title: t.title,
      artist: t.artist,
      // Same synthetic-identifier fallback pattern as addAudiusToPlaylist
      // above — permalink when SoundCloud gave us one (also useful as an
      // "open in SoundCloud" link), otherwise a scheme this file's own
      // playOurPlaylistSong knows how to parse back on playback.
      song_url: t.permalink || `soundcloud:${t.providerTrackId}`,
      platform: "soundcloud",
      thumbnail_url: t.thumbnail,
    });
    if (addError) { capture("DS-GROIC-PLAYLIST-ADD", { component: "Groic", action: "addSong", cause: addError }); toast({ title: "Couldn't add to playlist", description: addError, variant: "destructive" }); }
  ...
  const playOurPlaylistSong = (song: OurPlaylistSong) => {
    ...
    } else if (song.platform === "soundcloud" && song.song_url.startsWith("soundcloud:")) {
      // Only the synthetic soundcloud:<id> scheme this file itself wrote
      // (via addSoundCloudToPlaylist) is playable in-app — a raw pasted
      // soundcloud.com URL (from "Add link") has no resolvable track id
      // without an extra /resolve round trip this pass doesn't add, so it
      // falls through to the same "open externally" behavior as any other
      // pasted link below.
      const providerTrackId = song.song_url.slice("soundcloud:".length);
      playTrack({
  ...
  const [providerFilter, setProviderFilter] = useState<"all" | "youtube" | "audius" | "soundcloud">("all");
  // Section visibility/fetch-gating for each provider — "all" shows every
  // section; any specific filter shows only that one. Named booleans
  // instead of repeating the `!== "x" && !== "y"` chain at every call
  // site below.
  const showYouTube = providerFilter === "all" || providerFilter === "youtube";
  const showAudius = providerFilter === "all" || providerFilter === "audius";
  const showSoundCloud = providerFilter === "all" || providerFilter === "soundcloud";
  ...
  }, [query, showAudius, audiusRetryGen]);
  ...
  // ─── SoundCloud search (PRIMARY provider) ────────────────────────────────
  // Same shape as the Audius block above — a separate results list rather
  // than forcing SearchResult (YouTube) / GroicTrack (SoundCloud, already
  // normalized) into one array, for the same reasons documented on the
  // Audius block. SoundCloud is the primary source, so its section renders
  // first in the results JSX below, ahead of Audius and YouTube.
  const [soundcloudResults, setSoundcloudResults] = useState<GroicTrack[]>([]);
  ...
  const [soundcloudRetryGen, setSoundcloudRetryGen] = useState(0);
  useEffect(() => {
    const q = query.trim();
    if (!q || !showSoundCloud) {
    ...
    const t = window.setTimeout(() => {
      searchSoundCloud(q)
        .then((tracks) => { if (!cancelled) setSoundcloudResults(tracks); })
        .catch((err) => {
          if (cancelled) return;
          setSoundcloudResults([]);
          setSoundcloudError(err instanceof Error && err.message ? err.message : "SoundCloud is unreachable right now.");
        })
        .finally(() => { if (!cancelled) setSoundcloudLoading(false); });
    }, SEARCH_DEBOUNCE_MS);
    return () => { cancelled = true; window.clearTimeout(t); };
  }, [query, showSoundCloud, soundcloudRetryGen]);

  const onPlaySoundCloud = (t: GroicTrack, pool: GroicTrack[] = soundcloudResults) => {
    hapticMedium();
    const rest = buildUpNextPool(pool, t, (x) => x.id, Math.random, getRecentlyPlayedKeys());
    playTrack(t, [t, ...rest]);
  };
  ...
  const dedupedHome = useMemo(() => {
    ...
    const order = langPrefs;
    const trendingByLang: TrendingBuckets = {};
    for (const id of order) {
      const kept: GroicTrack[] = [];
      for (const t of trending[id] ?? []) {
        const key = songKey(t.title);
        if (used.has(key)) continue;
        used.add(key); kept.push(t);
      }
      trendingByLang[id] = kept;
    }
      ...
      <div className="px-5 pt-4 space-y-6">
                ...
                <div key={d.id} className="flex items-center gap-3 p-1.5 rounded-xl active:bg-foreground/5">
                  <button onClick={() => playDownload(d)} className="flex items-center gap-3 flex-1 min-w-0 text-left">
                    <div className="relative h-10 w-10 rounded-lg overflow-hidden bg-muted shrink-0">
                      {d.thumbnail && <img loading="lazy" decoding="async" src={d.thumbnail} alt="" className="h-full w-full object-cover" />}
                    </div>
                    <div className="min-w-0">
          ...
          <section>
            <div className="flex items-center justify-between mb-2">
              <p className="text-[11px] uppercase tracking-wider text-muted-foreground">
                Languages{langPrefs.length ? ` · ${langPrefs.length}` : ""}
              </p>
              <button onClick={() => { hapticLight(); loadTrending(true); }}
                className="text-[11px] text-muted-foreground flex items-center gap-1 active:scale-95" aria-label="Shuffle trending">
                <RefreshCw className={cn("h-3 w-3", trendingLoading && "animate-spin")} /> Shuffle
              </button>
            </div>
            {/* One scrollable bar. Tap = select, tap again = deselect. */}
            <div role="group" aria-label="Trending languages"
              className="flex gap-1.5 overflow-x-auto no-scrollbar p-1 -mx-1 rounded-2xl bg-muted/40 border border-border/50">
              {LANG_OPTIONS.map(l => {
                ...
                return (
                  <button key={l.id}
                    ...
                    aria-pressed={active}
                    className={cn(
                      "shrink-0 h-10 pl-2.5 pr-3 rounded-xl flex items-center gap-1.5 transition-all active:scale-95",
                      active
                  ...
                  >
                    <span className={cn(
                      ...
                      active ? "bg-primary-foreground/25 border-transparent" : "border-muted-foreground/40",
                    )}>
                      {active && <Check className="h-2.5 w-2.5" strokeWidth={3} />}
                    </span>
                    <span className="flex flex-col items-start leading-none">
                      <span className="text-xs font-semibold">{l.label}</span>
                      {l.native !== l.label && <span className={cn("text-[9px] mt-0.5", active ? "opacity-80" : "opacity-60")}>{l.native}</span>}
                    </span>
                  </button>
                );
              })}
        ...
        {query.trim() && (
          <div className="flex gap-2">
            {([["all", "All Music"], ["soundcloud", "SoundCloud"], ["audius", "Audius"], ["youtube", "YouTube"]] as const).map(([id, label]) => (
              <button key={id}
        ...
        {query.trim() ? (
        <>
        {/* SoundCloud results — PRIMARY provider, rendered first. Same
            GroicTrack card treatment as the Audius section below (kept
            structurally identical on purpose — no separate visual
            language per provider beyond the section label, matching the
            existing Audius section's own comment on this). Selecting one
            plays it natively via the same isNativelyStreamable() path
            Audius already uses. */}
        {showSoundCloud && (
        <section>
          ...
          {soundcloudLoading && soundcloudResults.length === 0 ? (
            <div className="flex gap-3 overflow-x-auto pb-1 -mx-1 px-1" aria-busy="true" aria-label="Loading SoundCloud songs">
            ...
            <div className="flex items-center gap-2 py-2">
              <p className="text-xs text-warning flex-1 min-w-0 truncate">{soundcloudError}</p>
              <button
                onClick={() => { hapticLight(); setSoundcloudRetryGen(g => g + 1); }}
                aria-label="Retry SoundCloud search"
            ...
            <p className="text-xs text-muted-foreground py-2">
              {soundcloudLoading ? "" : "No SoundCloud results for this search."}
            </p>
          ) : (
            <div className="flex gap-3 overflow-x-auto pb-1 -mx-1 px-1">
              {soundcloudResults.map((t) => {
                ...
                return (
                  <div key={t.id} className="shrink-0 w-[128px]">
                    ...
                    <div className="mt-1.5 flex items-start justify-between gap-1">
                      ...
                      <button onClick={() => addSoundCloudToPlaylist(t)}
        ...
        <>
          {trendingLoading ? (
            ...
            <>
              ...
              {dedupedHome.order.length === 0 && (
                <p className="text-xs text-muted-foreground text-center py-6">Pick a language above to see what's trending.</p>
              )}
              {dedupedHome.order.map((id) => (
                <TrackRail
                  key={id}
                  title={`Trending · ${LANG_OPTIONS.find(l => l.id === id)?.label ?? id}`}
                  items={(dedupedHome.trendingByLang[id] ?? []).slice(0, 12)}
                  currentId={current?.id}
                  onPlay={(t, pool) => { hapticLight(); playTrack(t, [t, ...pool.filter(x => x.id !== t.id)]); }}
                  onEnqueue={(t) => { hapticLight(); enqueue(t); toast({ title: "Added to queue" }); }}
                />
              ))}
            </>
          )}
