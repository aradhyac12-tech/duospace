...
async function resolveKey(userId: string): Promise<{ key: string; source: "self" | "partner" | "platform" } | null> {
  ...
  // LATENCY FIX (call-connect critical path): the own-key lookup and the
  // partner-key lookup used to run fully sequentially, with the second
  // only starting once the first had resolved AND been checked. Neither
  // query depends on the other's result — they're two independent reads
  // whose precedence (self beats partner beats platform) only matters
  // once BOTH answers are in hand, not before either starts — so firing
  // them together lets their round trips overlap instead of stacking.
  // The RPC's own SECURITY DEFINER access-control logic is unaffected;
  // this only changes when the request is sent, not what it's allowed to
  // return. On a shared network path (same Postgres connection pool/
  // region as the request) this typically halves this function's own
  // database-latency contribution to the call-connect critical path.
  const [ownResult, partnerKeyResult] = await Promise.all([
    admin.from("user_secrets").select("daily_api_key, daily_provides_calls").eq("user_id", userId).maybeSingle(),
    admin.rpc("get_partner_daily_key", { _user_id: userId }),
  ]);
  // 1. Caller's own key
  ...
  const partnerKey = partnerKeyResult.data;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "create-and-token") {
      ...
      const roomReqBody = JSON.stringify({
        name: finalRoomName,
        properties: {
          exp: Math.floor(Date.now() / 1000) + 86400,
          enable_chat: false,
      ...
      const tokenReqBody = (roomNameForToken: string) => JSON.stringify({
        properties: {
          room_name: roomNameForToken,
          exp: Math.floor(Date.now() / 1000) + 7200,
          is_owner: false,
          enable_screenshare: true,
        },
      });
      const doTokenFetch = (roomNameForToken: string) => fetch("https://api.daily.co/v1/meeting-tokens", {
        method: "POST",
        headers: { "Authorization": `Bearer ${DAILY_API_KEY}`, "Content-Type": "application/json" },
        body: tokenReqBody(roomNameForToken),
      });

      // LATENCY FIX (call-connect critical path): these were two fully
      // sequential Daily REST calls — the token request only started
      // once the room-creation response had fully round-tripped back to
      // this edge function. The token request only needs the room NAME,
      // which is generated client-side before either call, not the room
      // creation *response* — Daily's own self-signed-JWT path for this
      // same endpoint issues a token from nothing but a room-name string,
      // with no server round trip at all, which is a strong signal the
      // two are independent on Daily's side. Firing both together lets
      // their latencies overlap instead of add. Kept safe against the
      // (unverified from here) possibility that Daily's REST endpoint
      // does want the room to already exist: if the parallel token
      // attempt fails AND the room creation itself succeeded, retry the
      // token request once, now strictly after the room is confirmed to
      // exist — so a real ordering requirement, if Daily turns out to
      // have one, degrades to correct sequential behavior instead of a
      // failed call.
      const roomResPromise = fetch("https://api.daily.co/v1/rooms", {
        method: "POST",
        headers: { "Authorization": `Bearer ${DAILY_API_KEY}`, "Content-Type": "application/json" },
        body: roomReqBody,
      });
      const speculativeTokenPromise = doTokenFetch(finalRoomName).catch((err) => ({ __fetchError: err } as never));

      const roomRes = await roomResPromise;
      const roomData = await roomRes.json();
      if (!roomRes.ok) {
        console.error("Daily API error (create-and-token/room):", roomRes.status, roomData);
        speculativeTokenPromise.catch(() => {}); // avoid unhandled rejection; room failed, this attempt is moot either way
        return new Response(
          JSON.stringify({
      ...
      let tokenRes = await speculativeTokenPromise as Response | { __fetchError: unknown };
      let tokenData: { token?: string; error?: string; info?: string; message?: string };
      if ("__fetchError" in tokenRes || !(tokenRes as Response).ok) {
        // Fallback: retry sequentially, strictly after room creation is
        // confirmed — covers both a genuine ordering requirement on
        // Daily's side and a plain transient failure of the parallel
        // attempt.
        tokenRes = await doTokenFetch(roomData.name);
        tokenData = await (tokenRes as Response).json();
      } else {
        tokenData = await (tokenRes as Response).json();
      }
      if (!(tokenRes as Response).ok) {
        console.error("Daily API error (create-and-token/token):", (tokenRes as Response).status, tokenData);
        ...
        return new Response(
          JSON.stringify({
            error: formatDailyError(tokenData, resolved.source, (tokenRes as Response).status),
            code: tokenData?.error ?? null,
            detail: tokenData,
            keySource: resolved.source,
          }),
          { status: (tokenRes as Response).status, headers: { ...corsHeaders, "Content-Type": "application/json" } },
        );
      }
