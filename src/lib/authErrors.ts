...
export function getAuthErrorMessage(err: unknown): string {
  ...
  if (raw.includes("password should be at least") || raw.includes("password is too short") || code === "weak_password") {
    // BUG FIX: this said "at least 6 characters" — this app's actual
    // policy (enforced client-side at signup and password reset, see
    // Auth.tsx's handleSignUp / ResetPassword.tsx's handleReset) is 8+
    // characters with at least one letter and one number. Someone who hit
    // this message from the server (e.g. a weaker password that slipped
    // past a client check, or a direct API call) was being told a number
    // that would still get rejected if they followed it.
    return "Password is too short — use at least 8 characters, with letters and numbers.";
  }
  if (
    code === "otp_expired" ||
