 ...
 *
 * Updated to the current production frontend (Vercel) — was the earlier
 * Lovable preview domain. This constant is only ever reached when `window`
 * is undefined or resolves to a local/dev origin (see isLocalOrigin below);
 * on an actual deployed page, priority #2 already returns the real origin
 * DuoSpace is running on, so this rarely fires in practice, but it's the
 * value emailed reset/confirm links fall back to if it ever does.
 */
const PRODUCTION_WEB_ORIGIN = "https://duospace-ten.vercel.app";
 ...
 *
 * Web path per purpose:
 *   - email_confirm  → `/auth` (bare) — App.tsx's `/auth` and `/auth/callback`
 *     routes both render the same `<AuthRoute />`/`Auth` component, which
 *     already self-detects an incoming callback via `hasAuthCallback()`
 *     regardless of which of the two paths it's on, so this is a
 *     zero-risk rename, not a behavior change. Kept as its own case (not
 *     merged with oauth below) since it was called out specifically as the
 *     one to land on `/auth`.
 *   - oauth / magic_link → `/auth/callback` — unchanged. Google Cloud
 *     Console's own authorized-redirect-URIs list may already have this
 *     exact path registered; changing it here wasn't asked for and risks
 *     breaking that independently of anything in this project.
 *   - password_reset → `/reset-password` — unchanged; `ResetPassword.tsx`
 *     is a dedicated route with its own recovery-link detection.
 *
