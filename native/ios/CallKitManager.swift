...
///
/// This class deliberately does NOT touch WebRTC/the call engine directly — exactly
...
/// layer (the call engine adapter) remains the single source of truth for the
...
@objc public class CallKitManager: NSObject {
    ...
    /// Whether the call CallKit currently knows about is video or voice —
    /// tracked here because CXAnswerCallAction/CXEndCallAction/
    /// CXSetMutedCallAction only ever hand back the call's UUID, not any of
    /// the metadata reportIncomingCall/reportOutgoingCallStarted were given.
    /// BUG FIX: onCallAction's isVideo parameter used to be hardcoded
    /// `true` at every call site below regardless of the call's real type —
    /// meaning accepting a VOICE call from CallKit's native lock-screen UI
    /// (or a Bluetooth/CarPlay button) told the JS bridge it was a video
    /// call, which would have opened the camera on a call the person
    /// explicitly started/received as audio-only.
    ...
    /// True while CallKit has a call that hasn't been answered yet, i.e. its
    /// own ringtone (the user's chosen one — see applyRingtonePreference) is
    /// playing. `public` because the JS-bridge plugin lives in a separate
    /// module and can only see public members. Read by the plugin's
    /// isIncomingRinging(); JS only asks when an INCOMING call has just
    /// arrived, so an unanswered current call can only be that one.
    @objc public var isRingingUnanswered: Bool { currentCallUUID != nil && !isAnswered }

    /// (callId, action, isVideo, conversationId, roomName, wasAnswered) —
    /// called when CallKit answers/ends/mutes a call, so AppDelegate can
    /// forward it into the JS `duospace-call-action` / `duospace-call-
    /// control` events, the same events native/android/CallBridge.kt
    /// dispatches on Android. wasAnswered only matters for the "end"
    /// action — CallKit reports both "declined before pickup" and "hung up
    /// an active call" as the same CXEndCallAction, and the JS side needs
    /// to tell them apart (decline marks call_history missed; hanging up an
    /// active call runs the real leaveCall() path) the same way Android's
    /// separate ACTION_DECLINE vs mute/unmute/end already does.
    @objc public var onCallAction: ((String, String, Bool, String?, String?, Bool) -> Void)?
    ...
    private static let VALID_RINGTONE_IDS: Set<String> = [
    ...
    /// COLD-START LOST-ACTION FIX (mirrors the Android durable-localStorage
    /// fix in scripts/patch-native-permissions.mjs's handleDuospaceCallIntent
    /// — see its comment for the full symptom writeup): `onCallAction`
    /// above is a plain closure that only exists once
    /// DuospaceCallKitBridgePlugin.load() has run and wired it up, which in
    /// turn only forwards into JS via Capacitor's `notifyListeners`, which
    /// itself is a no-op if `DuospaceCallKitBridge.addListener('callAction',
    /// ...)` (usePushNotifications.ts) hasn't executed yet. Answering a call
    /// via CallKit's lock-screen UI while the app is cold/terminated can
    /// easily win that race — the OS answers instantly; the WebView/React/
    /// useAuth chain does not. UserDefaults is written here, synchronously,
    /// inside the CXProviderDelegate callback itself — before any of that
    /// JS-bridge machinery needs to exist — so it survives regardless of
    /// what state the WebView is in. Single-slot (one active call at a
    /// time, same as Android's design), drained by
    /// getPendingCallAction() below / src/lib/nativeCallActionBridge.ts.
    private static let PENDING_ACTION_DEFAULTS_KEY = "duospace_pending_call_action"

    fileprivate static func persistPendingAction(callId: String, action: String, isVideo: Bool, wasAnswered: Bool) {
        let dict: [String: Any] = [
            "callId": callId,
            "action": action,
            "isVideo": isVideo,
        ...
        UserDefaults.standard.set(dict, forKey: PENDING_ACTION_DEFAULTS_KEY)
    }

    /// Reads and clears the pending action recorded above. Called from
    /// DuospaceCallKitBridgePlugin's `getPendingCallAction` JS-facing
    /// method — read-and-clear (not just read) so a call that already got
    /// replayed once, or an old one nobody ever consumed, can't be
    /// mistakenly replayed again for a LATER call.
    @objc public static func drainPendingAction() -> [String: Any]? {
        let value = UserDefaults.standard.dictionary(forKey: PENDING_ACTION_DEFAULTS_KEY)
        UserDefaults.standard.removeObject(forKey: PENDING_ACTION_DEFAULTS_KEY)
        return value
    }
    ...
    @objc public func reportIncomingCall(
        ...
        // Request an immediate fresh location fix the moment a call comes
        // in, independent of reportNewIncomingCall's own completion —
        // CallKit's contract cares about that call finishing quickly, not
        // about location, so this is fired-and-forgotten here rather than
        // sequenced with it. See DuoSpaceLocationManager — this works even
        // if no Capacitor Bridge/WebView has ever loaded (VoIP pushes can
        // arrive before the app has ever been opened), since this manager
        // has no plugin/Bridge dependency of its own.
        DuoSpaceLocationManager.shared.requestImmediateFix(reason: "incoming_call")
    ...
    /// Call once the call engine/WebRTC media is actually flowing both ways (not at
    /// tap-to-call, not at local `call.join()` resolution) — flips CallKit's
    /// own outgoing-call UI from "Calling…" to a connected/timed call
    /// (`reportOutgoingCall(with:connectedAt:)`, only meaningful for a call
    /// this device placed — a no-op for an accepted incoming call, which
    /// CallKit already treats as connected the instant `perform(CXAnswerCallAction)`
    /// fulfilled), and turns on proximity-sensor screen-off for voice calls
    /// (`UIDevice.isProximityMonitoringEnabled`) — the actual mechanism the
    /// OS phone app uses to blank the screen against your ear; CallKit
    /// registration alone does not enable it. Left off for video calls,
    /// which need the screen. Call `setProximityMonitoring(false)` (or
    /// `reportCallEnded`) to release it — screen/video toggling should call
    /// this again with the new `isVideo` value.
    @objc public func reportCallConnected(isVideo: Bool) {
        if let uuid = currentCallUUID {
            provider.reportOutgoingCall(with: uuid, connectedAt: Date())
        }
        setProximityMonitoring(!isVideo)
    }

    /// See reportCallConnected's doc comment — this is the actual proximity
    /// mechanism, exposed separately so a mid-call camera toggle (voice ->
    /// video or back) can flip it without re-reporting connection state to
    /// CallKit. Always runs on the main thread — UIDevice's proximity flag
    /// is main-thread-only.
    @objc public func setProximityMonitoring(_ enabled: Bool) {
...
extension CallKitManager: CXProviderDelegate {
    ...
    public func provider(_ provider: CXProvider, perform action: CXAnswerCallAction) {
        ...
        isAnswered = true
        CallKitManager.persistPendingAction(callId: callId, action: "accept", isVideo: currentCallIsVideo, wasAnswered: false)
        onCallAction?(callId, "accept", currentCallIsVideo, nil, nil, false)
        action.fulfill()
        // Real remote audio isn't up yet at answer-tap — JS calls
        // reportCallConnected() once the call engine reports it (see
        // The call engine adapter's waitForRemoteAudioReady), same as the outgoing
        // side. Not done here to avoid enabling proximity monitoring before
        // there's actually a live call to hold the phone up to.
    }

    public func provider(_ provider: CXProvider, perform action: CXEndCallAction) {
        guard let callId = currentCallId else { action.fail(); return }
        let wasAnswered = isAnswered
        CallKitManager.persistPendingAction(callId: callId, action: "end", isVideo: currentCallIsVideo, wasAnswered: wasAnswered)
        onCallAction?(callId, "end", currentCallIsVideo, nil, nil, wasAnswered)
        action.fulfill()
        currentCallUUID = nil
        currentCallId = nil
        currentCallIsVideo = false
    ...
    // Mute/unmute deliberately does NOT persist a pending action: unlike
    // accept/end, it only ever fires mid-call (JS is already up and the
    // live `onCallAction` -> notifyListeners path already reliably has a
    // listener by then — see CallContext.tsx's `duospace-call-control`
    // handler, mounted for the whole app lifetime once CallProvider is up).
    // There's no cold-start race to close here, and durable-persisting it
    // would risk a stale mute toggle wrongly replaying against a LATER call.
    public func provider(_ provider: CXProvider, perform action: CXSetMutedCallAction) {
        guard let callId = currentCallId else { action.fail(); return }
        onCallAction?(callId, action.isMuted ? "mute" : "unmute", currentCallIsVideo, nil, nil, isAnswered)
        action.fulfill()
    }

    public func provider(_ provider: CXProvider, perform action: CXStartCallAction) {
        // GAP FIX: this used to just fulfill() with no reportOutgoingCall
        // call at all — CallKit's own outgoing-call bookkeeping (system
        // call UI, CallKit-aware Bluetooth/CarPlay displays) never learned
        // the call had actually started dialing, only that the action to
        // start it was accepted. startedConnectingAt is what puts CallKit
        // into its "Calling…" state; reportCallConnected() (called once
        // real remote audio is up) is the other half.
        provider.reportOutgoingCall(with: action.callUUID, startedConnectingAt: Date())
        action.fulfill()
    }

    public func provider(_ provider: CXProvider, didActivate audioSession: AVAudioSession) {
        // The call engine's WebRTC audio session configuration takes over from
