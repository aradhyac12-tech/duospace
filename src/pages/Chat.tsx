...
import LetterReader from "@/components/chat/LetterReader";
import { buildLetterContent } from "@/lib/letter";
...
import { markVanishMedia, markVanishMediaPath } from "@/lib/vanishMedia";
import { endVanishSession, sweepSeenVanish } from "@/lib/vanishPurge";
import { isSpentVanishForViewer, planEndVanish } from "@/lib/vanishPlan";
import { directUpload } from "@/lib/directUpload";
import { resumableUpload } from "@/lib/resumableUpload";
import { savePendingMedia, deletePendingMedia, listPendingMedia, purgeStalePendingMedia, savePendingText, deletePendingText, listPendingText, purgeStalePendingText } from "@/lib/pendingSendQueue";
import { getCachedPartner, setCachedPartner, clearCachedPartner } from "@/lib/partnerCache";
import { logError, logWarn, logInfo, getErrorMessage } from "@/lib/telemetry";
import { callRoomLimiter, scheduledMsgLimiter, formatRetryDelay } from "@/lib/rateLimit";
import { useReconnectRefetch } from "@/lib/networkState";
...
import {
  PERSIST_CHAT_CACHE, readLocalChatPage, readLocalChatBefore, newestLocalCreatedAt,
  persistMessages, removePersistedMessages, reconcilePersistedWindow, clearPersistedConversation,
} from "@/lib/chatCache";
import { isStorable } from "@/lib/localDb/messageStore";
...
// After a conversation's first jump-to-bottom, scroll adjustments within this
// window are instant (no smooth animation) — see firstScrollAtRef in Chat().
...
const Chat = () => {
  ...
  // Same-tick re-entrancy guard for handleSend — see its own comment at
  // the call site for why this replaced the old module-level
  // `createSendDedup()` lock (content+timestamp keyed, and scoped to the
  // whole network round trip rather than just the synchronous double-fire
  // window it needs to guard against).
  const sameTickSendGuard = useRef(false);
  ...
  // Vanish Mode: ids of "vanish"/"vanish_after_seen" messages I read during
  // THIS visit to the chat. Deliberately separate from markedReadRef (which
  // gets reset on e2e key changes / recover): a message I'm looking at right
  // now must never be mistaken for one I read on an earlier visit, or a
  // refetch would hide it — and the sweep delete it — while it's on screen.
  const vanishReadThisVisitRef = useRef<Set<string>>(new Set());
  const readAfterSeenThisVisitRef = useRef(false);
  const vanishSweepInFlightRef = useRef(false);
  const userIdRef = useRef<string | null>(null);
  const partnerIdRef = useRef<string | null>(null);
  // Remove deleted vanish messages from what's on screen AND from the
  // in-session cache, so reopening the chat can't flash them back.
  const dropMessagesLocally = useCallback((ids: string[]) => {
    if (ids.length === 0) return;
    const gone = new Set(ids);
    setMessages(prev => prev.filter(m => !gone.has(m.id)));
    if (messageCache) {
      messageCache = { ...messageCache, messages: messageCache.messages.filter(m => !gone.has(m.id)) };
    }
  }, []);
  const {
  ...
  // Decrypted content of the letter currently open in LetterReader — the
  // reader owns its own open/close envelope animation and only needs the
  // plaintext, same as PhotoViewer only needs a URL above.
  const [viewingLetter, setViewingLetter] = useState<{ content: string; isMine: boolean } | null>(null);
  ...
  // OFFLINE OUTBOX: ids of sends that didn't fail for a real reason but simply
  // because there was no connection. They stay on screen as "sending" (the
  // clock), get NO error toast, and are re-sent automatically, in order, the
  // moment connectivity returns (see flushQueuedSends). Their payloads are
  // already persisted by pendingSendQueue.ts, so they also survive an app kill.
  const queuedOfflineRef = useRef(new Set<string>());
  const flushingQueuedRef = useRef(false);
  ...
  // FIX: Vanish ("ghost") Mode was plain in-memory state, so ANY remount —
  // closing/reopening the app, the OS reclaiming the WebView while a file
  // picker was open, or the App Lock round-trip — silently switched it off.
  // It now persists per user+partner and only ends when the user turns it
  // off (endVanishMode). Read synchronously on first render so the very
  // first message sent after reopening is already a vanish message.
  const [disappearMode, setDisappearModeState] = useState<boolean>(() => {
    try {
      const k = storage.get("duo-vanish-active-key");
      return !!k && storage.get(k) === "1";
    } catch { return false; }
  });
  const setDisappearMode = useCallback((v: boolean) => {
    setDisappearModeState(v);
    const k = storage.get("duo-vanish-active-key");
    if (k) { if (v) storage.set(k, "1"); else storage.remove(k); }
  }, []);
  ...
  // Lets search "jump to result" / pinned-message "jump" ask the list to
  // scroll a specific message into view without knowing whether it's
  // currently rendering the plain path or the virtualized one for long
  // conversations — see MessageTimeline's scrollToMessage/
  // MessageTimelineHandle for why a raw document.getElementById here
  // stopped being reliable once virtualization shipped.
  const messageTimelineRef = useRef<MessageTimelineHandle>(null);
  ...
  // When the cold-start jump-to-bottom happened. Everything that lands in the
  // next SETTLE_WINDOW_MS (call history arriving late, the fresh fetch
  // replacing cached messages, images sizing in) re-anchors to the bottom
  // INSTANTLY rather than smooth-scrolling — that smooth scroll is what made
  // the whole chat visibly fly by on every launch.
  const firstScrollAtRef = useRef(0);
  // True once the network fetch for this conversation has returned; gates
  // writing the persisted cache (so stale cache is never re-saved as fresh) and
  // stops a slow cache read from overwriting fresh data.
  ...
  useLayoutEffect(() => {
    if (user && messageCache && messageCache.userId === user.id) {
      // Vanish messages I already read on an earlier visit are spent — never
      // flash them back from the cache (the fetch below also sweeps them).
      setMessages(messageCache.messages.filter(
        m => !isSpentVanishForViewer(m, user.id, vanishReadThisVisitRef.current),
      ));
      setMessagesLoading(false);
    }
  }, [user]);
  // PERF FIX (WhatsApp-latency brief §5): hydrate partnerId (+ display
  // name/avatar) from the persisted cache in the same commit, before the
  // network fetch below even starts. Everything downstream that gates on
  // partnerId — fetchMessages, useE2E's partner-key fetch, typing,
  // presence, call history — can now begin immediately on a cold start
  // instead of waiting for the profile round trip. The fetch effect right
  // below still always runs and overwrites this with the server's answer
  // (including writing the cache below), so a stale/missing cache only
  // ever costs the one head-start, never correctness.
  useLayoutEffect(() => {
    if (!user) return;
    const cached = getCachedPartner(user.id);
    if (cached) {
      setPartnerId(cached.partnerId);
      setPartnerName(cached.partnerName);
      setPartnerAvatar(cached.partnerAvatar);
    }
  }, [user]);
  const { ready: e2eReady, hasOwnKeys: e2eHasOwnKeys, hasPartnerKey: e2eHasPartnerKey, keyVersion: e2eKeyVersion, encrypt, decrypt } = useE2E(user?.id, partnerId);
  // Latest-value refs for the send path. A send can start BEFORE the key
  // exchange finishes (see attemptSendText) and then has to wait for it —
  // by which point the `e2eReady` / `encrypt` captured in that callback's
  // closure are stale (`encrypt` from the not-ready render hands back
  // PLAINTEXT), so anything after the wait must read these refs instead.
  const e2eReadyRef = useRef(e2eReady);
  const e2eEncryptRef = useRef(encrypt);
  const e2eStatusRef = useRef({ hasOwnKeys: e2eHasOwnKeys, hasPartnerKey: e2eHasPartnerKey });
  e2eReadyRef.current = e2eReady;
  e2eEncryptRef.current = encrypt;
  e2eStatusRef.current = { hasOwnKeys: e2eHasOwnKeys, hasPartnerKey: e2eHasPartnerKey };
  const waitForE2EReady = useCallback(async (timeoutMs = 12000): Promise<boolean> => {
    const startedAt = Date.now();
    while (!e2eReadyRef.current && Date.now() - startedAt < timeoutMs) {
      await new Promise<void>((resolve) => setTimeout(resolve, 150));
    }
    return e2eReadyRef.current;
  }, []);
  ...
  const urgentColumnMissing = useRef(false);
  ...
  const {
    ...
    switchCamera, listCameras, waitForRemoteAudioReady, waitForRemoteParticipant, retryRemoteAudioPlayback, isActiveCall,
  ...
  const { outcome: callOutcome, dismissOutcome: dismissCallOutcome } = useCallOutcome({
    ...
    everConnected,
    onRemoteEnded: () => {
      dispatchCallEvent({ type: "MISSED", source: "remote-outcome" }); // terminal first, then leave
      leaveCall();
  ...
  // Goes through a ref: fetchMessages is declared further down this
  // component, and naming a later `const` from an earlier hook is exactly the
  // pattern that produced DS-UNKNOWN-001 ("Cannot access 'kt' before
  // initialization") in minified production builds.
  const fetchMessagesRef = useRef<(() => unknown) | null>(null);
  useReconnectRefetch(useCallback(() => { void fetchMessagesRef.current?.(); }, []));

  // ─── Decrypt helper ───────────────────────────────────────────────────────
  const decryptMessages = useCallback(async (msgs: Message[]): Promise<DecryptedMessage[]> => {
    return Promise.all(msgs.map(async msg => {
      try {
        // Vanish media must stay out of the on-device cache — register it
        // BEFORE its URL is resolved (resolveSignedUrl consults the registry).
        markVanishMedia(msg);
        return {
          ...msg,
  ...
  // ─── Fetch partner ────────────────────────────────────────────────────────
  // ROOT-CAUSE FIX ("unable to send anything" — text, photos, videos, files,
  // voice notes ALL silently no-op): every send path in this file
  // (handleSend, attemptSendMedia, sendVoiceMessage, handleFileSelect) gates
  // on `if (!user || !partnerId) return;` with no error/toast on that early
  // return — by design, since it's meant to just mean "still loading". But
  // this fetch used to be a single, un-retried `.then()` with no error
  // handling at all: a single transient failure (network blip, a PGRST303
  // "JWT issued at future" race right after sign-in — see client.ts's own
  // retry comment for why that's a known, real occurrence — or any other
  // one-off Postgrest hiccup) left `partnerId` stuck `null` for the rest of
  // the session, with NO retry and NO signal to the user. Every send button
  // then looked and felt fully interactive (no disabled state, no error) but
  // did absolutely nothing when tapped — indistinguishable from "sending is
  // broken" from the person's side, and impossible to recover from short of
  // force-quitting the app. The layoutEffect above papers over this for a
  // *returning* user with a warm partnerCache entry, which is exactly why
  // this was easy to miss in everyday testing but still leaves a first
  // pairing, a cleared cache, or a new device fully exposed to it.
  //
  // Fix: retry with backoff on a real fetch error (not on "no partner yet",
  // which is a legitimate, non-retryable state), and if it still hasn't
  // recovered after several attempts AND there's no cached partner to fall
  // back on, tell the person outright instead of leaving them to discover
  // "sending does nothing" on their own.
  useEffect(() => {
    if (!user) return;
    let cancelled = false;
    let attempt = 0;
    ...
    const fetchPartner = async () => {
      ...
      if (error) {
        logWarn("Chat.fetchPartner", "partner_id fetch failed", { error, attempt });
        attempt += 1;
        if (attempt <= 5) {
          const delay = Math.min(1000 * 2 ** (attempt - 1), 15000);
          retryTimer = setTimeout(fetchPartner, delay);
        } else if (!getCachedPartner(user.id)) {
          // Only alarm the person if we truly have nothing to send to —
          // a cached partner from a prior successful fetch still works fine
          // even while this particular retry loop keeps failing.
          toast({
            title: "Couldn't load your partner info",
            description: "Sending may not work right now. Try reopening the chat or checking your connection.",
            variant: "destructive",
          });
        }
        return;
      ...
      attempt = 0; // reset so a later failure (e.g. after re-pairing) gets its own full retry budget
      // "No partner linked yet" is a resolved state, not "still loading":
      // fetchMessages() never runs without a partnerId, so without this the
      // timeline's loading skeleton would sit there forever instead of the
      // "Link with your partner in settings" empty state.
      if (!data?.partner_id) {
        setMessagesLoading(false);
        // The server is the source of truth: no partner means the cached one
        // (used above as a cold-start head start) is stale — e.g. the pairing
        // was ended from the other phone. Drop it so we don't keep showing,
        // or trying to message, someone this account is no longer linked to.
        clearCachedPartner(user.id);
        setPartnerId(null);
        setPartnerName("");
      ...
      if (data?.partner_id) {
        ...
        const { data: pp, error: ppError } = await supabase.from("profiles")
          .select("display_name,avatar_url").eq("user_id", data.partner_id).single();
        if (cancelled) return;
        if (ppError) {
          // Non-fatal: partnerId is already set, so sending works — only
          // the display name/avatar fell back to defaults. Log and move on
          // rather than retrying (fetchMessages/realtime already re-run
          // whenever partnerId changes, and this rarely fails on its own).
          logWarn("Chat.fetchPartner", "partner profile fetch failed", { error: ppError });
          return;
        }
        if (pp) {
          const resolvedName = myPetNameForPartner||pp.display_name||"Partner";
          setPartnerName(resolvedName);
          setPartnerAvatar(pp.avatar_url);
          // PERF FIX §5: persist for next cold start (see partnerCache.ts).
          setCachedPartner(user.id, { partnerId: data.partner_id, partnerName: resolvedName, partnerAvatar: pp.avatar_url });
        }
      }
    };
  ...
  const fetchMessages = useCallback(async (beforeCreatedAt?: string) => {
    ...
    // CHAT RELIABILITY v2: added client_message_id — the fetchMessages
    // optimistic-reconciliation block below (see its own comment) matches
    // stale "pending-<uuid>" bubbles against fetched rows by this column;
    // without selecting it here that match can never succeed.
    const MESSAGE_COLUMNS = "id,sender_id,receiver_id,content,message_type,file_url,file_name,is_read,reply_to_id,disappear_at,deleted_by_sender,deleted_by_receiver,created_at,client_message_id,silent,important" + (urgentColumnMissing.current ? "" : ",urgent");
    const convFilter =
    ...
    let query: any = supabase.from("messages").select(MESSAGE_COLUMNS)
      .or(convFilter)
    ...
    // Graceful fallback until 20260922120000_add_messages_urgent_flag.sql
    // is deployed: drop "urgent" from the select instead of failing the load.
    if (error && /urgent/i.test(error.message || "") && !urgentColumnMissing.current) {
      urgentColumnMissing.current = true;
      let q2: any = supabase.from("messages").select(MESSAGE_COLUMNS.replace(",urgent", ""))
        .or(convFilter).order("created_at", { ascending: false }).limit(PAGE_SIZE + 1);
      if (beforeCreatedAt) q2 = q2.lt("created_at", beforeCreatedAt);
      ({ data, error } = await q2);
    }
    if (error) {
      // OFFLINE-FIRST: a failed refresh is only an ERROR when there is nothing
      // to show. With the conversation already on screen from the on-device
      // copy (or while simply offline) the person keeps reading it; the next
      // reconnect/resume refetch tops it up. Previously this replaced the
      // whole conversation view with "Couldn't load messages." every time the
      // network was unavailable.
      if (messagesRef.current.length === 0 && isOnlineNow()) setMessagesError("Couldn't load messages.");
      setMessagesLoading(false);
      return;
    }
    ...
    if (data) {
      ...
      const now = new Date();
      // ROOT-CAUSE FIX ("my partner never saw my vanish message"): this filter
      // used to run `new Date(m.disappear_at) > now` on EVERY row that had a
      // disappear_at. For a vanish message that value is the string "vanish"
      // (or "vanish_after_seen"), which parses to an Invalid Date, and
      // `Invalid Date > now` is false — so every vanish message was dropped
      // from every fetch. Anyone who wasn't already sitting in the chat with
      // a live realtime connection when it was sent (partner in another tab,
      // app in the background, a reconnect/resume refetch, a cold start)
      // simply never saw it — and then turning Vanish Mode off deleted it
      // from the database, unseen. Vanish sentinels are not timestamps; only
      // real timestamps get compared against `now`.
      const valid = rawPage.filter(m => {
        if (isVanishValue(m.disappear_at)) {
          // Already read on an earlier visit → spent, don't show it again.
          return !isSpentVanishForViewer(m, user.id, vanishReadThisVisitRef.current);
        }
        return !m.disappear_at || m.disappear_at==="pending" || new Date(m.disappear_at)>now;
      });
      // Messages I read on an earlier visit and never got to sweep (app
      // killed, offline when I left) — finish the job now, in the background.
      if (!vanishSweepInFlightRef.current
          && rawPage.some(m => isSpentVanishForViewer(m, user.id, vanishReadThisVisitRef.current))) {
        vanishSweepInFlightRef.current = true;
        sweepSeenVanish(user.id, partnerId)
          .then(r => dropMessagesLocally(r.deletedIds))
          .catch(err => logWarn("Chat.sweepSeenVanish", "sweep on load failed", err))
          .finally(() => { vanishSweepInFlightRef.current = false; });
      }
      const pageItems = [...valid].reverse();
      const decrypted = await decryptMessages(pageItems);

      // ON-DEVICE COPY: keep what the server just told us. `rawPage` is
      // newest-first, so its LAST element is the window's oldest row.
      if (PERSIST_CHAT_CACHE) {
        const uid = user.id;
        const pid = partnerId;
        void (async () => {
          try {
            if (beforeCreatedAt) {
              // Older page (scroll-back): just store it. No reconcile here —
              // the window bound below only knows its lower edge.
              await persistMessages(uid, pid, decrypted);
              return;
            }
            const windowStart = hasMore ? (rawPage[rawPage.length - 1]?.created_at ?? null) : null;
            // Cursor for delta sync — read BEFORE this window is stored.
            const localNewest = await newestLocalCreatedAt(uid, pid);
            await persistMessages(uid, pid, decrypted);
            // Server-authoritative inside the window it answered for: rows we
            // hold that it no longer has were deleted/cleared elsewhere.
            await reconcilePersistedWindow(uid, pid, rawPage.map(m => m.id), windowStart);
            // GAP FILL: if MORE than a page of messages arrived while this
            // device was away, the newest-page window doesn't touch what we
            // already hold and the local history would silently have a hole
            // (visible when scrolling back offline). Walk the gap forward.
            if (hasMore && windowStart && localNewest && Date.parse(localNewest) < Date.parse(windowStart)) {
              let cursor = localNewest;
              for (let i = 0; i < 10; i++) {
                const { data: gap, error: gapErr } = await supabase.from("messages").select(MESSAGE_COLUMNS)
                  .or(convFilter).gt("created_at", cursor).lt("created_at", windowStart)
                  .order("created_at", { ascending: true }).limit(500);
                if (gapErr || !gap || gap.length === 0) break;
                const gapRows = (gap as Message[]).filter(m => !m.disappear_at);
                await persistMessages(uid, pid, await decryptMessages(gapRows));
                cursor = (gap as Message[])[gap.length - 1].created_at;
                if (gap.length < 500) break;
              }
            }
          } catch (err) {
            logWarn("Chat.localStore", "background local sync failed", err);
          }
        })();
      }
      ...
      if (beforeCreatedAt) {
        ...
        // otherwise we get duplicates with stale _sendStatus "sending" or "failed".
        //
        // CHAT RELIABILITY v2: matching used to be sender+receiver+type+content
        // — the same "content proximity" identity anti-pattern removed from
        // useChatRealtimeMessages.ts's INSERT handler, and with the same failure
        // mode: two legitimately identical in-flight/failed sends ("okay" +
        // "okay") could match the wrong bubble against a single server row,
        // silently dropping one instead of leaving it correctly marked
        // sending/failed. Matching on client_message_id is exact — a
        // `pending-<uuid>` bubble is THIS row's optimistic twin if and only if
        // the row's client_message_id equals that uuid (server-enforced unique
        // key, see idx_messages_client_message_id). Rows with no
        // client_message_id (legacy, or any insert path not yet updated to set
        // it) never match here, which is the safe default: leave the bubble in
        // place rather than risk dropping a message that isn't actually a
        // confirmed duplicate.
        setMessages(prev => {
          const optimistic = prev.filter(m => {
            ...
            const hasServerCopy = decrypted.some(s =>
              s.id !== m.id && !!s.client_message_id && `pending-${s.client_message_id}` === m.id
            );
    ...
    if (!beforeCreatedAt) freshFetchDoneRef.current = true;
    setMessagesLoading(false);
  }, [user, partnerId, decryptMessages, dropMessagesLocally]);
  ...
  useEffect(() => {
    ...
    fetchMessages();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user, partnerId, e2eReady, e2eKeyVersion]);
  ...
  // ON-DEVICE COPY, live changes: realtime deliveries, edits, read receipts,
  // pin/unpin, the optimistic→real swap for messages I send, and deletions all
  // flow through `messages` state, so ONE debounced diff-writer keeps the
  // encrypted local store in step with all of them (see chatCache.ts /
  // localDb/messageStore.ts). Only rows whose content actually changed are
  // rewritten.
  //
  // Deletion rule (important): "absent from state" does NOT mean "deleted" —
  // a refresh replaces state with the newest page and drops any older pages
  // the person had scrolled back through. A stored row is treated as deleted
  // only if it is missing AND newer than the oldest message currently in
  // state (i.e. it falls inside the window state covers). Anything older is
  // simply not loaded right now and is left alone.
  const persistedRef = useRef<{ key: string; sigs: Map<string, { sig: string; ts: number }> }>({ key: "", sigs: new Map() });
  useEffect(() => {
    if (!PERSIST_CHAT_CACHE || !user || !partnerId || messagesLoading) return;
    const uid = user.id;
    const convKey = `${uid}|${partnerId}`;
    const t = setTimeout(() => {
      if (persistedRef.current.key !== convKey) persistedRef.current = { key: convKey, sigs: new Map() };
      const sigs = persistedRef.current.sigs;
      const changed: DecryptedMessage[] = [];
      const present = new Set<string>();
      let oldestTs = Infinity;
      for (const m of messages) {
        present.add(m.id);
        const ts = Date.parse(m.created_at);
        if (Number.isFinite(ts) && !m.id.startsWith("pending-") && ts < oldestTs) oldestTs = ts;
        if (!isStorable(m)) continue;
        const sig = `${m.created_at}|${m.edited_at ?? ""}|${m.is_read ? 1 : 0}|${m.is_pinned ? 1 : 0}|${m.file_url ? 1 : 0}|${m.decryptedContent ?? ""}`;
        if (sigs.get(m.id)?.sig !== sig) {
          changed.push(m);
          sigs.set(m.id, { sig, ts });
        }
      }
      const removed: string[] = [];
      if (messages.length > 0 && Number.isFinite(oldestTs)) {
        for (const [id, v] of sigs) {
          if (!present.has(id) && v.ts >= oldestTs) { removed.push(id); sigs.delete(id); }
        }
      }
      if (changed.length > 0) void persistMessages(uid, partnerId, changed);
      if (removed.length > 0) void removePersistedMessages(uid, removed);
    }, 600);
    return () => clearTimeout(t);
  }, [messages, partnerId, user, messagesLoading]);

  // COLD-LAUNCH HYDRATION from the on-device store — the "WhatsApp" part: the
  // conversation is on screen from local data immediately, with or without a
  // network, and the network only tops it up afterwards. Runs as soon as
  // partnerId is known (a cached partner identity makes that immediate) and
  // deliberately does NOT wait for E2E keys or auth refresh: the stored
  // messages are already decrypted. Skipped when the in-session memory cache
  // already covers this conversation, and never applied over data the network
  // fetch has already returned.
  useEffect(() => {
    if (!PERSIST_CHAT_CACHE || !user || !partnerId) return;
    if (messageCache && messageCache.userId === user.id && messageCache.partnerId === partnerId) return;
    let cancelled = false;
    void readLocalChatPage(user.id, partnerId, PAGE_SIZE).then((page) => {
      if (cancelled || freshFetchDoneRef.current) return;
      if (page.messages.length === 0) {
        // Nothing stored yet. Offline there is nothing coming either — don't
        // leave the loading skeleton up forever.
        if (!isOnlineNow()) setMessagesLoading(false);
        return;
      }
      // Baseline for the diff-writer above so hydrated rows aren't rewritten.
      persistedRef.current = {
        key: `${user.id}|${partnerId}`,
        sigs: new Map(page.messages.map(m => [m.id, {
          sig: `${m.created_at}|${m.edited_at ?? ""}|${m.is_read ? 1 : 0}|${m.is_pinned ? 1 : 0}|${m.file_url ? 1 : 0}|${m.decryptedContent ?? ""}`,
          ts: Date.parse(m.created_at),
        }])),
      };
      // Registering it as the in-session cache makes fetchMessages() reconcile
      // quietly in the background instead of flashing the skeleton back on.
      messageCache = { userId: user.id, partnerId, messages: page.messages };
      setMessages(prev => (prev.length > 0 ? prev : page.messages));
  ...
  const loadMoreMessages = async () => {
    ...
    try {
      if (!isOnlineNow()) {
        // OFFLINE scroll-back: page from the on-device history.
        const page = await readLocalChatBefore(user!.id, partnerId!, messages[0].created_at, PAGE_SIZE);
        if (page.messages.length > 0) setMessages(prev => [...page.messages, ...prev]);
        setHasMoreMessages(page.hasMore);
      } else {
  ...
  useLayoutEffect(() => {
    ...
    if (isColdStart) {
      // Hold the first jump until the conversation itself has loaded. Call
      // history (which only needs partnerId) routinely arrives BEFORE the
      // messages (which wait on E2E keys + decryption): the old code marked
      // the cold start "done" on the call rows alone, so when the messages
      // landed a moment later they got a smooth scroll — the whole chat
      // visibly flying by on every launch.
    ...
    // Instant for the cold start AND for anything that settles shortly after
    // it; only genuinely new messages later get the smooth slide.
    const instant = isColdStart || Date.now() - firstScrollAtRef.current < SETTLE_WINDOW_MS;
    ...
    return () => clearTimeout(t);
  }, [messages.length, callHistory.length, importedMessages.length, surprises.length, messagesLoading]);
  ...
  const handleScheduleMessage = useCallback(async (sendAt: Date) => {
    ...
    const text = message;
    // KI-14: never store a scheduled message as plaintext. Wait for the key
    // exchange (same as the main send path) and encrypt BEFORE clearing the
    // composer, so a slow/failed setup loses nothing — the text stays put and
    // the person can just tap Schedule again.
    if (!e2eReadyRef.current && !(await waitForE2EReady(6000))) {
      toast({ title:"Secure setup isn't finished yet", description:"Your message wasn't scheduled. Try again in a moment.", variant:"destructive" });
      return;
    }
    const enc = await e2eEncryptRef.current(text);
    if (!isEncrypted(enc)) {
      toast({ title:"Couldn't encrypt this message", description:"It wasn't scheduled. Try again.", variant:"destructive" });
      return;
    }
  ...
  }, [message,user,partnerId,waitForE2EReady,disappearMode,toast]);

  // ─── Love letter ──────────────────────────────────────────────────────────
  const handleSendLoveLetter = useCallback(async (subject: string, body: string) => {
    if (!user||!partnerId) return;
    const content = buildLetterContent(subject, body);
    // Optimistic bubble appears the instant the envelope finishes sealing —
    // same pattern as handleSend's text bubble (see above): the dialog
    // closes and a local echo shows up right away, encryption + the network
    // round trip happen after, inside this same function. Letters used to
    // skip this entirely and insert straight to the DB with zero local
    // state update, so the ONLY way one could ever appear was the realtime
    // postgres_changes round trip landing back on this same client — on
    // any latency, or a dropped/delayed event, the letter looked like it
    // never sent even when the row made it into the table fine. This is
    // the actual fix for that: a letter's presence in the chat no longer
    // depends on realtime alone.
    const clientId = `pending-${crypto.randomUUID()}`;
    setShowLoveLetter(false);
    setMessages(prev => [...prev, {
      id: clientId, sender_id:user.id, receiver_id:partnerId,
      content: null, decryptedContent: content,
      message_type:"letter", file_url:null, file_name:null,
      created_at: new Date().toISOString(), is_read:false,
      reply_to_id: null, disappear_at:disappearMode?"vanish":null,
      _sendStatus: "sending",
    } as DecryptedMessage]);
    // KI-14: never send a letter as plaintext. A failure past this point
    // still leaves the bubble above visible (now marked failed) instead of
    // silently discarding what was written — the plaintext lives in that
    // bubble's decryptedContent regardless of how far the send got.
    if (!e2eReadyRef.current && !(await waitForE2EReady(6000))) {
      toast({ title:"Secure setup isn't finished yet", description:"Your letter wasn't sent. Try again in a moment.", variant:"destructive" });
      setMessages(prev => prev.map(m => m.id===clientId ? { ...m, _sendStatus: "failed" } : m));
      return;
    }
    const enc = await e2eEncryptRef.current(content);
    if (!isEncrypted(enc)) {
      toast({ title:"Couldn't encrypt this letter", description:"It wasn't sent. Try again.", variant:"destructive" });
      setMessages(prev => prev.map(m => m.id===clientId ? { ...m, _sendStatus: "failed" } : m));
      return;
    }
    hapticMessageSent();
    try {
      // insertMessageIdempotent (used by every other send path already)
      // also gives this the same client_message_id — useChatRealtimeMessages'
      // own dedup already knows to match `pending-<that id>` and fold the
      // realtime INSERT into this bubble in place instead of adding a
      // second copy.
      const { data: inserted } = await insertMessageIdempotent({
        sender_id:user.id, receiver_id:partnerId, content:enc,
        message_type:"letter", disappear_at:disappearMode?"vanish":null,
      }, toClientMessageId(clientId));
      if (!inserted) throw new Error("insert returned no row");
      setMessages(prev => prev.map(m => m.id===clientId
        ? ({ ...(inserted as any), decryptedContent: content } as DecryptedMessage)
        : m));
      toast({ title:"Letter delivered 💌" });
    } catch {
      toast({ title:"Failed to send letter", variant:"destructive" });
      setMessages(prev => prev.map(m => m.id===clientId ? { ...m, _sendStatus: "failed" } : m));
    }
  // insertMessageIdempotent is declared further down this component with an
  // empty dep list (stable identity), so it is intentionally NOT listed here —
  // naming it in this array evaluates it during render, before its `const` is
  // initialized, which threw "Cannot access before initialization" on mount.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user,partnerId,waitForE2EReady,disappearMode,toast]);
  ...
  // A vanish message only counts as SEEN if the screen was actually visible —
  // the realtime INSERT (and this effect) can also fire while the app sits in
  // the background with Chat still mounted, and marking it read then would
  // delete a message its recipient never laid eyes on. Ordinary messages keep
  // their existing read-receipt behaviour.
  ...
  useEffect(() => {
    ...
    const unread = messages.filter(m =>
      m.sender_id===partnerId && !m.is_read && !markedReadRef.current.has(m.id)
      && (pageVisible || !isVanishValue(m.disappear_at)));
    if (!unread.length) return;
    unread.forEach(m => {
      markedReadRef.current.add(m.id);
      if (isVanishValue(m.disappear_at)) {
        vanishReadThisVisitRef.current.add(m.id);
        if (m.disappear_at !== "vanish") readAfterSeenThisVisitRef.current = true;
      }
    });
    ...
    run();
  }, [messages,user,partnerId,pageVisible]);

  // Vanish Mode: no per-message timer — a vanish message stays for as long as
  // the session runs. Turning the mode off ends the session:
  //   • everything the other person HAS SEEN is deleted for both of you,
  //     including the photos / videos / files / voice notes (rows AND the
  //     files in storage, plus this device's local copies);
  //   • everything they have NOT seen yet is kept — a message nobody has read
  //     must not be erased — and vanishes once they have read it and left the
  //     chat ("vanish_after_seen", see vanishPlan.ts / vanishPurge.ts).
  const endVanishMode = useCallback(async () => {
    ...
    // banner, dialog) synchronously up front instead of after awaiting the
    // network round-trip — otherwise, with "don't show again" checked, the
    // whole vanish-off feel hung until the request finished.
    //
    // Messages the other person has already read are removed from view right
    // away for the same reason. Unread ones stay on screen (they're kept) and
    // are relabelled so the bubble shows they'll vanish once seen.
    const optimisticGone = planEndVanish(
      messagesRef.current.filter(m => isVanishValue(m.disappear_at)),
      user.id,
    ).deleteRows.map(r => r.id);
    setMessages(prev => prev.map(m =>
      m.disappear_at === "vanish" && !m.is_read && m.sender_id === user.id
        ? { ...m, disappear_at: "vanish_after_seen" } : m));
    dropMessagesLocally(optimisticGone);
    setDisappearMode(false);
    setShowEndVanishConfirm(false);
    try {
      const result = await endVanishSession(user.id, partnerId);
      // Reconcile with what the server actually did.
      dropMessagesLocally(result.deletedIds);
      const deleted = new Set(result.deletedIds);
      if (optimisticGone.some(id => !deleted.has(id))) {
        // Something I hid turned out not to be deletable (e.g. read receipt
        // hadn't landed yet) — show the truth again.
        void fetchMessages();
      }
      if (result.keptUnseenCount > 0) {
        toast({
          title: "Some messages haven't been seen yet",
          description: result.keptUnseenCount === 1
            ? "1 message is kept until they've seen it, then it vanishes."
            : `${result.keptUnseenCount} messages are kept until they've seen them, then they vanish.`,
        });
      }
      if (result.mediaLeftBehind > 0) {
        logWarn("Chat.endVanishMode", "some of the partner's uploaded files could not be removed from storage from this device", { count: result.mediaLeftBehind });
      }
    } catch (error) {
      logError("Chat.endVanishMode", "vanish cleanup failed", error);
      toast({ title: "Couldn't clear vanish messages", description: "Some may remain — try again.", variant: "destructive" });
      void fetchMessages();
    }
  }, [user, partnerId, toast, skipVanishConfirmChecked, dropMessagesLocally, fetchMessages]);

  // Keep the id refs current so the unmount cleanup below can read them.
  useEffect(() => { userIdRef.current = user?.id ?? null; }, [user]);
  useEffect(() => { partnerIdRef.current = partnerId; }, [partnerId]);

  // Leaving the chat = "seen and closed": delete the kept-until-seen messages
  // I read during this visit (and their files). Skipped entirely — no network
  // call — when I read none. If this doesn't get to run (app killed, offline),
  // fetchMessages sweeps them the next time the chat opens.
  useEffect(() => () => {
    const uid = userIdRef.current;
    const pid = partnerIdRef.current;
    if (!uid || !pid || !readAfterSeenThisVisitRef.current) return;
    sweepSeenVanish(uid, pid)
      .then(r => dropMessagesLocally(r.deletedIds))
      .catch(err => logWarn("Chat.sweepSeenVanish", "sweep on leave failed", err));
  }, [dropMessagesLocally]);

  // Bind the persisted Vanish flag to THIS conversation once ids are known.
  useEffect(() => {
    if (!user || !partnerId) return;
    const k = `duo-vanish:${user.id}:${partnerId}`;
    storage.set("duo-vanish-active-key", k);
    const saved = storage.get(k) === "1";
    setDisappearModeState(prev => {
      if (prev && !saved) storage.set(k, "1"); // turned on before ids resolved
      return prev || saved;
    });
  }, [user, partnerId]);
  ...
  type TextRetryPayload = { kind: "text"; text: string; replyToId: string | null; isSilent: boolean; isImportant: boolean; isUrgent?: boolean };

  // CHAT RELIABILITY v2 — stable send identity. `clientId` is always
  // `pending-<uuid>` (see every `crypto.randomUUID()` call site below);
  // this strips the local-only "pending-" marker to get the bare UUID
  // that's actually sent to the server and enforced unique there
  // (idx_messages_client_message_id). Kept as a tiny pure helper rather
  // than inlining the regex at every call site so the derivation is
  // provably the same value everywhere (send, retry, rehydrate, and the
  // realtime-reconciliation check in useChatRealtimeMessages.ts).
  const toClientMessageId = (clientId: string) => clientId.replace(/^pending-/, "");

  // Postgres unique_violation. Thrown by idx_messages_client_message_id
  // when a retry's insert reuses a client_message_id that already made it
  // into the table on a previous attempt (e.g. the original request
  // actually succeeded but its HTTP response was lost). supabase-js
  // surfaces this as a PostgrestError with this `code`.
  const PG_UNIQUE_VIOLATION = "23505";

  /**
   * Insert into `messages`, idempotent on `client_message_id`.
   *
   * If this exact client_message_id was already inserted by a prior
   * attempt (this is a retry of a send whose response never made it back
   * to the client), the unique index rejects the second insert and this
   * fetches + returns the row that's already there instead of creating a
   * duplicate canonical message — the server-side half of Invariant 2
   * ("retrying the same send cannot create a duplicate canonical
   * message"). Any other error is rethrown unchanged for the caller's
   * existing failure handling.
   */
  const insertMessageIdempotent = useCallback(async (
    row: Record<string, unknown>, clientMessageId: string,
  ) => {
    let { data, error } = await supabase.from("messages")
      .insert({ ...row, client_message_id: clientMessageId } as any).select().single();
    // urgent column not deployed yet → still send (as important), never fail.
    if (error && "urgent" in row && /urgent/i.test(error.message || "")) {
      urgentColumnMissing.current = true;
      const { urgent: _u, ...rest } = row;
      ({ data, error } = await supabase.from("messages")
        .insert({ ...rest, client_message_id: clientMessageId } as any).select().single());
    }
    if (!error) return { data, alreadyExisted: false as const };
    if (error.code !== PG_UNIQUE_VIOLATION) throw error;
    // Conflict on client_message_id specifically — anything else that also
    // happens to be a 23505 (there is no other unique constraint on this
    // table today) would be surfaced the same, safe way: fetch canonical.
    logInfo("Chat.insertMessageIdempotent", "retry matched an already-accepted send — returning canonical row instead of duplicating", { clientMessageId });
    const { data: existing, error: fetchErr } = await supabase.from("messages")
      .select().eq("client_message_id", clientMessageId).single();
    if (fetchErr || !existing) throw fetchErr || error;
    return { data: existing, alreadyExisted: true as const };
  }, []);

  const attemptSendText = useCallback(async (clientId: string, text: string, replyToId: string | null, isSilent: boolean, isImportant: boolean, isUrgent = false) => {
    if (!user || !partnerId) return;
    setMessages(prev => prev.map(m => m.id===clientId ? { ...m, _sendStatus: "sending" } : m));
    try {
      // FIX ("Securing connection… Please wait a moment" on send): this
      // used to be a hard stop in handleSend — the message was refused with
      // a toast and never even got a bubble, and if key setup was slow the
      // person just had to keep re-tapping. Now the bubble appears at once
      // (as always) and the send WAITS for the key exchange to finish, then
      // encrypts and goes out. If it still isn't ready after the timeout the
      // bubble is marked failed (tap to retry) with a reason that says
      // which half is missing. It NEVER falls back to sending plaintext.
      if (!e2eReadyRef.current && !(await waitForE2EReady())) {
        throw new Error(
          e2eStatusRef.current.hasOwnKeys
            ? "Your partner needs to open DuoSpace once to finish secure setup."
            : "Secure setup isn't finished yet — tap the message to retry.",
        );
      }
      const enc = await e2eEncryptRef.current(text);
      if (!isEncrypted(enc)) throw new Error("Couldn't encrypt this message, so it wasn't sent. Tap to retry.");
      const { data: inserted } = await insertMessageIdempotent({
      ...
      // Send succeeded — drop the reload-resilience copy (see the save
      // call in the catch block below for why this exists).
      deletePendingText(clientId);
    } catch (err) {
      const errMsg = getErrorMessage(err);
      // No connection is not a failure the person needs to act on — queue it
      // (WhatsApp-style clock) and send when we're back. A real server/logic
      // error still falls through to the failed-bubble path below.
      if (!isOnlineNow() || isNetworkError(err)) {
        logInfo("Chat.handleSend", "offline — queued text send for reconnect", { clientId });
        pendingSendPayloads.current.set(clientId, { kind: "text", text, replyToId, isSilent, isImportant, isUrgent });
        queuedOfflineRef.current.add(clientId);
        savePendingText({ clientId, partnerId: partnerId!, text, replyToId, isSilent, isImportant, isUrgent, createdAt: Date.now() });
        setMessages(prev => prev.map(m => m.id===clientId ? { ...m, _sendStatus: "sending" } : m));
        return;
      }
      ...
      setMessages(prev => prev.map(m => m.id===clientId ? { ...m, _sendStatus: "failed" } : m));
      pendingSendPayloads.current.set(clientId, { kind: "text", text, replyToId, isSilent, isImportant, isUrgent });
      // RELOAD-RESILIENCE FIX (section 4 / 27): pendingSendPayloads above
      // is a pure in-memory ref — a reload or app kill right after a
      // failed (or still in-flight) text send used to lose the message
      // completely, with no "failed" bubble to retry and no trace it was
      // ever attempted, unlike media sends which already persist to
      // IndexedDB for exactly this reason (see pendingSendQueue.ts).
      // Persisting the plaintext + reply/silent flags here closes that
      // same gap for text. Best-effort: a persistence failure still lets
      // this session's retry-by-tap work normally, it just won't survive
      // a reload.
      savePendingText({ clientId, partnerId: partnerId!, text, replyToId, isSilent, isImportant, isUrgent, createdAt: Date.now() });
      // DIAGNOSTIC FIX: previously always showed a static "Tap the message
      // to retry." description regardless of cause — meant a text-send
      // failure (RLS, constraint violation, auth issue) gave zero signal
      // toward what was actually wrong, unlike media sends which already
      // surfaced their real error. Now shows the real reason when there is
      // one, same as attemptSendMedia.
      const briefText = errMsg.length > 80 ? errMsg.slice(0, 80) + "…" : errMsg;
      toast({ title: "Failed to send", description: briefText || "Tap the message to retry.", variant: "destructive" });
    }
  }, [user, partnerId, waitForE2EReady, disappearMode, toast, insertMessageIdempotent]);
  ...
  const attemptSendMedia = useCallback(async (
    ...
    // Photos are sent at their original resolution/quality — no downscaling
    // or re-encoding. This used to call compressImageForSend() here (resize
    // to 2048px + re-encode at 0.82 JPEG quality) as a "SPEED FIX" for big
    // camera photos, but that traded away raw quality for upload speed,
    // which is the opposite of what's wanted: raw bytes go straight to
    // directUpload/resumableUpload below exactly as selected, up to the
    // 100MB cap validated in handleFileSelect/handlePasteImage above.
    ...
    try {
      ...
      // SPEED FIX: one direct request with real XHR progress for anything
      // within the bucket's 100 MB cap; chunked resumableUpload only as a
      // fallback when that fails (flaky network, proxy limits).
      let lastPct = -1;
      const onProgress = (uploaded: number, total: number) => {
        const pct = total > 0 ? Math.round((uploaded / total) * 100) : 0;
        if (pct === lastPct) return; // avoid a re-render per XHR event
        lastPct = pct;
        setMessages(prev => prev.map(m => m.id===clientId ? { ...m, _uploadProgress: pct } : m));
      };
      const contentType = (file as File).type || undefined;
      let result: { pseudoPublicUrl: string; path: string };
      try {
        if (file.size > 100 * 1024 * 1024) throw new Error("too large for direct path");
        result = await directUpload({ bucket: "chat-files", objectPath, file, contentType, onProgress });
      } catch (directErr) {
        logWarn("chat.send", "direct upload failed — falling back to chunked", { error: String(directErr) });
        result = await resumableUpload({ bucket: "chat-files", objectPath, file, contentType, onProgress });
      }
      // Vanish Mode: keep this file out of the on-device media cache from the
      // very first time its URL is resolved (see vanishMedia.ts).
      if (isVanishValue(disappearAt)) markVanishMediaPath(result.pseudoPublicUrl);
      ...
      // HARDENING ("[object Object]" toast fix): `err instanceof Error ? err.message
      // : String(err)` produces the literal text "[object Object]" whenever
      // something throws a plain object (no custom toString) instead of a real
      // Error — e.g. a non-standard rejection shape from a native WebView bridge,
      // a future supabase-js error type, or any third-party code in this path
      // that doesn't extend Error. Every current supabase-js error class
      // (PostgrestError, StorageError, FunctionsError, EdgeFunctionError) does
      // extend Error, so this wasn't reachable through the documented SDK
      // surface at the time of writing — but this send path is exactly where a
      // silent regression would be worst (the media literally cannot go out,
      // with no readable reason), so it's made unconditionally safe rather than
      // relying on every future throw site upstream staying well-behaved.
      const errMsg = getErrorMessage(err);
      if (!isOnlineNow() || isNetworkError(err)) {
        // Offline outbox — see queuedOfflineRef. The raw bytes were already
        // persisted before the upload started (savePendingMedia above).
        logInfo("Chat.sendMedia", "offline — queued media send for reconnect", { clientId, msgType });
  ...
  }, [user, partnerId, toast, insertMessageIdempotent]);
  ...
  const retryMessage = useCallback((msg: DecryptedMessage) => {
    ...
    if (!payload) return;
    hapticLight();
    if (payload.kind === "text") attemptSendText(msg.id, payload.text, payload.replyToId, payload.isSilent, payload.isImportant, payload.isUrgent);
  ...
  /** Re-send everything that was queued while offline, oldest first, one at a time. */
  const flushQueuedSends = useCallback(async () => {
    if (flushingQueuedRef.current || !isOnlineNow() || queuedOfflineRef.current.size === 0) return;
    flushingQueuedRef.current = true;
    try {
      // Snapshot: a send that fails offline again re-adds itself to the live
      // set for the NEXT reconnect, it doesn't extend this loop.
      for (const id of [...queuedOfflineRef.current]) {
        if (!isOnlineNow()) break;
        const payload = pendingSendPayloads.current.get(id);
        queuedOfflineRef.current.delete(id);
        if (!payload) continue;
        if (payload.kind === "text") await attemptSendText(id, payload.text, payload.replyToId, payload.isSilent, payload.isImportant, payload.isUrgent);
        else await attemptSendMedia(id, payload.file, payload.msgType, payload.fileName, payload.disappearAt);
      }
    } finally {
      flushingQueuedRef.current = false;
    }
  }, [attemptSendText, attemptSendMedia]);
  // Fires on regained connectivity AND on app resume (lib/networkState.ts).
  useReconnectRefetch(useCallback(() => { void flushQueuedSends(); }, [flushQueuedSends]));
  ...
  // RELOAD-RESILIENCE FIX (text messages — see savePendingText's comment
  // in Chat.tsx and pendingSendQueue.ts for the root cause): rehydrate any
  // text send that failed (or was still mid-insert) the last time this
  // conversation was open, from IndexedDB.
  //
  // CHAT RELIABILITY v2: this used to be deliberately NOT auto-resent like
  // media rehydration above, because a plain `insert()` had no
  // server-side idempotency key the way a resumable upload's objectPath
  // does — auto-resending risked a real duplicate if the original insert
  // had actually succeeded and only the client crashed before seeing the
  // response. It fell back instead to a "same text within a 5-minute
  // window" heuristic to guess whether a queued send already went
  // through — exactly the "content + created_at proximity" identity
  // anti-pattern this effort exists to remove (three genuinely repeated
  // "okay" sends within 5 minutes could each satisfy that heuristic and
  // get silently dropped instead of shown as failed/retryable).
  //
  // Now that every send carries a stable `client_message_id` enforced
  // unique server-side (idx_messages_client_message_id), that gap is
  // closed at the source: auto-resending via attemptSendText reuses the
  // SAME clientId → same client_message_id as the original attempt, so
  // if the original insert already succeeded, insertMessageIdempotent's
  // unique-violation fallback fetches the canonical row instead of
  // creating a duplicate — this now mirrors media rehydration exactly.
  useEffect(() => {
    if (!user || !partnerId) return;
    let cancelled = false;
    (async () => {
      await purgeStalePendingText();
      const pending = await listPendingText(partnerId);
      if (cancelled || pending.length === 0) return;
      // Guard against double-rehydration (React StrictMode's double-invoke
      // in dev, or this effect re-running) the same way the media
      // rehydration effect above does — never insert/resend the same
      // clientId twice. messagesRef.current (kept in sync by the effect
      // near the top of this component) reflects state synchronously here,
      // unlike reading `messages` from the closure.
      const toRehydrate = pending.filter(e => !messagesRef.current.some(m => m.id === e.clientId));
      if (toRehydrate.length === 0) return;
      setMessages(prev => [...prev, ...toRehydrate.map(e => ({
        id: e.clientId, sender_id: user.id, receiver_id: partnerId,
        content: e.text, decryptedContent: e.text,
        message_type: "text", file_url: null, file_name: null,
        created_at: new Date(e.createdAt).toISOString(), is_read: false,
        reply_to_id: e.replyToId, disappear_at: null,
        _sendStatus: "sending",
      } as DecryptedMessage))]);
      for (const e of toRehydrate) {
        pendingSendPayloads.current.set(e.clientId, { kind: "text", text: e.text, replyToId: e.replyToId, isSilent: e.isSilent, isImportant: e.isImportant, isUrgent: e.isUrgent });
      }
      for (const e of toRehydrate) {
        if (cancelled) break;
        attemptSendText(e.clientId, e.text, e.replyToId, e.isSilent, e.isImportant, e.isUrgent).catch(() => {});
        // error handled inside attemptSendText (sets _sendStatus:"failed"
        // + re-persists to IndexedDB), same as every other send path —
        // this is fire-and-forget exactly like the media rehydration loop.
      }
    })();
    return () => { cancelled = true; };
    // eslint-disable-next-line react-hooks/exhaustive-deps -- attemptSendText is stable (useCallback); only user/partnerId should re-trigger this
  }, [user, partnerId]);
  ...
  const handleSend = useCallback(async () => {
    ...
    if (editingMsg) {
      ...
      if (!e2eReadyRef.current && !(await waitForE2EReady(6000))) {
        // Don't write an edited message back as plaintext — keep the edit
        // box open so nothing is lost and the person can just tap again.
        toast({ title:"Secure setup isn't finished yet", description:"Try saving the edit again in a moment.", variant:"destructive" });
        return;
      }
      const enc = await e2eEncryptRef.current(editText);
    ...
    if (!message.trim()||!user||!partnerId) return;
    // (No "Securing connection…" early-return here any more — attemptSendText
    // waits for the key exchange itself and refuses to send unencrypted.)
    ...
    // "/important your text" (or "/emergency") is the opposite: it sends
    // normally but flags the push to ring/alert through the recipient's Do
    // Not Disturb or mute — see send-push/index.ts's bypassesQuiet check.
    // The two are mutually exclusive; whichever prefix is actually on the
    // message wins, checked independently below.
    let text = message;
    let isSilent = false;
    ...
    if (silentMatch) {
      ...
      // FIX: "/urgent" was never parsed at all, so it went out as plain
      // text with the "/urgent" prefix still in it. It's now the strongest
      // mode: same DND-bypassing push as /important, plus urgent=true so
      // the bubble is shown with a distinct red "Urgent" treatment.
      const importantMatch = text.match(/^\/(important|emergency|urgent)\s+([\s\S]+)/i);
      if (importantMatch) {
        isImportant = true;
        isUrgent = /^(urgent|emergency)$/i.test(importantMatch[1]);
        text = importantMatch[2];
      } else if (/^\/(?:important|emergency|urgent)\s*$/i.test(text.trim())) {
        return;
      }
    }
    // FIX AUDIT #15 (Chat Reliability v2 correction): this used to key by
    // `${user.id}-${text.slice(0,20)}-${Date.now()}` — the exact
    // "content + timestamp" anti-pattern the reliability effort exists to
    // remove — and via `sendDedup`'s Set, held that key locked for the
    // full network round trip (release() only runs after attemptSendText
    // resolves). Two problems, in opposite directions:
    //   1. Because Date.now() was baked into the key, two real taps
    //      almost never produced the same key, so the guard rarely
    //      blocked anything — it was close to a no-op against the
    //      double-tap it was meant to catch.
    //   2. Simply dropping Date.now() (keying by `user.id+text` alone)
    //      would overcorrect: since the lock is held for the whole
    //      request, a second genuine tap sending the SAME text ("okay"
    //      again) while the first "okay" is still in flight would get
    //      silently swallowed by tryAcquire — violating the invariant
    //      that legitimately repeated identical sends must remain
    //      distinct messages.
    // What this guard actually needs to catch is narrower than either:
    // the SAME physical tap being delivered as two synchronous handler
    // invocations (a stray double-bound handler, or Enter-to-send and the
    // button's onClick both firing for one press) — both such calls share
    // one JS call stack, before React has re-rendered or any network
    // request has started. A ref-based flag that's set synchronously and
    // cleared on the next microtask catches exactly that window, and
    // nothing longer — it has no relationship to message content, so it
    // can never suppress a later, genuinely separate send of identical
    // text. Message identity/duplication safety over the network is
    // handled independently by client_message_id (attemptSendText).
    if (sameTickSendGuard.current) return;
    sameTickSendGuard.current = true;
    queueMicrotask(() => { sameTickSendGuard.current = false; });
    ...
    setMessages(prev => [...prev, {
      ...
      silent: isSilent, important: isImportant, urgent: isUrgent,
      _sendStatus: "sending",
    } as DecryptedMessage]);
    await attemptSendText(clientId, text, rep?.id||null, isSilent, isImportant, isUrgent);
  }, [message,user,partnerId,waitForE2EReady,replyTo,disappearMode,toast,editingMsg,editText,attemptSendText]);
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>, type:"image"|"file") => {
    ...
    // Raised to 100MB (from 50MB) to match the "chat-files" bucket's server-side
    // limit — see finalize-upload/index.ts's ALLOWED_BUCKETS and the
    // storage_buckets migration, both now 100MB, so this client-side check
    // and the server enforcement agree and a same-size file never round-trips
    // just to be rejected after upload.
    const MAX_MB = 100;
  ...
  // IMAGE PASTE: same pipeline as handleFileSelect — optimistic bubble +
  // background upload. Called from MessageComposer when the user pastes an
  // image from the clipboard (Ctrl+V / mobile paste with image on clipboard).
  const handlePasteImage = async (file: File) => {
    if (!user || !partnerId) return;
    const MAX_MB = 100; // kept in sync with handleFileSelect's MAX_MB above
    if (file.size > MAX_MB * 1024 * 1024) {
      toast({ title: "Image too large", description: `Maximum size is ${MAX_MB}MB.`, variant: "destructive" });
      return;
    }
    const clientId = `pending-${crypto.randomUUID()}`;
    const localUrl = URL.createObjectURL(file);
    ...
    setMessages(prev => [...prev, {
      id: clientId, sender_id: user.id, receiver_id: partnerId,
      content: null, decryptedContent: null,
      message_type: "image", file_url: null, file_name: file.name || "pasted-image.png",
      created_at: new Date().toISOString(), is_read: false,
      reply_to_id: null, disappear_at: disappearAt,
      _sendStatus: "sending", _uploadProgress: 0, _localPreviewUrl: localUrl,
    } as DecryptedMessage]);
    attemptSendMedia(clientId, file, "image", file.name || "pasted-image.png", disappearAt)
      .catch(() => {});
  };
  ...
  const clearChat = async () => {
    ...
    revokeLocalPreviews(messagesRef.current);
    // The on-device copy holds the WHOLE history, not just what is loaded.
    void clearPersistedConversation(user.id, partnerId);
  ...
  const startCall = async (mode:"video"|"voice") => {
    ...
    // was racing two joinCall() calls and causing the call engine's "Duplicate
    // call-object instances are not allowed" error.
    ...
    if (callMachineRef.current.status !== "IDLE") dispatchCallEvent({ type: "RESET", source: "start-new-outgoing-call" });
    ...
    const preJoinWatchdog = setTimeout(() => {
      ...
      dispatchCallEvent({ type: "FAILED", source: "start-call-exit" }); 
      setIsStartingCall(false);
    ...
    // PHASE 2 gate: see Calls.tsx's matching comment — don't create (and
    // then have to discard) a real the call engine room for a self_hosted attempt.
    // Self-hosted is the only calling provider: DuoSpace WebSocket signaling
    // for call control, LiveKit for media. Nothing is created server-side
    // before the call_history row below, so there is nothing to discard.
    const signalingReadyPromise = signalingBridge.ensureReady({ timeoutMs: 5_000 });
    let startedCallId: string | null = null;
    const discardRoom = () => { /* no pre-created room on the self-hosted path */ };
    ...
    try {
      ...
      // getUserMedia({video:true}) inside the call engine's join() never triggers the
      ...
      if (mode === "video" && Capacitor.isNativePlatform()) {
        ...
        if (!camGranted) { clearTimeout(preJoinWatchdog); dispatchCallEvent({ type: "FAILED", source: "start-call-exit" }); setIsStartingCall(false); discardRoom(); return; }
      }
      ...
      // recovery path on denial. Camera is left for the call engine to request
      ...
      if (!micGranted) { clearTimeout(preJoinWatchdog); dispatchCallEvent({ type: "FAILED", source: "start-call-exit" }); setIsStartingCall(false); discardRoom(); return; }
      ...
      // PHASE 2 WIRING: self-hosted branch — see Calls.tsx's matching
      // comment (same reasoning, same NOT RUNTIME-VERIFIED caveat) for why
      // this is a separate flow rather than sharing the call engine's
      // insert-overlaps-with-join code below. Chat.tsx has no state
      // machine to dispatch into (unlike Calls.tsx), so this stays inside
      // the same try/catch as the call engine path — the shared catch block
      // below is already generic enough to handle either provider's
      // failures.
      {
        ...
        const room = await createOutgoingCallRoom("self_hosted", {
          ...
          insertCallHistoryForSelfHosted: async ({ callId, roomName }) => {
            const { data, error } = await supabase.from("call_history").insert({
              ...
              provider: "self_hosted", // REMEDIATION P0-2 — see Calls.tsx's matching comment
            } as any).select().single();
            if (error || !data) throw error ?? new Error("call_history insert failed");
            return { id: (data as any).id as string, sessionId: ((data as any).session_id ?? "") as string };
          },
        });
        ...
        // PHASE 4 — see Calls.tsx's matching comment: CALL_OFFER over the
        // WebSocket is the ring; wait for the gateway's typed answer and fail
        // clearly (after stopping the DB-side ring) if it wasn't accepted.
        ...
        if (media.result !== "connected") {
          leaveCall();
          await cancelOutgoingCall(outgoingCallId, media.result).catch(() => false);
          setCurrentCallId(null);
        ...
        // roomUrl has no real meaning for self_hosted — see Calls.tsx's
        // matching comment.
      ...
      if ((err as Error | null)?.name !== "JoinCancelledError") dispatchCallEvent({ type: "FAILED", source: "start-call-exit" }); 
      ...
      if ((err as Error | null)?.name === "JoinCancelledError") {
        // Cancelled while joining — cancelStartingCall already cleaned up.
        finishCallLatencyTrace("cancelled");
        setIsStartingCall(false);
        return;
      }
      if (classifyCallError(err).code === "PERMISSION_DENIED") invalidateNativeMicGrantCache();
      // DETERMINISTIC CLEANUP: a failure AFTER the call_history row was
      // created (LiveKit token/connect/join failed) used to leave the row
      // in_progress and the callee's phone ringing. End the session over
      // signaling + persistence, and release media.
      if (startedCallId) {
        leaveCall();
  ...
  // gate below.
  const endCall = () => {
    // Instant hang-up — see CallContext.hangUpCall.
  ...
  const cancelStartingCall = () => {
    ...
    // PHASE 4 (self-hosted): stop the callee's ring immediately if the row/offer
    // already exist; idempotent with the bail-outs inside startCall().
  ...
  useEffect(() => {
    ...
    messageTimelineRef.current?.scrollToMessage(searchResults[searchIndex]);
  ...
  return (
    <div className={`flex flex-col h-full bg-background overflow-hidden transition-colors duration-300 ${disappearMode ? "vanish-mode" : ""}`} style={rootBackgroundStyle}>
      ...
      <AnimatePresence>
        {pinnedMsg && (
          <PinnedMessageBanner msg={pinnedMsg} onJump={() => messageTimelineRef.current?.scrollToMessage(pinnedMsg.id)} />
        )}
      </AnimatePresence>
          ...
          <motion.div initial={{ height:0,opacity:0 }} animate={{ height:"auto",opacity:1 }} exit={{ height:0,opacity:0 }} className="overflow-hidden">
            <div className="px-4 py-1.5 bg-primary/10 flex items-center justify-center gap-1.5">
              ...
              <span className="text-[10px] text-primary font-medium">
                Vanish Mode is on — messages &amp; media vanish when you turn it off
              </span>
      ...
      <MessageTimeline
        ...
        setViewingPhoto={setViewingPhoto} setViewingLetter={setViewingLetter} formatTime={formatTime} allReactions={allReactions}
        ...
        <AlertDialogContent className="rounded-2xl max-w-[320px]">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-xs">
              Everything sent while it was on — texts, photos, videos, files and voice notes — will be permanently deleted for both of you once it's been seen. Anything your partner hasn't seen yet is kept until they have, then it vanishes too. Can't be undone.
