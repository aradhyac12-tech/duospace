...
type FlatRow = { key: string; node: ReactNode; isDateSeparator: boolean };

const MESSAGE_ROW_KEY_RE = /^(call-|surprise-|imp-|date-)/;
...
const MessageTimeline = forwardRef<MessageTimelineHandle, MessageTimelineProps>(({
  ...
  const flatRows: FlatRow[] = useMemo(() => {
      ...
      group.items.forEach((item, idx) => {
        if (item.type === "call") {
          ...
          rows.push({
            ...
            node: <CallEvent callType={c.call_type} status={c.status} direction={c.call_direction} durationSeconds={c.duration_seconds} createdAt={c.created_at} isMine={c.caller_id===userId} />,
          });
          return;
        }
        ...
        const msg = item.data;
        const wasSeen = seen.has(msg.id); seen.add(msg.id);
        ...
        rows.push({
          key: msg.id, isDateSeparator: false,
          node: (
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
      ...
      setViewingPhoto, setReactingMsgId, onRetryMessage]);

  // Prefixed keys on every other row type (call-/surprise-/imp-/date-)
  // can't collide with a bare message id, so this only ever maps real
  // messages — exactly what search/pinned-jump need to look up.
  const flatIndexByMessageId = useMemo(() => {
    const m = new Map<string, number>();
    flatRows.forEach((row, i) => { if (!MESSAGE_ROW_KEY_RE.test(row.key)) m.set(row.key, i); });
    return m;
  }, [flatRows]);
