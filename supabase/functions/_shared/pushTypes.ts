...
export interface SendPushRequestBody {
  ...
  /** /important: bypass the recipient's do-not-disturb/mute preferences and ring/alert like a call would. Message-like types only. */
  important?: boolean;
  /**
   * /urgent (a.k.a. /emergency): the strongest tier. Implies `important` (the
   * composer/trigger always send both) but rings and vibrates differently —
   * see MessageAlertLevel in fcm.ts and native/android/MessageAlertService.kt.
   */
  urgent?: boolean;
...
const VALID_TYPES: ReadonlySet<string> = new Set([
  ...
  "incoming_video_call", "call_ended", "call_rejected", "typing",
  "unlink_request", "unlink_approved", "unlink_declined", "song_added", "custom",
]);
...
export function validateBody(raw: unknown): SendPushRequestBody {
  ...
  return {
    ...
    // urgent implies important: a caller that only sends `urgent` must still
    // get the do-not-disturb/mute bypass in send-push.
    important: b.important === true || b.urgent === true,
