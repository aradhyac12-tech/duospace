...
import { applyMessageAlert, iosAlertAps, resolveAlertLevel } from "./messageAlert.ts";
...
export function buildNotificationContent(
  ...
  const content = buildContentForType();
  // /important and /urgent: re-route onto the tier's own alert channel/data
  // (see messageAlert.ts). Only meaningful for message-like types.
  const alertLevel = resolveAlertLevel(body);
  if (alertLevel) applyMessageAlert(content, alertLevel, senderName);
  return content;

  function buildContentForType(): NotificationContent {
  switch (body.type) {
    case "chat_message":
    ...
    case "unlink_request":
      // Consent-critical: the partner's approval is what actually unlinks the
      // pair, so this is high priority (still not a call alert).
      return { title: "Unlink request", body: `${senderName} has asked to unlink from you. Open DuoSpace to approve or decline.`, data: baseData, channelId: CHANNELS.system, priority: "high", isCallAlert: false };
    case "unlink_approved":
      return { title: "You're unlinked", body: `${senderName} approved your unlink request`, data: baseData, channelId: CHANNELS.system, priority: "normal", isCallAlert: false };
    case "unlink_declined":
      return { title: "Unlink request declined", body: `${senderName} declined your unlink request. You're still linked.`, data: baseData, channelId: CHANNELS.system, priority: "normal", isCallAlert: false };
    case "song_added":
      // Our Playlist (playlist_songs) — the collaborative queue both
      // partners add to (see useOurPlaylist.ts / Playlist.tsx). `preview`
      // carries "title — artist", built by the DB trigger that dispatches
      // this (see the playlist_songs AFTER INSERT trigger).
      return { title: `${senderName} added a song 🎵`, body: truncate(body.preview, 140) || "Added a song to Our Playlist", data: baseData, channelId: CHANNELS.system, priority: "normal", isCallAlert: false };
...
function buildFcmMessage(token: string, content: NotificationContent, ttlSeconds: number, platform: PushToken["platform"]): FcmV1Message {
  ...
  // Every Android push is now data-only (no top-level `android.notification`
  // block at all — previously only calls were). Root cause this fixes: with
  // a `notification` block present, the OS can auto-display its own bare
  // notification whenever the app is backgrounded, entirely bypassing any
  // FirebaseMessagingService — so DuoSpaceMessagingService.kt (see
  // native/android/DuoSpaceMessagingService.kt) never even ran for message/
  // reaction/call-status pushes in that state, and Capacitor's own default
  // push-notifications service could independently auto-display the same
  // message, producing the duplicate/blank stacked notifications seen on
  // device. Now every push (calls via CallNotificationService.kt, everything
  // else via DuoSpaceMessagingService.kt) is rendered exactly once, by our
  // own code, with WhatsApp-style BigTextStyle + per-conversation grouping
  // (a real summary line, not Android's blank auto-summary) + a redacted
  // setPublicVersion() fallback for lock-screen privacy. title/body/
  // channelId/groupKey travel inside `data` for the native side to read.
  msg.message.data.title = content.title;
  msg.message.data.body = content.body;
  msg.message.data.channelId = content.channelId;
  if (content.groupKey) msg.message.data.groupKey = content.groupKey;
  ...
  if (platform === "ios" && !content.isCallAlert) {
    const level = content.data.alertLevel === "urgent" ? "urgent"
      : content.data.alertLevel === "important" ? "important" : null;
    // Critical alerts need Apple's entitlement — opt in per deployment.
    const criticalEnabled = Deno.env.get("IOS_CRITICAL_ALERTS") === "true";
    const alertAps = level ? iosAlertAps(level, criticalEnabled) : null;
    msg.message.apns = {
      // Alerts: priority 10 + time-sensitive (or critical) interruption level
      // breaks through Focus/Do Not Disturb. Neither can override the hardware
      // silent switch unless the critical-alert entitlement is active — see
      // iosAlertAps() and PUSH_NOTIFICATIONS.md.
      headers: level ? { "apns-push-type": "alert", "apns-priority": "10" } : undefined,
      payload: {
        aps: {
          alert: { title: content.title, body: content.body },
          // Tier-specific bundled .caf (native/ios/Sounds/alert_<tier>.caf,
          // ~20-27 s, plays once — iOS caps push sounds at 30 s). Ordinary
          // messages keep the recipient's chosen "<id>_msg.caf".
          sound: alertAps ? alertAps.sound : `${content.data.messageSound ?? "classic"}_msg.caf`,
          "mutable-content": 1,
          ...(alertAps ? { "interruption-level": alertAps["interruption-level"], "relevance-score": alertAps["relevance-score"] } : {}),
        },
      },
    };
  }
