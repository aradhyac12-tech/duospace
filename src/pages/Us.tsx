...
const Us = () => {
  ...
  const calcStreak = useCallback(async (uid: string, pid: string | null): Promise<number | null> => {
    if (!pid) return 0;
    ...
    // A failed query is "unknown", NOT "no streak" — offline it used to read as 0.
    if (!data) return null;
  ...
  // OFFLINE-FIRST: paint from the last known hub data (encrypted on this phone,
  // lib/screenCache.ts) instead of a skeleton → error card whenever the network
  // is down. The network refreshes it; a failed refresh with data on screen
  // changes nothing. Today's daily-question answers are only reused if they
  // were saved today (they're date-scoped).
  type UsSnapshot = {
    profile: any; partnerProfile: any; countdowns: any[];
    answerDate: string; myAnswer: string | null; partnerAnswer: string | null; streak: number;
  };
  ...
  useEffect(() => {
    if (!user) return;
    let cancelled = false;
    void readScreenCache<UsSnapshot>(user.id, "us").then((snap) => {
      if (cancelled || !snap?.profile || fetchedRef.current) return;
      hasDataRef.current = true;
      setProfile(snap.profile);
      setMoodEmoji(snap.profile.mood_emoji || "😊");
      setMoodText(snap.profile.mood_text || "");
      setPartnerId(snap.profile.partner_id ?? null);
      setPartnerProfile(snap.partnerProfile ?? null);
      setCountdowns(snap.countdowns ?? []);
      if (snap.answerDate === todayKey()) { setMyAnswer(snap.myAnswer); setPartnerAnswer(snap.partnerAnswer); }
  ...
  useEffect(() => {
    if (!user || !hasDataRef.current || !profile) return;
    writeScreenCache(user.id, "us", {
      profile, partnerProfile, countdowns, answerDate: todayKey(), myAnswer, partnerAnswer, streak,
    } satisfies UsSnapshot);
  }, [user, profile, partnerProfile, countdowns, myAnswer, partnerAnswer, streak]);
    ...
    const load = async () => {
      ...
      try {
        ...
        if (p) {
          ...
          if (!p.partner_id) { setPartnerProfile(null); setCountdowns([]); setMyAnswer(null); setPartnerAnswer(null); setStreak(0); }
          if (p.partner_id) {
        ...
        if (cancelled) return;
        // Something (cached or already fetched) is on screen → a failed refresh isn't an error card.
        if (!hasDataRef.current) setLoadError(capture("DS-US-001", { component: "Us", action: "load", cause: err }));
      } finally {
  ...
  // F1: Partner online presence — shared with Chat via useChatPresence
  // (was a duplicate inline `presence-${pair}` subscription; same channel
  // name as useChatPresence.ts meant two independent client subscriptions
  // tracking/listening on the identical channel whenever both were mounted)
  ...
  const updateMood = async () => {
    if (!requireOnline("Updating your mood")) return;
    if (!user) return;
  ...
  const sendTap = async () => {
    if (!requireOnline("Sending a tap")) return;
    if (!user || !partnerId) {
  ...
  const addCountdown = async () => {
    if (!requireOnline("Adding a countdown")) return;
  ...
  const deleteCountdown = async (id: string) => {
    if (!requireOnline("Deleting a countdown")) return;
  ...
  const submitDailyAnswer = async () => {
    if (!requireOnline("Submitting your answer")) return;
