...
class DuoSpaceLocationService : Service() {
    ...
    private fun startWatcher(intervalMs: Long) {
        if (!hasLocationPermission()) {
            LocationFixBridge.listener?.onError("denied", "ACCESS_FINE_LOCATION not granted")
            stopSelf()
            return
        }
        ...
        val callback = object : LocationCallback() {
            override fun onLocationResult(result: LocationResult) {
                ...
                LocationFixBridge.listener?.onFix(
        ...
        try {
            ...
            LocationFixBridge.listener?.onError("denied", e.message ?: "SecurityException requesting updates")
        }
    }
    ...
    private fun requestOneShotFix(reason: String, timeoutMs: Long) {
        ...
        val timeoutRunnable = Runnable {
            cts.cancel()
            LocationFixBridge.listener?.onError("timeout", "No fix within ${timeoutMs}ms (reason=$reason)")
        }
        ...
        try {
            fusedClient.getCurrentLocation(Priority.PRIORITY_HIGH_ACCURACY, cts.token)
                .addOnSuccessListener { loc ->
                    ...
                    if (loc == null) {
                        LocationFixBridge.listener?.onError("unavailable", "getCurrentLocation returned null (reason=$reason)")
                ...
                .addOnFailureListener { e ->
                    mainHandler.removeCallbacks(timeoutRunnable)
                    LocationFixBridge.listener?.onError("unknown", e.message ?: "getCurrentLocation failed (reason=$reason)")
                }
        } catch (e: SecurityException) {
            mainHandler.removeCallbacks(timeoutRunnable)
            LocationFixBridge.listener?.onError("denied", e.message ?: "SecurityException on one-shot fix")
        }
    }
