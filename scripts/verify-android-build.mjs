...
function checkNative() {
  ...
  // Screenshots + screen recording must stay blocked in the APK. Checks both
  // MainActivity variants (Capacitor's template is .java; the .kt path above
  // is the older assumption) so this can't pass or fail on the wrong file.
  const mainActivityJavaPath = join(ROOT, "android", "app", "src", "main", "java", "com", "duospace", "app", "MainActivity.java");
  const secureCandidates = [mainActivityKt, mainActivityJavaPath].filter((p) => existsSync(p));
  if (secureCandidates.length > 0) {
    const hasSecure = secureCandidates.some((p) => readFileSync(p, "utf8").includes("DUOSPACE FLAG_SECURE"));
    if (hasSecure) ok("MainActivity sets FLAG_SECURE (screenshots + screen recording blocked)");
    else fail("MainActivity missing FLAG_SECURE — this APK would allow screenshots/screen recording; run `npm run cap:patch-permissions`");
  }

  // Java/Kotlin interop guard (compileReleaseJavaWithJavac): the Java
  // MainActivity assigns `CallNotificationService.isAppForegrounded = ...` as a
  // plain static field. That only compiles if the Kotlin companion property is
  // annotated @JvmField — otherwise javac fails with "isAppForegrounded has
  // private access in CallNotificationService" after a ~2 minute Gradle run.
  if (existsSync(mainActivityJavaPath)) {
    const javaSrc = readFileSync(mainActivityJavaPath, "utf8");
    if (/CallNotificationService\.isAppForegrounded\s*=/.test(javaSrc)) {
      const cnsPath = join(ROOT, "android", "app", "src", "main", "java", "com", "duospace", "app", "CallNotificationService.kt");
      if (existsSync(cnsPath)) {
        const cnsSrc = readFileSync(cnsPath, "utf8");
        if (/@JvmField\s+@Volatile\s+var\s+isAppForegrounded\b|@Volatile\s+@JvmField\s+var\s+isAppForegrounded\b/.test(cnsSrc)) {
          ok("CallNotificationService.isAppForegrounded is @JvmField (Java MainActivity can assign it)");
        } else {
          fail("CallNotificationService.isAppForegrounded is missing @JvmField but MainActivity.java assigns it directly — javac would fail with 'private access'");
        }
      }
    }
  }
