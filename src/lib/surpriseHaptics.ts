...
import { stripInlineData } from "@/lib/surpriseDocument";
 ...
 * free).
 */
export const analyzeSurpriseContent = (htmlRaw: string, cssRaw: string, jsRaw: string): SurpriseAnalysis => {
  // Imported surprises embed images/audio as base64 data: URIs. Those are
  // megabytes of noise to every scan below (and can contain hex-looking or
  // digit runs that would score as "color"/"timing") — drop them first.
  const html = stripInlineData(htmlRaw);
  const css = stripInlineData(cssRaw);
  const js = stripInlineData(jsRaw);
...
// ── Surprise 2.0 §12/13: semantic lifecycle events ──────────────────────────
// The brief asks for a dedicated engine with named events (RECEIVE,
// MATERIALIZE, INTERACT, MAJOR_REVEAL, COMPLETE, CLOSE) instead of every
// caller picking a raw HapticSequence off SCORES by hand. These are derived
// from the existing per-mood open/expand/close tables above rather than 30
// more hand-authored entries — each event is a genuinely distinct MOMENT in
// the surprise's life, not a distinct mood-design exercise, so reusing the
// tuned open/expand/close shapes (just recombined/retimed) keeps one source
// of truth for "what does romantic feel like" instead of two that can drift
// apart.
//
// Mapping, and why:
//   RECEIVE      — a surprise silently entering the timeline (Phase 1's
//                  auto-"delivered→received" step). The person may not even
//                  be looking at the screen, so this is deliberately the
//                  quietest event that exists: one beat, not a sequence.
//   MATERIALIZE  — the SurpriseMessage row's entrance animation actually
//                  playing. A quick arrival ping, NOT the full anticipation
//                  build the old standalone-overlay `open` sequence used —
//                  that build made sense when opening WAS the first time you
//                  saw anything; now the row itself is the first thing you
//                  see, so the ping should be more "message arrived" than
//                  "beginning a whole show".
//   INTERACT     — tapped the row / expanded the overlay's glass phase. This
//                  is where the ORIGINAL open sequence's full anticipation
//                  arc still belongs — reused as-is, unmodified.
//   MAJOR_REVEAL — the deliberate full-screen expand (existing `expand`
//                  sequence — already the biggest, most confident beat of
//                  the four, unmodified).
//   COMPLETE     — closed AFTER genuinely engaging with the content (not an
//                  instant bounce). One extra affirming tap appended after
//                  the mood's own close beat, so it reads as "that's done"
//                  rather than identical to an early bail.
//   CLOSE        — closed without meaningfully engaging (previewing your
//                  own sent surprise, or dismissing before the intro even
//                  finished). Reuses the mood's existing close beat as-is —
//                  deliberately the least eventful of the six, matching how
//                  it already behaved before this pass.
const receiveSequence = (mood: SurpriseMood): HapticSequence => [SCORES[mood].ambient[0]];

const materializeSequence = (mood: SurpriseMood): HapticSequence =>
  SCORES[mood].open.slice(0, 2).map((s, i) => ({ ...s, delayMs: i * 90 }));

const majorRevealSequence = (mood: SurpriseMood): HapticSequence => SCORES[mood].expand;

const completeSequence = (mood: SurpriseMood): HapticSequence => [
  ...SCORES[mood].close,
  { kind: "double", delayMs: 160 },
];

/**
 * SurpriseHapticEngine — the semantic surface everything in the app should
 * call instead of reaching into SCORES/buildHapticScore directly. Every
 * method fires immediately (or, for ambientLoop, starts the existing
 * mood-paced interval) — callers invoke each one at the exact moment its
 * matching VISUAL event already happens (see the call sites in
 * SurpriseMessage.tsx / useChatSurprise.ts / SurpriseReveal.tsx), which is
 * what keeps haptics synchronized to the animation timeline per §13 without
 * a second, independent scheduler: Framer Motion's own `variants`/
 * `transition` delays already ARE this surprise's one visual timeline, so
 * anchoring haptic calls to the same event handlers that drive those
 * transitions (onClick, onAnimationComplete, the `visible` effect) keeps
 * both in lockstep by construction rather than by two clocks agreeing.
 */
export const SurpriseHapticEngine = {
  receive: (mood: SurpriseMood) => playHapticSequence(receiveSequence(mood)),
  materialize: (mood: SurpriseMood) => playHapticSequence(materializeSequence(mood)),
  interact: (mood: SurpriseMood) => playHapticSequence(SCORES[mood].open),
  majorReveal: (mood: SurpriseMood) => playHapticSequence(majorRevealSequence(mood)),
  complete: (mood: SurpriseMood) => playHapticSequence(completeSequence(mood)),
  close: (mood: SurpriseMood) => playHapticSequence(SCORES[mood].close),
  /** Same ambient loop as before (see playHapticMovie) — kept as its own
   *  method since "keep gently pulsing while open" isn't a discrete event
   *  the way the six above are. */
  ambientLoop: (mood: SurpriseMood, contentDurationMs: number | null) =>
    playHapticMovie(SCORES[mood], contentDurationMs),
};
