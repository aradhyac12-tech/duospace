...
// PERF: `qrcode` is loaded on demand (inside the render call below) instead of at
// module top level, so it is no longer part of the entry bundle every launch pays for.
...
/**
 * What happened to the QR that was on screen:
 *  - "partner_linked": a signed-in partner scanned this signed-in device's invite
 *    QR and the server linked the two accounts on the spot.
 */
export type RedeemKind = "session" | "signup_invite" | "anon_signup" | "partner_linked";
...
interface QRSignInDisplayProps {
   ...
   */
  onRedeemed?: (kind: RedeemKind, info?: { linked: boolean }) => void;
  /**
...
const QRSignInDisplay = ({ onClose, onRedeemed, mode = "device_pairing" }: QRSignInDisplayProps) => {
  ...
  const [redeemed, setRedeemed] = useState<{ kind: RedeemKind; linked: boolean } | null>(null);
  ...
  const mintAndRender = async () => {
    ...
    try {
      ...
      if (canvasRef.current) {
        ...
        // Re-check after the await: the canvas can unmount while the chunk loads.
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (typeof document !== "undefined" && document.hidden) return;
      try {
        const res = await invokeEdgeFunction<{ redeemed: boolean; kind?: string; linked_partner?: boolean }>(
        ...
        if (res.redeemed && currentTokenRef.current === token) {
          const rawKind = (res.kind as "session" | "signup_invite" | "anon_signup" | undefined) ?? "session";
          const linked = !!res.linked_partner;
          // A signup_invite that the server already turned into a link is a
          // completed partner link, not "they're still signing up".
          const kind: RedeemKind = rawKind === "signup_invite" && linked ? "partner_linked" : rawKind;
          // This device showed the QR while signed OUT and a signed-in partner
          // scanned it. The link can only be finished once this device has an
          // account, so remember the token: it is the proof the claim needs
          // (see lib/qrPartnerClaim.ts).
          if (kind === "anon_signup" && linked) rememberQrClaim(token);
          setRedeemed({ kind, linked });
          onRedeemed?.(kind, { linked });
        }
      } catch {
  ...
  const redeemedCopy = redeemed && {
    ...
    partner_linked: { title: "Linked ✓", body: "You're now partners." },
    anon_signup: redeemed.linked
      ? { title: "Scanned ✓", body: "You'll be linked as soon as you finish creating your account." }
      : { title: "Scanned", body: "They weren't signed in, so nobody was linked. Once you both have accounts, link from Settings." },
  }[redeemed.kind];
