...
export const AudioFallbackBanner = () => (
  <div
    role="status" aria-live="polite"
    className="absolute top-16 left-1/2 -translate-x-1/2 z-20 rounded-full bg-call-stage-foreground/20 backdrop-blur-md px-3 py-1.5"
  >
    <span className="text-[11px] font-medium text-call-stage-foreground">Audio-only · weak connection</span>
  </div>
);
...
export const PartnerLeftBanner = ({ partnerName }: { partnerName: string }) => (
  ...
  >
    <p className="text-lg font-medium text-call-stage-foreground">{partnerName} left the call</p>
    <p className="text-xs text-call-stage-foreground/50">Tap the red button to end, or wait — they may rejoin.</p>
  </div>
);
