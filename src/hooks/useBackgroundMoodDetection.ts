...
import { acquireCamera, isCameraPaused } from "@/lib/cameraBus";
...
import {
  type ExpressionSample, type MoodLabel, extractExpression, moodToValence,
  scoreMoods, scoreMoodsFromBlendshapes, softmaxScores, filterBlinks, weightedAvg,
  weightedAvgBlendshapes, filterBlinksBlendshapes,
  getLocalDistrust, sampleLuma, moodFrameIssue, MOOD_MIN_LUMA, topMoodWithMargin,
  sessionBaseline, blendshapeBaseline,
...
export const useBackgroundMoodDetection = () => {
  ...
  useEffect(() => {
    ...
    const runOnce = async () => {
      ...
      // FIX (silent camera contention during calls): this runner never
      // checked the camera bus's pause signal, unlike usePeekDetection —
      // a call asks other camera consumers to yield (see CallContext.tsx /
      // Calls.tsx's pauseCameraConsumers calls) but nothing here was
      // listening. Since this mode has no UI and no human watching, a
      // missed/failed capture here was silent either way, but it could
      // still contend for the device with an in-progress video call's own
      // camera track. Skip this cycle entirely and let the next scheduled
      // run try again once the call ends.
      ...
      try {
        // Don't even open the camera for a read that couldn't be saved. The
        // same gate is checked again right before the write below, so a
        // consent revoked during the ~2.5s capture still stops the save.
        if (!(await canPersistDerivedMood(user.id)).allowed) return;
        ...
        lease = await acquireCamera("user", "MOOD_BACKGROUND");
        video.srcObject = lease.stream;
        await video.play().catch(() => {});
        const landmarker = await getMoodLandmarker().catch(() => { throw new Error("model unavailable"); });

        const samples: ExpressionSample[] = [];
        // Parallel pool of trained-model blendshape reads — see
        // MoodDetector.tsx's blendshapeSamplesRef for the full rationale;
        // same primary-path/fallback split applies here.
        const blendshapeSamples: BlendshapeScores[] = [];
        const deadline = Date.now() + CAPTURE_MS;
        let ts = performance.now();
        while (Date.now() < deadline) {
          // A call can start mid-capture (this window runs a few seconds) —
          // bail out immediately rather than finishing the read on a
          // now-contended camera. Whatever the loop collected so far below
          // MIN_USABLE_SAMPLES just won't be scored (same as any other
          // low-signal capture).
          if (isCameraPaused()) break;
          await new Promise((r) => setTimeout(r, SAMPLE_INTERVAL_MS));
          if (!video || video.readyState < 2) continue;
          try {
            ts = Math.max(ts + 1, performance.now());
            const { faces, blendshapes } = detectMoodFrame(landmarker, video, ts);
            ...
            if (faces.length === 1 && canvas) {
              ...
              if (!tooDark && !moodFrameIssue(faces[0])) {
                ...
                samples.push(expr);
                if (blendshapes) blendshapeSamples.push(blendshapes);
              }
            }
        ...
        // Prefer the trained blendshape model whenever this run collected
        // enough usable reads from it — see scoreMoodsFromBlendshapes' doc
        // comment in lib/moodScoring.ts. Geometry is the fallback, not a
        // second vote — see MoodDetector.tsx's analyzeMood for the same split.
        const blendshapePool = filterBlinksBlendshapes(blendshapeSamples);
        let evidence: Record<MoodLabel, number>;
        if (blendshapePool.length >= 3) {
          const avgBlendshapes = weightedAvgBlendshapes(blendshapePool);
          // Same camera-angle/per-face bias correction as MoodDetector.tsx —
          // see blendshapeBaseline's doc comment in lib/moodScoring.ts.
          evidence = scoreMoodsFromBlendshapes(avgBlendshapes, distrustRef.current, blendshapeBaseline(blendshapePool));
        } else {
          ...
          // See sessionBaseline's doc comment in lib/moodScoring.ts — same
          // fix as the manual card, normalizing for this face's own resting
          // brow/mouth-curve/eye-openness instead of fixed universal constants.
          const baseline = sessionBaseline(pool);
          evidence = scoreMoods({ mouthCurve, mouthOpen, browRaise, eyeOpenness }, distrustRef.current, baseline);
        }
        ...
        // PRIVACY (Phase 1.6): this path is unattended, so it is the one that
        // most needs the gate — a camera-derived value is only saved when
        // MOOD_PROCESSING consent is granted. Denied = skip silently (there
        // is no UI to explain, and nothing has left the device).
        const persist = await canPersistDerivedMood(user.id);
        if (!persist.allowed) return;
        ...
        const { error } = await supabase.from("mood_logs").insert({
          ...
          features: {
            source: "background",
            // KI-04: raw facial measurements are deliberately not stored
            // (partner-readable table, nothing reads them) — see MoodDetector.tsx.
