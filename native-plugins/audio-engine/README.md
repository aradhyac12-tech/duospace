...
- **iOS** (`ios/Plugin/`): `AVPlayer` + `AVAudioSession(.playback)` +
  ...
  existing calling feature's own `AVAudioSession` usage (the call engine's
...
This plugin's core job (above) is still Audius/SoundCloud-only — YouTube
audio is never extracted, proxied, or routed through it. But this plugin
*also* exposes `startWebKeepAlive()`/`updateWebKeepAlive()`/
`stopWebKeepAlive()`, added to close a real, separate gap: a YouTube
track playing through GroicContext's existing hidden IFrame had no
protection against Android freezing the app's process (and the WebView
with it) once backgrounded, unlike Audius/SoundCloud tracks, which
already had this via their own foreground service.

These three methods start/update/stop a **second, independent** foreground
service (`WebKeepAliveService.kt` on Android) / Now Playing registration
(iOS) that never plays or reads any audio — it only mirrors state
GroicContext already has (position/duration/playing/title/artist, pushed
from its own existing YouTube IFrame polling) into a real OS-level
MediaSession/Now Playing entry, and forwards any lock-screen/Bluetooth
command back to JS as a `webKeepAliveCommand` event, which GroicContext
routes to the IFrame's own `playVideo()`/`pauseVideo()`/`seekTo()` — the
same way it already routes every other command source. See
`src/definitions.ts`'s `WebKeepAliveMeta` doc comment and
`docs/MUSIC_BACKGROUND_KEEPALIVE.md` for the full design and current
verification status.
