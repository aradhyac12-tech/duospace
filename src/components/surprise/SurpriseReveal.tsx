...
import { analyzeSurpriseContent, buildHapticScore, playHapticMovie, type SurpriseMood } from "@/lib/surpriseHaptics";
...
const SurpriseReveal = ({ surprise, visible, onClose }: SurpriseRevealProps) => {
  ...
  // Fires the full "haptic score" once per surprise, when it actually
  // becomes visible — open beat immediately, then a soft ambient loop
  // paced off the surprise's own CSS animation duration for as long as
  // the card stays open (see lib/surpriseHaptics.ts). Cancelled on
  // unmount/surprise-change so nothing from a closed surprise can fire
  // late into whatever opens next.
  const score = useMemo(() => buildHapticScore(analysis), [analysis]);
  const stopMovieRef = useRef<(() => void) | null>(null);
  useEffect(() => {
    if (!visible) return;
    stopMovieRef.current = playHapticMovie(score, analysis.contentDurationMs);
  ...
  const deviceTilt = useDeviceTilt(!expanded && !instant);
  useEffect(() => {
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    if (expanded || instant || deviceTilt.active) return; // gyroscope takes over once it's actually reporting
              ...
              <motion.div
                ...
                className={cn(
                  ...
                  expanded
                    ...
                    : "rounded-[1.75rem] bg-background/55 backdrop-blur-2xl"
                )}
              >
                ...
                <div className="relative z-30 flex items-center justify-between px-4 pt-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {!expanded && (
                      <button
                        onClick={() => {
                          // Stop the ambient loop and hand off to the
                          // expand beat — a bigger, more confident hit
                          // than open/ambient, since committing to the
                          // full-screen experience is its own moment.
                          // Without stopping the loop first, the ambient
                          // interval would keep firing underneath expand's
                          // sequence and muddy it.
                          stopMovieRef.current?.();
                          import("@/lib/surpriseHaptics").then(({ playHapticSequence }) => playHapticSequence(score.expand));
                          setExpanded(true);
                        }}
                    ...
                    <button
                      ref={closeButtonRef}
                      onClick={() => {
                        // Close ALWAYS uses score.close, never a generic
                        // tap — this is the one beat that should never be
                        // loud regardless of mood (it plays as the partner
                        // is already leaving the surprise).
                        stopMovieRef.current?.();
                        import("@/lib/surpriseHaptics").then(({ playHapticSequence }) => playHapticSequence(score.close));
                        onClose();
                      }}
                      className="h-10 w-10 rounded-full bg-muted/70 flex items-center justify-center"
