...
export function useChatRealtimeMessages(
  ...
  useEffect(() => {
    ...
    // SECURITY/PERF FIX (UPDATE only — see note below on DELETE): this
    // listener used to have no filter at all — every authenticated client
    // received every UPDATE on the entire `messages` table (every
    // conversation, every user pair), the same metadata-leak class of bug
    // INSERT was already fixed for above (see the BUG-02 comment), and
    // real "event amplification" (section 6): every client ran a
    // decrypt() attempt against every other pair's edited messages
    // app-wide. Supabase postgres_changes only supports one equality
    // filter per listener, so — same pattern as INSERT — two listeners,
    // deduped by id, scoped to rows where this user is sender or receiver.
    // This is safe to filter on `new.sender_id`/`new.receiver_id` because
    // an UPDATE's WAL record always carries the complete new row
    // regardless of the table's replica identity setting.
    //
    // DELETE is deliberately left UNFILTERED here, not fixed to match:
    // filtering it the same way would require the same two-listener
    // sender_id/receiver_id pattern, but a DELETE's old-row image only
    // contains the columns covered by the table's REPLICA IDENTITY —
    // which for `messages` is still the default (primary key only, see
    // supabase/migrations — no `REPLICA IDENTITY FULL` for this table).
    // Filtering on sender_id/receiver_id against a DELETE's old row today
    // would silently match nothing, since those columns aren't present in
    // the payload, breaking realtime delete-sync entirely — a worse
    // regression than the leak it would fix. Doing this safely needs a
    // schema migration (`ALTER TABLE public.messages REPLICA IDENTITY
    // FULL;`) applied and verified first; flagged in the final report
    // rather than applied here since it touches the live schema.
    const seenDeleteIds = new Set<string>();
    const handleDelete = (payload: { old: Record<string, unknown> }) => {
      const id = (payload.old as any)?.id;
      if (!id || seenDeleteIds.has(id)) return;
