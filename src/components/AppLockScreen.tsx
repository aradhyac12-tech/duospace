...
import { hapticTick, hapticLight, hapticMedium, hapticSuccess, hapticWarning, hapticError } from "@/lib/haptics";
...
const AppLockScreen = () => {
  ...
  const handlePinDigit = async (digit: string) => {
    ...
    if (next.length === PIN_LENGTH) {
      ...
      if (ok) {
        setPin(""); setAttempts(0); hapticSuccess(); setIsAppLocked(false);
      } else {
            ...
            {isLockedOut ? (
              ...
              <>
                ...
                <div className="grid grid-cols-3 gap-4 w-full max-w-[260px]">
                  {DIGITS.flat().map((d, i) => (
                    <button key={i}
                      onClick={() => { hapticTick(); if (d === "⌫") handlePinBackspace(); else if (d) handlePinDigit(d); }}
