...
## Android APK: headless CI (primary path)

`.github/workflows/android-build.yml` builds a signed release APK on
GitHub Actions with no Android Studio involved — dispatch it manually, on
push to `main` when native-relevant paths change, or from the Lovable
"Start Android Build" button. Order: `npm ci` → dependency validation →
web build → `cap add/sync android` → native plugin-registration
validation → signing → `gradlew assembleRelease` → APK inspection →
artifact upload. Every stage fails loudly with a specific cause instead of
producing a silently-broken APK.

Required repo secrets for a signed release build:
`ANDROID_KEYSTORE_BASE64` (keystore file, base64-encoded),
`ANDROID_KEYSTORE_PASSWORD`, `ANDROID_KEY_ALIAS`, `ANDROID_KEY_PASSWORD`.
Without them, dispatch with `build_type: debug` instead — release builds
refuse to fall back to an unsigned artifact.

## Prerequisites (manual/local path below)
- Node.js 24+
- Android Studio (for Android APK, manual path only)
...
## 4. Android (manual — prefer the CI workflow above)
```bash
npm run cap:add:android   # first time — verifies deps, cap add android, patches, verifies native
npm run cap:sync          # subsequent syncs
...
- `MODIFY_AUDIO_SETTINGS` — required alongside RECORD_AUDIO for WebRTC (Daily.co) call audio to actually work, not just be granted
