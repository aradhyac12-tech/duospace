...
## Addendum (2026-09-20) — `surprise_library`

New table (`20260920110000_surprise_library.sql`): saved surprise sources. RLS enabled; SELECT /
INSERT / UPDATE / DELETE policies are all `owner_id = auth.uid()` (UPDATE also `WITH CHECK`, so an
owner can't reassign a row to another uid). There is intentionally NO partner policy — unlike
`code_surprises`, whose SELECT policy exposes every row (active or not) to the creator's partner, so
drafts must not live there. Static review + `check-rls-coverage.mjs` only; REQUIRES LIVE SUPABASE to
