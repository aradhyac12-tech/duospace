...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  avatar_url text, mood_emoji text DEFAULT '😊', mood_text text DEFAULT 'Feeling good',
  mood_updated_at timestamptz DEFAULT now(),
  partner_id uuid, gallery_shared boolean NOT NULL DEFAULT false,
  public_key text, gender text, phone_number text, pet_name text,
  username text UNIQUE, location_mode text NOT NULL DEFAULT 'on_open',
  push_token text, push_platform text, couple_theme text,
  last_seen_at timestamptz, tracking_state text DEFAULT 'offline',
  app_visibility text DEFAULT 'visible', device_platform text,
...
CREATE TABLE IF NOT EXISTS public.messages (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  sender_id uuid NOT NULL, receiver_id uuid NOT NULL, content text,
  message_type text NOT NULL DEFAULT 'text' CHECK (message_type IN ('text','image','file','voice')),
  file_url text, file_name text,
  is_read boolean NOT NULL DEFAULT false, reply_to_id uuid,
  disappear_at timestamptz,
  deleted_by_sender boolean NOT NULL DEFAULT false,
  deleted_by_receiver boolean NOT NULL DEFAULT false,
  edited_at timestamptz, is_pinned boolean NOT NULL DEFAULT false,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.countdowns (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  creator_id uuid NOT NULL, title text NOT NULL, target_date timestamptz NOT NULL,
  emoji text DEFAULT '🎉', created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.memories (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  creator_id uuid NOT NULL, image_url text, caption text,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.taps (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  sender_id uuid NOT NULL, receiver_id uuid,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.daily_answers (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL, question text NOT NULL, answer text NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.gallery_items (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  owner_id uuid NOT NULL, file_url text NOT NULL, file_name text,
...
CREATE TABLE IF NOT EXISTS public.call_history (
  ...
  duration_seconds integer DEFAULT 0, room_name text,
...
CREATE TABLE IF NOT EXISTS public.playlist_songs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  added_by uuid NOT NULL, title text NOT NULL, artist text NOT NULL DEFAULT '',
  song_url text NOT NULL, platform text NOT NULL DEFAULT 'youtube',
  thumbnail_url text, created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.invite_links (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  code text NOT NULL UNIQUE, creator_id uuid NOT NULL,
  created_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS public.shayaris (
  ...
  is_favorite boolean NOT NULL DEFAULT false, delete_requested_by uuid,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.blend_invites (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  sender_id uuid NOT NULL, status text NOT NULL DEFAULT 'pending',
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.mood_logs (
  ...
  confidence real NOT NULL DEFAULT 0, valence real DEFAULT 0, arousal real DEFAULT 0.5,
  feedback text,
...
CREATE TABLE IF NOT EXISTS public.code_surprises (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  creator_id uuid NOT NULL, title text NOT NULL DEFAULT 'Surprise',
  html_content text NOT NULL DEFAULT '', css_content text NOT NULL DEFAULT '',
  js_content text NOT NULL DEFAULT '',
  max_views integer NOT NULL DEFAULT 1, views_used integer NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS public.code_surprise_events (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  surprise_id uuid NOT NULL, user_id uuid NOT NULL, event_type text NOT NULL,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.menstrual_cycles (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL, cycle_start_date date NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.scheduled_messages (
  ...
  sent boolean NOT NULL DEFAULT false, is_processing boolean NOT NULL DEFAULT false,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.pending_uploads (
  ...
  total_chunks integer NOT NULL, uploaded_chunks integer NOT NULL DEFAULT 0,
  total_bytes bigint, content_type text,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS public.rate_limits (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL, bucket text NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.qr_pairing_tokens (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL, token_hash text NOT NULL UNIQUE,
  token_type text NOT NULL DEFAULT 'device_pairing' CHECK (token_type IN ('device_pairing','signup_invite')),
  created_at timestamptz NOT NULL DEFAULT now(),
  expires_at timestamptz NOT NULL,
  redeemed_at timestamptz, redeemed_ip text, redeemed_ua text,
  issuer_ip text, issuer_ua text
);
...
CREATE TABLE IF NOT EXISTS public.webauthn_credentials (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL, credential_id text NOT NULL UNIQUE,
  public_key text NOT NULL, counter bigint NOT NULL DEFAULT 0,
  transports text[] NOT NULL DEFAULT '{}',
  device_name text, aaguid text,
...
CREATE TABLE IF NOT EXISTS public.webauthn_challenges (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid, challenge text NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.email_change_otps (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL, email text NOT NULL, otp_hash text NOT NULL,
...
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN DELETE FROM public.messages WHERE disappear_at IS NOT NULL AND disappear_at <= now(); END; $$;
...
RETURNS SETOF public.scheduled_messages LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  UPDATE public.scheduled_messages SET is_processing = true
   WHERE id IN (SELECT id FROM public.scheduled_messages WHERE sent = false AND is_processing = false AND send_at <= now() ORDER BY send_at ASC LIMIT 25)
  RETURNING *;
$$;
...
-- Efficient email→user_id lookup for edge functions (replaces admin.listUsers).
